...
seo-description: A WCAG 2.0 consiste em um conjunto de diretrizes de tecnologia independentes e critérios de sucesso para ajudar a tornar o conteúdo da Web acessível e utilizável para pessoas com necessidades especiais.
...
O campo **Texto alternativo** está disponível na guia propriedades **Avançadas** de imagem na caixa de diálogo do componente de ** Imagem:**

![Editar a caixa de diálogo do componente de Imagem na interface clássica; mostra o campo de Texto alternativo.](assets/chlimage_1-17.png)

O AEM adiciona um **Texto alternativo** em suas imagens por padrão. Para a interface clássica existem dois cenários diferentes para a forma como o atributo padrão é criado (embora o valor padrão possa não ser suficiente como uma alternativa e provavelmente precisará ser editado na guia de propriedades ** Avançadas** de imagem):

* Arquivo:

   Uma imagem é carregada do disco rígido do usuário. Se você adicionar um componente de imagem a uma página e, em seguida, escolher uma imagem do seu disco rígido ou de outra fonte, o valor padrão para **Texto alternativo** será `file`. Isso deve ser alterado na guia de propriedades **Avançadas** de imagem. Mais uma vez, esse valor não é exibido no campo **Texto alternativo,** mas quando é alterado, o novo valor é mostrado no campo.

* Ativo:

   Uma imagem é adicionada do repositório de ativos digitais. Se você arrastar uma imagem do repositório de ativos digitais para uma página da Web, em seguida, os valores **Título** e **Texto alternativo** serão transferidos dos metadados para a imagem.

>[!NOTE]
>
>Em ambos os cenários acima, o valor padrão **Texto alternativo** não é visível na guia **Propriedades avançadas de imagem**. Para alterar o valor padrão, basta inserir um novo valor no campo **Texto alternativo**.

>[!NOTE]
>
>Se a imagem for meramente decorativa (consulte [Criar boas alternativas em texto](#creating-good-text-alternatives)), você pode inserir um espaço no campo **Texto alternativo** usando a barra de espaço. Isso criará um atributo vazio `alt`, que solicitará que um leitor de tela ignore a imagem.
...
* Fotos ilustrativas:
   ...
   Estas são imagens de pessoas, objetos ou lugares. Pense no papel da foto na página; um equivalente de texto apropriado pode ser *Foto de [object]*, mas pode depender do texto ao redor.

* Ícones:

   Estes são pequenos pictogramas (gráficos) que transmitem informações específicas. Eles devem ser usados de forma consistente em uma página e um site. Todas as instâncias do ícone em uma página ou um site devem ter a mesma alternativa em texto curta e sucinta, a menos que isso resulte em duplicação desnecessária do texto adjacente.

* Gráficos e gráficos:

   Normalmente, representam dados numéricos. Dessa forma, uma opção para fornecer uma alternativa em texto pode ser incluir um breve resumo das principais tendências indicadas na tabela ou gráfico. Se necessário, forneça também uma descrição mais detalhada no texto usando o campo **Description** na guia **Advanced** de propriedades de imagem. Além disso, você pode fornecer os dados de origem em forma de tabela em outro lugar na página ou no site.

   ![Exemplo de um gráfico. Abaixo é apresentada a melhor abordagem para fornecer uma alternativa.](assets/chlimage_1-2.jpeg)

   Para fornecer uma alternativa para este gráfico de exemplo, adicione um texto conciso `alt` à própria imagem e, em seguida, siga a imagem com uma alternativa em texto completo.
...
* Imagens de plano de fundo:
   ...
   Isso é feito usando folhas de estilos em cascata (CSS) em vez de HTML. Isso significa que não é possível especificar um valor de texto alternativo. Por conseguinte, as imagens de fundo não devem fornecer informações textuais importantes - se o fizerem, essas informações também devem ser fornecidas no texto da página.
...
O AEM facilita a criação de páginas da Web usando os elementos de HTML adequados. Abra o conteúdo da página no RTE (um componente de Texto) e use o menu **Formatar** para especificar o elemento estrutural adequado (por exemplo, parágrafo, cabeçalho, etc.).

A imagem a seguir mostra o texto que foi estilizado como o texto do parágrafo; a exibição do código de origem usado mostra que ele tem as tags corretas de abertura e fechamento &lt;p> &lt;/p>.

![Um exemplo do elemento Parágrafo mostrado no modo de edição da origem (interface clássica).](assets/chlimage_1-18.png)

É possível verificar se as suas páginas da Web têm a estrutura adequada através:

* **Utilização de cabeçalhos:**

   Desde que você tenha os recursos de acessibilidade do RTE ativados (consulte [AEM e Acessibilidade](/help/sites-administering/rte-accessible-content.md)), AEM os níveis do oferta 3 do cabeçalho da página. É possível usá-los para identificar seções e subseções de conteúdo. O cabeçalho 1 é o nível mais alto, o Cabeçalho 3 o mais baixo. O administrador do sistema pode configurar o sistema para permitir o uso de mais níveis de cabeçalho.

   A imagem a seguir mostra um exemplo dos diferentes tipos de cabeçalhos.

   ![Os cabeçalhos H1 até o H3 são exibidos no seletor suspenso (interface clássica).](assets/chlimage_1-19.png)
...
* **Texto destacado**:
   ...
   * Clique no ícone **B** (para &lt;strong>) ou no ícone **I** (para &lt;em>) mostrado no painel **Propriedades** (verifique se o HTML está selecionado).
   ...
   * &lt;i> para &lt;em>
   ...
   Eles são efetivamente iguais, mas &lt;strong> e &lt;em> são preferíveis, já que são um html semanticamente correto. Sua equipe de desenvolvimento pode configurar o RTE para usar &lt;strong> e &lt;em> (em vez de &lt;b> e &lt;i>), ao desenvolver a instância do projeto.

* **Use listas**: você pode usar o HTML para especificar três diferentes tipos de listas:

   * O elemento `<ul>` é usado para listas *desordenadas* listas (com marcadores). Os itens da lista individual são identificados usando o elemento `<li>`.
   ...
   * O elemento `<ol>` é usado para as listas *numeradas*. Os itens da lista individual são identificados usando o elemento `<li>`.
   ...
   Caso deseje alterar o conteúdo existente em um tipo de lista específica, destaque o texto e selecione o tipo de lista adequado. Como no exemplo anterior que mostra como o texto do parágrafo é inserido, os elementos desejados da lista são adicionados automaticamente ao seu HTML, mas você pode visualizá-los na exibição da edição da fonte.
...
* **Use tabelas**:
   ...
   * um elemento `<td>` para cada célula de dados
   ...
   >[!NOTE]
   As tabelas devem ser observadas com o componente **Tabela**. Embora as tabelas possam ser criadas no componente Texto, isso não é recomendado.
   ...
   * O `scope` atributo do elemento `<th>` é usado para indicar se uma célula representa um cabeçalho de uma linha ou de uma coluna específica. Uma abordagem semelhante é a de usar o cabeçalho e os atributos de id em tabelas complexas, onde as células de dados podem ser associadas a um ou mais cabeçalhos.
   ...
   Ao adicionar uma **Tabela** é possível configurar as **Propriedades da tabela** usando uma caixa de diálogo.
   ...
   * Remova qualquer valor padrão para **Largura**, **Altura**, **Borda**, **Preenchimento da célula e** **Espaçamento entre células**. já que essas propriedades podem ser definidas em uma planilha de estilos global.
   ...
   ![Caixa de diálogo de propriedades da tabela.](assets/chlimage_1-20.png)
   ...
   ![Caixa de diálogo das propriedades da célula; definir uma linha (geralmente a primeira) como a de cabeçalho.](assets/chlimage_1-21.png)
...
Você também pode editar o título da página ao editar uma página, que pode ser acessada pela guia **Sidekick** - **Página** - **Propriedades da página...**
...
![Guia Título e Texto (janela de edição); o titulo &#39;Descrição&#39; foi adicionado.](assets/chlimage_1-22.png)
...
![Botão de imagem com o texto alternativo definido no campo Título (janela de edição).](assets/chlimage_1-23.png)
...
![Adicionando itens ao grupo de opções. O título do grupo é &#39;Entrar em contato por&#39; - definido no campo Título.](assets/chlimage_1-24.png)
...
![Adicione informações adicionais (a palavra “obrigatório”) para os usuários de leitores de tela no campo &#39;Título&#39;.](assets/chlimage_1-25.png)
