...
* Imagens de plano de fundo: Isso é feito usando folhas de estilos em cascata (CSS) em vez de HTML. Isso significa que não é possível especificar um valor de texto alternativo. Por conseguinte, as imagens de fundo não devem fornecer informações textuais importantes - caso o façam, essas informações também devem ser fornecidas no texto da página.
