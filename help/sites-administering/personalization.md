...
A personalização centra-se no fornecimento ao utilizador de um ambiente adaptado que exiba conteúdos dinâmicos selecionados de acordo com as suas necessidades específicas; seja com base em perfis predefinidos, na seleção de usuários ou no comportamento interativo do usuário.

Há três elementos principais envolvidos na personalização:

**Usuários**

* têm perfis individuais e de grupo. Esses perfis contêm características (como descrição da tarefa, local, interesses) que podem ser usadas para personalizar o conteúdo que podem ver.
* tome medidas. Eles podem então ser analisados e comparados às regras de comportamento para adaptar o conteúdo que veem.

**Conteúdo**

* é o que o usuário deseja ver. De preferência, o conteúdo dos interesses e a sua utilização para o desempenho das suas funções.
...
* Baseado em regras: os gerentes de negócios definem regras específicas para ações com base em perfis e/ou comportamento específicos.
...
**Grupos de usuários-alvo específicos, limitados (extranets)**

* Os utilizadores requerem um início de sessão para autorização; Estar ligado a um perfil que forneça as informações necessárias à personalização; possivelmente detalhes como localização, relacionamento com o produto, histórico de utilização, responsabilidades orçamentárias etc.
...
* Empresas que fornecem sítios Web que permitem ao seu cliente visualizar as informações relativas à conta corrente e à faturação; por exemplo, provedores de telefone.

**Site de vendas e distribuição**

* Sites de vendas e distribuição, como a Amazon, podem combinar um perfil de usuário, o histórico de vendas do usuário e seu histórico de navegação para fazer sugestões sobre o que poderá interessar ao usuário em seguida.
...
### Força de personalização e pontos a considerar {#strengths-of-personalization-and-points-to-consider}
...
* Os recursos de colaboração social estão disponíveis para que os usuários se comuniquem entre si, pois eles podem ser identificados por seus perfis.
* Um usuário pode receber o conteúdo necessário para realizar uma tarefa específica. Na intranet de uma empresa, isso pode fornecer uma ferramenta inestimável para a disseminação de informações.
* Um usuário pode receber o conteúdo que precisa/deseja, reduzindo o tempo necessário para executar operações de pesquisa.
* O provedor de conteúdo pode direcionar o conteúdo a ser visto por categorias específicas de usuários.
...
* Naturalmente, a análise e a avaliação suplementares têm impacto no desempenho. No entanto, os métodos utilizados são altamente sofisticados e podem ser otimizados para minimizar o impacto.
...
* O armazenamento em cache é uma consideração essencial ao configurar a personalização e o tempo deve ser considerado para garantir que a implementação correta seja usada. Esta questão será debatida mais tarde.
...
A personalização deve ser considerada separadamente do controle de acesso, mas elas interrelacionam-se.
...
| Campo de endereço do formulário | Um campo complexo que permite a entrada de um endereço internacional. |
| Início dos formulários | Inicia uma definição de formulário |
| Captcha de formulários | Um campo que consiste em uma palavra alfanumérica que é atualizada automaticamente. O componente de captcha protege websites contra bots. |
| Grupo de caixa de seleção Formulários | Vários itens organizados em uma lista e precedidos por caixas de seleção. Os usuários podem selecionar várias caixas de seleção. |
| Lista suspensa de formulários | Vários itens organizados em uma lista suspensa. A opção Multisselecionáveis especifica se vários elementos podem ser selecionados na lista. |
| Fim de formulários | Encerra a definição do formulário. |
| Upload de arquivo de formulários | Um elemento de carregamento que permite ao usuário carregar um arquivo no servidor. |
| Campo oculto de formulários | Este campo não é exibido para o usuário. Ele pode ser usado para transportar um valor para o cliente e de volta para o servidor. Esse campo não deve ter nenhuma restrição. |
| Botão Imagem de formulários | Um botão de envio adicional para o formulário que é renderizado como uma imagem. |
| Campo de senha dos formulários | Igual ao campo de texto, mas somente uma única linha é permitida e a entrada de texto do usuário não fica visível no campo. |
| Grupo de opções de formulários | Vários itens organizados em uma lista precedida por um botão de rádio. Os usuários devem selecionar somente um botão de rádio. |
| Botão Enviar formulários | Um botão de envio adicional para o formulário, onde o título é exibido como texto no botão. |
| Campo de texto de formulários | Campo de texto que permite que os usuários insiram informações. |
...
Recursos da comunidade, como blogs, fóruns e calendários, resultam na criação de conteúdo da comunidade, comumente conhecido como conteúdo gerado pelo usuário (UGC). Quando o UGC é inserido em um ambiente de publicação que consiste em várias instâncias do AEM (um farm [de](/help/communities/topologies.md)publicação), um problema importante é como sincronizar o UGC em todas as instâncias.

Com a extensão do [AEM Communities 6.1](/help/communities/overview.md) , esse problema é resolvido usando uma loja [comum para UGC](/help/communities/working-with-srp.md). No que diz respeito à personalização, Comunidades inclui logon [](/help/communities/social-login.md) social - a capacidade de fornecer a opção para que os visitantes do site façam logon com Facebook e Twitter.
...
* enviar o UGC da instância de publicação para o ambiente do autor, de onde ele pode ser publicado de uma maneira semelhante ao conteúdo da página de publicação
