---
title: Mapeamento de dados de componentes com as propriedades do Adobe Analytics
seo-title: Mapeamento de dados de componentes com as propriedades do Adobe Analytics
...
# Mapeamento de dados de componentes com as propriedades do Adobe Analytics{#mapping-component-data-with-adobe-analytics-properties}
...
1. Para disponibilizar a estrutura na instância de publicação do site, abra a **página **guia do sidekick e clique em **Ativar estrutura.**

## Mapeamento de variáveis relacionadas ao produto {#mapping-product-related-variables}

O AEM usa uma convenção para nomear variáveis e eventos relacionados ao produto que devem ser mapeados para as propriedades relacionadas ao produto do Adobe Analytics:
...
| `product.events.*eventName*` | Os eventos bem-sucedidos para associar ao produto em seu relatório. | `product.events` é o prefixo de eventos chamados *eventName.* |
...
1. Em **Relatórios** (no lado esquerdo da página), selecione Conversão **** personalizada, Conversão **personalizada 1-10** e selecione a variável correspondente a `eVar7`
...
## Uso do Content Finder (cf#) com estruturas do Adobe Analytics {#using-the-content-finder-cf-with-adobe-analytics-frameworks}
...
1. Quando estiver na exibição do Adobe Analytics, o Localizador de conteúdo exibirá todas as variáveis CQ pertencentes aos componentes de rastreamento arrastados na exibição do CQ.

   * Por exemplo, se o **componente de download **for o *único arrastado* na exibição do CQ (que tem duas variáveis mapeáveis *eventdata.downloadLink* e *eventdata.events.startDownload*), o Localizador de conteúdo terá a seguinte aparência ao alternar para a exibição do Adobe Analytics:
...
## Uso da exibição do AEM e da exibição do Analytics {#using-aem-view-and-analytics-view}
...
1. **Visualização AEM**:
   ...
   1. Se a estrutura tiver componentes herdados de seu pai, eles serão listados aqui, juntamente com as variáveis mapeadas para os componentes.
      ...
      1. Para desbloquear um componente herdado, basta clicar duas vezes no cadeado ao lado do nome do componente
   ...
   1. Você pode encontrar todos os componentes incluídos atualmente na estrutura de análise:
      ...
      1. Lembre-se de que um componente só pode ser excluído da estrutura em que foi criado e não pode ser excluído das estruturas secundárias no sentido tradicional (eles só podem ser substituídos).
...
1. Lado esquerdo: Localizador de conteúdo (cf#) preenchido pelas variáveis CQ com base nos componentes arrastados para a estrutura na exibição CQ.
1. Cabeçalhos de guia (exibição **do** AEM e exibição **do** Analytics): use-os para alternar entre as duas exibições.

1. As três tabelas (Tráfego, Conversão, Evento) listam todas as variáveis disponíveis do Adobe Analytics. pertencente aos RSID selecionados. Os mapeamentos mostrados aqui devem ser os mesmos da exibição do AEM:
