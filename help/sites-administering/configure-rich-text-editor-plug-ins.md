...
<table> 
 <tbody> 
  <tr> 
   <td><strong> </strong></td> 
   <th><strong>Habilitar todos os recursos<br /> </strong></th> 
   <th><strong>Habilite alguns recursos específicos</strong></th> 
  ...
  <tr> 
   ...
   <td>String</td> 
   <td>Sequência de caracteres (várias sequências; defina Tipo como String e clique em Multi in CRXDE Lite)</td> 
   <td>String</td> 
  </tr> 
  <tr> 
   <td><strong>Valor</strong></td> 
   <td>* (um asterisco)<br /> </td> 
   <td>definir para um ou mais valores de recurso</td> 
   <td><strong>-</strong></td> 
  </tr> 
 </tbody> 
</table>
...
|---|---|---|
| `allowBlockTags` | String | Define a lista de tags de bloco permitidas. Algumas tags de bloco possíveis incluem: <ul> <li>manchetes (h1, h2, h3)</li> <li>alínea p)</li> <li>listas (ol, ul)</li> <li>tabelas (tabela)</li> </ul> |
| `fallbackBlockTag` | String | Define a tag de bloco usada para qualquer bloco que tenha uma tag de bloco não incluída em `allowBlockTags`. `p` na maioria dos casos, é suficiente. |
...
>[!NOTE]
>
>Você pode definir Estilos para [tabelas ou células de tabela](/help/sites-administering/configure-rich-text-editor-plug-ins.md#table-styles). Essas configurações exigem procedimentos separados.
...
1. Na definição do componente, navegue até o nó `<rtePlugins-node>/styles`, como criado em [Ativação do seletor suspenso de estilo](#style-selector-list).
...
Primeiro habilite o `paraformat` plug-in:
...
1. Na definição do componente, navegue até o nó `<rtePlugins-node>/paraformat`, como criado em [Ativação do seletor suspenso de formato](#style-selector-list).
...
## Configurar caracteres especiais {#special-char}
...
![No CRXDE, adicione um único caractere a ser disponibilizado na barra de ferramentas do RTE](assets/chlimage_1-412.png "No CRXDE, adicione um único caractere a ser disponibilizado na barra de ferramentas do RTE")



### Definir um intervalo de caracteres {#define-range-char}

1. Use os passos 1 a 3 de [Definição de um caractere único](#define-single-char).
...
1. Salve as alterações.
   ...
   ![No CRXDE, defina um intervalo de caracteres a serem disponibilizados no RTE](assets/chlimage_1-413.png)

         *No CRXDE, defina um intervalo de caracteres a serem disponibilizados no RTE*

   ![Caracteres especiais disponíveis no RTE são exibidos para os autores em uma janela pop-up](assets/rtepencil.png)
...
1. Defina o local das folhas de estilos CSS para referenciá-las. Consulte [Especificação do local da folha de estilos](#location-stylesheet) já que isso é o mesmo que definir [estilos para texto](#text-styles). O local pode ser definido se você tiver definido outros estilos.
...
1. Crie um novo nó (no `table-styles` ou `cellStyles` (conforme apropriado) para representar um estilo individual:
...
* `hidden-headerEditingCSS`: Especifica o nome da classe CSS aplicada na célula de cabeçalho oculta, quando o RTE é editado.
* `hidden-headerEditingStyle`: Especifica uma sequência de estilo que é aplicada na célula de cabeçalho oculta quando o RTE é editado.
...
* `hidden-headerClassName`: Especifica o nome da classe CSS aplicada na célula do cabeçalho oculta no modo de visualização.
* `hidden-headerStyle`: Especifica uma sequência de estilo que é aplicada na célula do cabeçalho oculto no modo de visualização.
...
1. Crie um novo nó no mesmo nível que `<rtePlugins-node>`, ou seja, crie o nó sob o nó pai de `<rtePlugins-node>`:
   ...
   * **Valor** `richtext`
   ...
   A localização da variável `../items/text` pode variar, dependendo da estrutura da caixa de diálogo; dois exemplos incluem:
