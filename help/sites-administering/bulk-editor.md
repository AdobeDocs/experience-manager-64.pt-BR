...
Por padrão, a funcionalidade de importação fica oculta quando você abre o Editor de itens em massa. A simples adição do parâmetro `hib=false` ao URL exibirá o botão **Importar** na página do Editor em massa. É possível importar conteúdo de qualquer arquivo separado por tabulação ( `.tsv`). Para que a importação funcione corretamente, os cabeçalhos de coluna (primeira linha de células) devem corresponder aos cabeçalhos de coluna da tabela para a qual você está importando.
