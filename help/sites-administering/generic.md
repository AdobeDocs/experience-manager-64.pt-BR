...
## Produtos e variações de produto {#products-and-product-variations}

>[!NOTE]
>
>Os procedimentos a seguir se aplicam a Produtos e Variações de Produto.

Antes de criar os produtos, é necessário definir um [scaffold](/help/sites-authoring/scaffolding.md). Isso especifica os campos necessários para definir os produtos e como eles são editados.
...
1. Navegue até o console **Produtos**, por **Commerce**.
1. Usando o console **Products**, navegue até o local desejado.
...
1. Especifique:
   ...
   * **Importador**
      ...
      O importador do provedor de comércio [específico](/help/sites-administering/concepts.md#commerce-providers), por padrão `Geometrixx`.
...
1. Selecione **Próximo** para importar os produtos, será mostrado um log das ações realizadas.
   ...
   >[!NOTE]
   >
   >O uso repetido de **Next** e **Back** irá importar repetidamente as definições do produto. No entanto, como eles têm os mesmos SKUs, as informações existentes no repositório serão simplesmente substituídas.

1. Selecione **Concluído** para fechar o assistente.

#### Importar produtos - Interface clássica {#importing-products-classic-ui}

1. Usando o console **Ferramentas** abra a pasta **Commerce**.
1. Clique com o duplo para abrir o **Importador do produto**:
...
1. Especifique:
   ...
   * **Provedor de comércio**
      ...
      O importador para seu [provedor de comércio](/help/sites-administering/concepts.md#commerce-providers); por padrão.
...
>[!NOTE]
>
>O gerenciamento de produtos padrão é básico, pois o conjunto de produtos Geometrixx-Outdoors foi mantido básico. A complexidade se baseia no produto [andaime](/help/sites-authoring/scaffolding.md), portanto, com o seu próprio andaime de produto é possível obter uma edição mais sofisticada.

#### Criar informações do produto - IU otimizada ao toque {#creating-product-information-touch-optimized-ui}

1. Usando o console **Products** (via **Commerce**), navegue até o local desejado.
...
1. O assistente abrirá. Use as **Basic** e **guias de produto** para inserir os [atributos de produto](/help/sites-administering/concepts.md#product-attributes) para o novo produto ou variante de produto.

   >[!NOTE]
   >
   >**O** título e o  **** SKU são os mínimos necessários para criar um produto ou uma variante.

1. Selecione **Criar** para guardar as informações.
...
>
>Isso significa que, por padrão, eles são bloqueados pelo [dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html), portanto, configure conforme necessário.

#### Editar informações do produto - IU otimizada ao toque {#editing-product-information-touch-optimized-ui}

1. Usando o console **Products** (via **Commerce**), navegue até as informações do seu produto.
1. Usar:
...
1. Os [atributos do produto](/help/sites-administering/concepts.md#product-attributes) serão mostrados. Use **Editar** e **Concluído** para fazer quaisquer alterações.

### Mostrando referências de produto {#showing-product-references}

#### Referências do produto exibidas - IU otimizada ao toque {#showing-product-references-touch-optimized-ui}
...
1. Navegue até o console **Produtos**, por **Commerce**.
1. Usando o console **Products**, navegue até o produto desejado.
...
1. Selecione o ativo que deseja adicionar.
   ...
   >[!NOTE]
   >
   >Os ativos que você pode selecionar são de [Assets](https://helpx.adobe.com/experience-manager/aem-previous-versions.html#assets).
...
1. Toque/clique no ícone Concluído. Agora você precisa [implantar](#rolling-out-a-catalog) suas alterações.
...
1. [](#rolling-out-a-catalog) Implantar suas alterações.
...
1. Toque/clique em **Dados de produto de Visualização**.
1. Toque/clique em **Editar**.
1. Digite uma **Categoria de ativos padrão**.
1. Toque/clique em **Concluído**.
1. [](#rolling-out-a-catalog) Implantar suas alterações.

### Publicar/Cancelar publicação de informações do produto {#publishing-unpublishing-product-information}

#### Publicar/Cancelar publicação de informações do produto - IU otimizada ao toque {#publishing-unpublishing-product-information-touch-optimized-ui}
...
>[!NOTE]
>
>Leia também [manipulação de Eventos em AEM](https://blogs.adobe.com/experiencedelivers/experience-management/event_handling_incq/). [](https://blogs.adobe.com/experiencedelivers/experience-management/event_handling_incq/)
...
1. Arraste e solte uma imagem no componente do navegador [assets](/help/sites-authoring/author-environment-tools.md#assets-browser).
1. Você pode:
...
1. Clique na forma.
1. Clicar no ícone Procurar abre o seletor [Ativo](/help/assets/asset-selector.md#using-the-asset-selector).
...
1. Alterne para o modo [pré-visualização](/help/sites-authoring/editing-content.md#previewing-pages).
1. Clique no ponto de conexão +. Uma caixa de diálogo é aberta onde você pode escolher o tamanho e a quantidade do produto inserido em **Caminho**.
...
1. Na lista selecione **Criar catálogo**, o assistente Criar catálogo abrirá.

   ![chlimage_1-337](assets/chlimage_1-337.png)

1. Navegue até o Blueprint do catálogo necessário.
1. Toque/clique no botão **Selecionar** e toque/clique no Blueprint de Catálogo necessário.
1. Toque/clique em **Próximo**.

   ![chlimage_1-338](assets/chlimage_1-338.png)

1. Digite um **Title** e um **Nome**.
1. Toque/clique no botão **Criar**. O catálogo é criado e uma caixa de diálogo é aberta.

   ![chlimage_1-339](assets/chlimage_1-339.png)

1. Tocar/clicar no botão **Concluído** traz você de volta ao console Sites, onde você poderá ver seu catálogo.
...
>[!NOTE]
>
>O catálogo fará referência aos [Dados do Produto](#editing-product-information).

1. Usando o console **Sites**, navegue até **Catalog Blueprint** e, em seguida, selecione o Catálogo básico.
...
1. Clique em **OK** para salvar suas propriedades; os produtos de exemplo serão mostrados em **Critérios de seleção de produto** na página de blueprint.
1. Clique em **Alterações de implantação...**, selecione **Página de implantação e todas as subpáginas**, em seguida, clique em **Próximo** e em **Implantação**. Quando a implementação for concluída com êxito, o indicador **Status** será mostrado como verde.
...
1. Clique em **OK** para salvar suas propriedades; as informações do banner serão mostradas em **Critérios de seleção de produto** na página de blueprint.
1. Implantar essas novas alterações.

### Reversão de um catálogo {#rolling-out-a-catalog}

#### Distribuição de um catálogo - IU otimizada ao toque {#rolling-out-a-catalog-touch-optimized-ui}

Para implementar um catálogo:

1. Navegue até o console **Catálogos**, por **Commerce**.
...
#### Desenvolvendo um catálogo - Interface clássica {#rolling-out-a-catalog-classic-ui}
...
### Importador do Blueprint {#blueprint-importer}

#### Importador do Blueprint - IU otimizada ao toque {#blueprint-importer-touch-optimized-ui}
...
1. Toque/clique em **Concluído** quando a importação estiver concluída.

#### Importador do Blueprint - Interface clássica {#blueprint-importer-classic-ui}
...
>[!NOTE]
>
>O exemplo a seguir trata de uma promoção mantida diretamente em uma [campanha](/help/sites-authoring/personalization.md), usada para comprovantes.
>
...
1. Clique em **Novo**, selecione o modelo **Promoção** e especifique um **Título** (e **Nome**, se necessário) para o seu novo comprovante.
...
1. Edite as **Propriedades**:
   ...
   * selecionando a página no console Sites e usando o menu de contexto (geralmente o botão direito do mouse) para selecionar **Propriedades...** e abra a caixa de diálogo de propriedades
   ...
   Especifique **Tipo de Promoção**, **Tipo de Desconto**, **Valor do Desconto** e quaisquer outros campos, conforme necessário.
...
### Criando um Comprovante {#creating-a-voucher}
...
1. Abra sua nova página de comprovante com um clique de duplo e clique em **Editar** para configurar as informações conforme necessário.
1. Clique em **OK** para salvar.
...
Para alterar as propriedades de um comprovante ou promoção, clique com o duplo no console **Sites** e clique em **Editar**. Depois de salvá-la, você deve ativá-la para que as alterações sejam encaminhadas para as instâncias de publicação.

### Adicionando vendedores a um carrinho {#adding-vouchers-to-a-cart}

Para permitir que os usuários adicionem comprovantes a seus carrinhos, você pode usar o componente **Vouchers** integrado (categoria de comércio). É necessário adicionar isso à mesma página em que o carrinho é exibido (mas não é obrigatório). O componente de comprovantes é apenas um formulário no qual o usuário pode digitar um código de comprovante, é o componente do carrinho de compras que realmente mostra a lista dos comprovantes aplicados e seu desconto.
...
#### Criando informações de pedido - IU otimizada ao toque {#creating-order-information-touch-optimized-ui}
...
1. O assistente abrirá. Use as guias **Basic**, **Content**, **Payment** e **Fulfillment** para inserir as [informações sobre o novo pedido](/help/sites-administering/concepts.md#order-information).
...
1. As [informações do pedido](/help/sites-administering/concepts.md#order-information) serão mostradas. Use **Editar** e **Concluído** para fazer quaisquer alterações.
