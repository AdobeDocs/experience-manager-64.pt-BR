...
* [Compartilhamento de pacotes](#package-share), um servidor centralizado com pacotes disponíveis publicamente e aqueles privados para sua empresa. Os pacotes públicos podem conter hotfixes, nova funcionalidade, documentação, etc.
...
* Excluir pacotes
* Baixe pacotes, como hotfixes, da biblioteca de Compartilhamento de pacotes
* Fazer upload de pacotes para a seção interna da empresa da biblioteca de Compartilhamento de pacotes
...
O **Configurações do pacote** está disponível por meio do **Editar** botão quando [criação](#creating-a-new-package) ou [edição](#viewing-and-editing-package-information) um pacote e fornece três guias para configuração. Depois de fazer qualquer alteração, clique em **OK** para salvá-los.
...
| Descrição | Breve descrição do pacote. A marcação de HTML pode ser usada para formatação. |
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Especifique como as informações de controle de acesso definidas no pacote são tratadas quando o pacote é importado:</p> 
    ...
    </ul> <p>O valor padrão é <strong>Ignorar</strong>.</p> </td> 
   <td> 
    <ul> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td>Você pode definir um caminho ou usar uma expressão regular para especificar todos os nós que deseja incluir.<br /> <br /> A inclusão de um diretório irá: 
    <ul> 
     <li>inclua esse diretório <i>e</i> todos os arquivos e pastas nesse diretório (ou seja, a subárvore inteira)</li> 
     <li><strong>not</strong> incluir outros arquivos ou pastas do caminho raiz especificado</li> 
    </ul> </td> 
   <td>/libs/sling/install(/.*)? </td> 
  </tr> 
  <tr> 
   <td> excluir</td> 
   <td>Você pode especificar um caminho ou usar uma expressão regular para especificar todos os nós que deseja excluir.<br /> <br /> Excluir um diretório excluirá esse diretório <i>e</i> todos os arquivos e pastas nesse diretório (ou seja, a subárvore inteira).<br /> </td> 
...
Os filtros de pacote são definidos com mais frequência ao [criar o pacote](#creating-a-new-package), mas também podem ser editados posteriormente (depois disso, o pacote deve ser recriado).
...
O Gerenciador de pacotes gerencia os pacotes na instalação local do AEM. Depois de [atribuiu as permissões necessárias](#permissions-needed-for-using-the-package-manager) você pode usar o Gerenciador de pacotes para várias ações, incluindo configuração, criação, download e instalação dos seus pacotes. Os principais elementos a serem configurados são:
...
* **/etc/packages** (direitos totais excluindo a exclusão)
...
1. Na tela de boas-vindas AEM, clique em **Pacotes** ou da **Ferramentas** clique duas vezes no console em **Pacotes**).
...
1. Clique em **Editar** para editar o [configurações do pacote](#package-settings).

   Aqui, você pode adicionar informações e/ou definir determinadas configurações; por exemplo, eles incluem uma descrição, a variável [ícone](#package-icons), bugs relacionados e adicionar detalhes do provedor.

   Clique em **OK** depois que terminar de editar as configurações.

1. Adicionar **[Capturas de tela](#package-screenshots)** ao pacote conforme necessário. Uma instância está disponível quando o pacote é criado, adicione mais se necessário usando **Captura de tela do pacote** do sidekick.

   Adicione a imagem real clicando duas vezes no componente de imagem na **Capturas de tela** , adicionar uma imagem e clicar em **OK**.

1. Defina as **[Filtros do pacote](#package-filters)** ao arrastar instâncias do **Definição de filtro** no sidekick, em seguida, clicando duas vezes para abrir para edição:
   ...
   * **Caminho raiz**
O conteúdo a embalar; essa pode ser a raiz de uma subárvore.
   * **Regras**
As regras são opcionais; para definições de pacote simples, não é necessário especificar regras de inclusão ou exclusão.

      Se necessário, você pode definir [**Incluir** ou **Excluir** regras](#package-filters) para definir exatamente o conteúdo do pacote.

      Adicionar regras usando o **+** , como alternativa, remova as regras usando o **-** símbolo. As regras são aplicadas de acordo com sua ordem, de forma que sejam posicionadas conforme necessário usando a variável **Up** e **Down** botões.
   Em seguida, clique em **OK** para salvar o filtro.

   >[!NOTE]
   >
   >Você pode usar quantas definições de filtro forem necessárias, embora seja necessário tomar cuidado para garantir que elas não entrem em conflito. Use **Visualizar** para confirmar o conteúdo do pacote.

1. Para confirmar o que o pacote manterá, você poderá usar **Visualizar**. Isso executa uma execução seca do processo de criação e lista tudo que será adicionado ao pacote quando ele for realmente criado.
...
Geralmente, um pacote é criado ao mesmo tempo que você [criar a definição de pacote](#creating-a-new-package), mas você pode retornar posteriormente para criar ou reconstruir o pacote. Isso pode ser útil se o conteúdo no repositório tiver sido alterado.
...
Reenvolver altera as informações do pacote - *without* alteração do conteúdo do pacote. As informações do pacote são a miniatura, a descrição etc., em outras palavras, tudo o que você pode editar com a **Configurações do pacote** para abrir este clique **Editar**).

Um caso de uso importante para o rewrap é ao preparar um pacote para o compartilhamento de pacotes. Por exemplo, você pode ter um pacote existente e decidir compartilhá-lo com outras pessoas. Para isso, adicione uma miniatura e uma descrição. Em vez de recriar o pacote inteiro com toda a funcionalidade (o que pode levar algum tempo e correr o risco de o pacote não ser mais idêntico ao original), você pode refazer o encapsulamento e adicionar a miniatura e a descrição.

1. Abra a definição de pacote em **Gerenciador de pacotes** (clique no ícone ou nome do pacote).

1. Clique em **Editar** e atualize o **[Configurações do pacote](#package-settings)** conforme necessário. Clique em **OK** para salvar.

1. Clique em **Reembrulhar**, uma caixa de diálogo solicitará confirmação.
...
>[!NOTE]
>
>Consulte [Compartilhamento de pacotes](#package-share) para obter informações sobre o download de hotfixes, pacotes de recursos e pacotes da área pública e da área interna de compartilhamento de pacotes da sua empresa.
>
>No Compartilhamento de pacotes, é possível:
>
>* baixe pacotes de [Compartilhamento de pacotes diretamente na instância de AEM local](#downloading-and-installing-packages-from-package-share).\
   >  Após o download, o pacote é importado para seu repositório, depois você pode instalá-lo imediatamente em sua instância local usando o **Gerenciador de pacotes**. Esses pacotes incluem hotfixes e outros pacotes compartilhados.
>
>* baixe pacotes de [Compartilhamento de pacotes no seu sistema de arquivos](#downloading-packages-to-your-file-system-from-package-share).
>


1. Na tela de boas-vindas AEM, clique em **Pacotes**, em seguida selecione **Gerenciador de pacotes**.
...
>[!NOTE]
>
>Consulte [Upload de pacotes no Compartilhamento de pacotes interno da empresa](#uploading-a-package) para fazer upload de um pacote para a área privada de Compartilhamento de pacotes da sua empresa.
...
1. Clique em **Fazer upload do pacote**.
   ...
   * **Arquivo**
      ...
      Você pode digitar o nome do arquivo diretamente ou usar a variável **Procurar...** para selecionar o pacote necessário do sistema de arquivos local (após a seleção, clique em **OK**).
   ...
   * **Forçar atualização**
   ...
   >[!NOTE]
   >
   >Para disponibilizar o conteúdo para o AEM, certifique-se de [instale o pacote](#installing-packages).
...
* **Validar sobreposições**
   ...
   **Como é reportado**
   ...
   Essas sobreposições são descritas na seção **Log de atividades** do Gerenciador de pacotes.
...
* **Validar ACLs**
   ...
   **Como é reportado**
   ...
   As permissões são descritas na seção **Log de atividades** do Gerenciador de pacotes.
...
1. Selecione o pacote na lista e depois selecione **Mais** lista suspensa do cabeçalho e, em seguida **Validar** no menu suspenso.
...
>[!NOTE]
>
>O `type` pode ser qualquer lista não ordenada separada por vírgulas consistindo em:
>
>* `osgiPackageImports`
>* `overlays`
>* `acls`
>
>O valor de `type` o padrão é `osgiPackageImports` se não tiver passado.
...
* carregado em AEM (ou [carregado do sistema de arquivos](#uploading-packages-from-your-file-system) ou [baixado do compartilhamento de pacotes](#downloading-and-installing-packages-from-package-share))
...
>* Execute uma instalação de teste do pacote sem modificar qualquer conteúdo:\
   >  Abra o pacote (clique no ícone ou nome do pacote) e clique em **Instalar teste**.
>
...
1. No Gerenciador de pacotes, navegue até o pacote que deseja instalar.
   ...
   Um **Instalar** é exibido na lateral de Pacotes que ainda não foram instalados.

   >[!NOTE]
   >
   >Como alternativa, abra o pacote clicando no ícone para acessar o **Instalar** botão ali.
...
1. Clique em **Instalar** para iniciar a instalação. Uma caixa de diálogo solicitará a confirmação e listará todas as alterações feitas. Quando terminar, clique em **Fechar** na caixa de diálogo.
   ...
   A palavra **Instalado** é exibido ao lado do pacote após a instalação.

### Upload e instalação baseados no sistema de arquivos {#file-system-based-upload-and-installation}

Há uma maneira alternativa de carregar e instalar pacotes na sua instância. Em seu sistema de arquivos, você tem um `crx-quicksart` ao lado de seu jar e `license.properties` arquivo. É necessário criar uma pasta chamada `install` under `crx-quickstart`. Você terá algo assim: `<aem_home>/crx-quickstart/install`
...
Se a instância estiver em execução, adicione um pacote à `install` A pasta iniciará diretamente o upload e a instalação na instância. Se a instância não estiver em execução, os pacotes inseridos no `install` será instalada na inicialização na ordem alfabética.

>[!NOTE]
>
>Você também pode fazer isso antes mesmo de iniciar a instância pela primeira vez. Para fazer isso, você precisa criar a variável `crx-quickstart` criar a pasta manualmente `install` coloque seus pacotes lá embaixo. Em seguida, ao iniciar a instância pela primeira vez, os pacotes serão instalados na ordem alfabética.
...
1. Clique em **Desinstalar** para remover o conteúdo deste pacote do repositório. Uma caixa de diálogo solicitará a confirmação e listará todas as alterações feitas. Quando terminar, clique em **Fechar** na caixa de diálogo.
...
>[!NOTE]
>
>Os arquivos/nós instalados do pacote são **not** excluído.

1. No **Ferramentas** expanda o **Pacotes** pasta para mostrar o pacote no painel direito.
...
>[!CAUTION]
>
>Se este pacote já tiver sido instalado, então a variável *instalado* o conteúdo **not** ser eliminado.
...
1. No **Mais** no menu suspenso da barra de ferramentas, selecione **Replicar**.
...
Foi substituído por [Distribuição de software.](#software-distribution)

## Distribuição de software {#software-distribution}

[Distribuição de software](https://downloads.experiencecloud.adobe.com) é a nova interface de usuário criada para simplificar a pesquisa e o download de AEM Pacotes.
...
>[!CAUTION]
>
>No momento, o gerenciador de pacotes AEM não pode ser usado com a Distribuição de software. Você pode baixar os pacotes no disco local.
