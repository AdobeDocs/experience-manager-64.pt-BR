...
A lista inclui um URL para cada instância de publicação que executará o diagnóstico para essa instância. O parâmetro url `syncUser` é anexado ao URL de diagnóstico com seu valor definido para o usuário *de sincronização* autorizado criado na [Etapa 2](/help/sites-administering/sync.md#createauthuser).
