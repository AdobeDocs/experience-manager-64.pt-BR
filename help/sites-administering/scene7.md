...
Para usar o Dynamic Media Classic, é necessário configurar a configuração de nuvem para que o Dynamic Media Classic e os ativos AEM possam interagir entre si. Este documento descreve como configurar o AEM e o Dynamic Media Classic.
...
>* Antes de configurar o Dynamic Media Classic para trabalhar com o AEM, consulte Práticas [](#best-practices-for-integrating-scene-with-aem) recomendadas para integração do Dynamic Media Classic com o AEM.
...
Os usuários do AEM têm uma opção entre duas soluções para trabalhar com mídia dinâmica: Integrando sua instância do AEM com o Dynamic Media Classic ou usando a solução Dynamic Media integrada ao AEM.

Use os seguintes critérios para determinar qual solução escolher:

* Se você for um cliente **existente** do Dynamic Media Classic cujos ativos de mídia avançada residem no Dynamic Media Classic para publicação e entrega, mas quiser integrar esses ativos à criação do Sites (WCM) e/ou aos ativos AEM para gerenciamento, use a integração [ponto a ponto do](#aem-scene-point-to-point-integration) AEM/Dynamic Media Classic descrita neste documento.

* Se você for um **novo** cliente AEM que tenha necessidades de fornecimento de mídia avançada, selecione a opção [Mídia](#aem-dynamic-media)dinâmica. Essa opção faz mais sentido se você não tiver uma conta S7 existente e muitos ativos armazenados nesse sistema.
...
* Faça upload para os ativos AEM e ative a publicação automática para o Dynamic Media Classic; você acessa o navegador de conteúdo do **Assets** para criação de página
...
Os componentes que você usa para o Dynamic Media são encontrados na área do componente **[!UICONTROL Dynamic Media]** no modo [](/help/sites-authoring/author-environment-tools.md#page-modes)Design. Eles incluem:
...
1. Siga as instruções específicas do caso de uso:
   ...
   <table> 
    ...
    <td> </td> 
    <td> </td> 
    <td><strong>Mídia dinâmica</strong></td> 
    <td> </td> 
    ...
    <td>Carregar ativos no AEM e usar o componente Mídia dinâmica do AEM para criar ativos nas páginas do Sites</td> 
    ...
    <td>Faça upload de ativos não relacionados ao produto para o AEM para gerenciamento e entrega. Faça upload de ativos de produto para o Dynamic Media Classic e use o navegador de conteúdo do Dynamic Media Classic no AEM e componente para criar páginas de detalhes do produto nos sites.</td> 
    <td><p>Ligado</p> <p>(Consulte a etapa 3)</p> </td> 
    <td><a href="/help/assets/adding-dynamic-media-assets-to-pages.md">Ligado</a></td> 
    ...
    <tr> 
    <td>Novidade em ativos e mídia dinâmica</td> 
    <td>Carregar ativos para os ativos AEM e usar o URL/código incorporado publicado do Dynamic Media</td> 
    <td><p>Ligado</p> <p>(Consulte a etapa 3)</p> </td> 
    <td>Desativado</td> 
    <td>Desativado</td> 
    <td>Desativado</td> 
    ...
    <td>Carregue ativos no DAM e publique automaticamente no Dynamic Media Classic para entrega. Use o navegador de conteúdo do AEM Dynamic Media Classic para pesquisar e criar ativos nas páginas do Sites.</td> 
    <td>Desativado</td> 
    <td>Desativado</td> 
    <td><a href="/help/assets/scene7.md#scene-content-browser">Ligado</a></td> 
    ...
    <td><p>Carregue ativos no AEM e use o Dynamic Media para gerar renderizações para download/compartilhamento. Publicar automaticamente ativos AEM no Dynamic Media Classic para entrega.</p> <p><strong></strong> Importante: O processamento duplicado presente e as execuções geradas no AEM não serão sincronizados com o Dynamic Media Classic</p> </td> 
    <td><p>Ligado</p> <p>(Consulte a etapa 3)</p> </td> 
    <td>Desativado</td> 
    <td>Desativado</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">Ligado</a></p> <p>(Consulte a etapa 4)</p> </td> 
...
1. (Opcional; consulte a tabela de caso de uso) - Se você optar por ativar o Carregamento automático dos ativos para o Dynamic Media Classic, será necessário adicionar o seguinte:
   ...
   1. (Opcional) Restrinja o carregamento de ativos do Dynamic Media Classic por tipo MIME em [https://&lt;servidor>:&lt;porta>/system/console/configMgr/com.day.cq.dam.scene7.Scene7AssetMimeTypeServiceImpl](http://localhost:4502/system/console/configMgr/com.day.cq.dam.scene7.impl.Scene7AssetMimeTypeServiceImpl). Os tipos MIME de ativos não incluídos nesta lista não serão carregados no servidor do Dynamic Media Classic.
   1. (Opcional) Configure o vídeo na configuração do Dynamic Media Classic. Você pode ativar a codificação de vídeo para o Dynamic Media ou para o Dynamic Media Classic simultaneamente. As renderizações dinâmicas são usadas para visualização e reprodução localmente na instância do AEM, enquanto as renderizações de vídeo do Dynamic Media Classic são geradas e armazenadas nos servidores do Dynamic Media Classic. Ao configurar serviços de codificação de vídeo para o Dynamic Media e o Dynamic Media Classic, aplique um perfil [de processamento de](/help/assets/video-profiles.md) vídeo à pasta de ativos do Dynamic Media Classic.
...
* Fazer upload manual para o Dynamic Media Classic selecionando um ativo e arrastando-o para um componente do Dynamic Media Classic em uma página do AEM não funciona.
* Embora os ativos sincronizados do AEM-Dynamic Media Classic sejam atualizados automaticamente para o Dynamic Media Classic quando o ativo é editado no Assets, uma ação de reversão não ativa um novo upload, portanto o Dynamic Media Classic não obteria a versão mais recente imediatamente após uma reversão. A solução alternativa é editar novamente depois que a reversão for concluída.
* Se precisar usar o Dynamic Media para um caso de uso e a integração do Dynamic Media Classic para outro caso de uso, para que os ativos do Dynamic Media não interajam com o sistema do Dynamic Media Classic, não aplique a configuração do Dynamic Media Classic à pasta Dynamic Media ou a configuração do Dynamic Media (perfil de processamento) a uma pasta do Dynamic Media Classic.
...
>[!CAUTION]
>
>A importação de ativos de uma conta existente de empresa do Dynamic Media Classic pode levar muito tempo para ser exibida no AEM. Certifique-se de designar uma pasta no Dynamic Media Classic que não tenha muitos ativos (por exemplo, a pasta raiz geralmente terá muitos ativos e poderá travar o sistema).
...
* Os tipos de ativos do Dynamic Media Classic que os ativos AEM ainda não suportam precisam ser adicionados a um site do AEM diretamente do Dynamic Media Classic, por meio do navegador de conteúdo do Dynamic Media Classic, por exemplo, modelos de imagem.
* Para tipos de ativos compatíveis com os ativos AEM e o Dynamic Media Classic, decidir como fazer upload deles depende do seguinte:
...
Você pode configurar o AEM para carregar ativos no Dynamic Media Classic. Os ativos de uma pasta de destino do CQ podem ser carregados (automática ou manualmente) do AEM para uma conta de empresa do Dynamic Media Classic.

>[!NOTE]
>
>A Adobe recomenda que você use apenas a pasta de destino designada para importar ativos do Dynamic Media Classic. Os ativos digitais que residem fora da pasta de destino só podem ser usados nos componentes do Dynamic Media Classic nas páginas em que a configuração do Dynamic Media Classic foi ativada. Além disso, eles são colocados em uma pasta ad hoc no Dynamic Media Classic. A pasta adhoc não é sincronizada com o AEM (mas os ativos podem ser descobertos no navegador de conteúdo do Dynamic Media Classic).

Para configurar o Dynamic Media Classic para integração com o AEM, é necessário concluir as seguintes etapas:

1. [Definir uma configuração](#creating-a-cloud-configuration-for-scene) de nuvem - Define o mapeamento entre uma pasta do Dynamic Media Classic e uma pasta Assets. É necessário concluir essa etapa mesmo se você quiser apenas sincronização unidirecional (AEM Assets to Dynamic Media Classic).
1. [Ative o **Adobe CQ s7dam Dam Listener **](#enabling-the-adobe-cq-scene-dam-listener)- Concluído no console do[!UICONTROL OSGi].
1. Se quiser que os ativos AEM sejam carregados automaticamente no Dynamic Media Classic, ative essa opção e adicione o Dynamic Media Classic ao fluxo de trabalho de ativos de atualização do DAM. Você também pode fazer upload manual de ativos.
...
1. [Mapear a configuração para a página no AEM](#enabling-scene-for-wcm) - Essa etapa é necessária para exibir todas as predefinições de vídeo criadas no Dynamic Media Classic. Também é necessário se você precisar executar um ativo de publicação fora da pasta de destino do CQ para o Dynamic Media Classic.

Esta seção aborda como executar todas essas etapas e lista limitações importantes.

### Como funciona a sincronização entre o Dynamic Media Classic e os ativos AEM {#how-synchronization-between-scene-and-aem-assets-works}

Ao configurar a sincronização dos ativos AEM e do Dynamic Media Classic, é importante entender o seguinte:

#### Fazer upload para o Dynamic Media Classic a partir dos ativos AEM {#uploading-to-scene-from-aem-assets}
...
Com a sincronização entre os ativos AEM e o Dynamic Media Classic, há atualmente as seguintes limitações/implicações de design:
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Uma pasta de sincronização designada (destino)</td> 
   <td>Você só pode ter uma pasta designada por empresa no AEM para uploads do Dynamic Media Classic. Você pode criar várias configurações se precisar ter acesso a mais de uma conta da empresa no Dynamic Media Classic.</td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td>Pasta ad-hoc</td> 
   <td>Os ativos que residem fora da pasta de destino que são carregados manualmente para o Dynamic Media Classic no WCM são colocados automaticamente em uma pasta ad-hoc separada no Dynamic Media Classic. Isso é configurado na configuração da nuvem no AEM.</td> 
  </tr> 
  <tr> 
   <td>Mídia mista</td> 
  ...
  <tr> 
   <td>PDFs</td> 
   <td>Os PDFs gerados a partir de eCatalogs no Dynamic Media Classic são importados para a pasta de destino do CQ.</td> 
  </tr> 
  <tr> 
   <td>Atualização da interface</td> 
   <td>Ao sincronizar entre o AEM e o Dynamic Media Classic, atualize a interface do usuário para exibir as alterações. </td> 
  </tr> 
  <tr> 
   <td>Miniaturas de vídeo</td> 
   <td>Se um vídeo for carregado nos ativos AEM para codificação por meio do Dynamic Media Classic, as miniaturas de vídeo e os vídeos codificados podem levar algum tempo para estarem disponíveis nos ativos AEM, dependendo do tempo de processamento do vídeo.</td> 
  </tr> 
  <tr> 
...
Uma configuração em nuvem define o mapeamento entre uma pasta do Dynamic Media Classic e uma pasta do AEM Assets. Ele precisa ser configurado para sincronizar os ativos AEM com o Dynamic Media Classic. Consulte Como a sincronização funciona para obter mais informações.

>[!CAUTION]
>
>A importação de ativos de uma conta existente de empresa do Dynamic Media Classic pode levar muito tempo para ser exibida no AEM. Certifique-se de designar uma pasta no Dynamic Media Classic que não tenha muitos ativos (por exemplo, a pasta raiz geralmente terá muitos ativos).
>
...
1. Toque no ícone AEM e navegue até **[!UICONTROL Implantação > Serviços]** em nuvem para acessar o Adobe Dynamic Media Classic.
...
1. Insira o nome da **[!UICONTROL empresa]** e o Caminho **** raiz (este é o nome do servidor publicado junto com qualquer caminho que você deseja especificar; se você não souber o nome do servidor publicado, no Dynamic Media Classic, vá para **[!UICONTROL Configuração > Configuração]** do aplicativo.)
   ...
   >[!CAUTION]
   >
   >Dependendo do tamanho da pasta Dynamic Media Classic, a importação de uma pasta raiz pode levar muito tempo. Além disso, os dados do Dynamic Media Classic podem exceder o armazenamento do AEM. Verifique se você está importando a pasta correta. Importar demasiados dados pode parar o sistema.
...
1. Altere o número conforme desejado no campo Tempo limite **[!UICONTROL do trabalho]** Ativo. Qualquer número não negativo é aceite com a unidade de medida em segundos. Por padrão, isso é definido como 2100.
...
### Carregamento automático de ativos AEM {#autouploading-from-aem-assets}

A partir do AEM 6.3.2, os ativos AEM agora são configurados para você para que todos os ativos digitais carregados no gerenciador de ativos digitais sejam automaticamente atualizados para o Dynamic Media Classic se os ativos estiverem em uma pasta de destino do CQ.

Quando um ativo é adicionado aos ativos AEM, é automaticamente carregado e publicado no Dynamic Media Classic.

>[!NOTE]
>
>O tamanho máximo de arquivo para carregamento automático dos ativos AEM para o Dynamic Media Classic é 500 MB.

Para configurar o carregamento automático dos ativos AEM:

1. Toque no ícone do AEM e navegue até **[!UICONTROL Implantação > Serviços]** em nuvem e, em seguida, sob o cabeçalho Mídia dinâmica, em Configurações disponíveis, toque em **[!UICONTROL dms7 (Mídia]** dinâmica)
...
1. Navegue até a página de boas-vindas do AEM e toque em **[!UICONTROL Fluxos de trabalho]**. Clique duas vezes no fluxo de trabalho Atualizar ativo **do** DAM para abri-lo.
1. No sidekick, navegue até os componentes do **[!UICONTROL Fluxo de trabalho]** e selecione **[!UICONTROL Dynamic Media Classic]**. Arraste o **[!UICONTROL Dynamic Media Classic]** para o fluxo de trabalho e toque em **[!UICONTROL Salvar]**. Os ativos adicionados aos ativos AEM na pasta de destino serão carregados automaticamente para o Dynamic Media Classic.
...
Talvez você não queira publicar ativos imediatamente no Dynamic Media Classic se quiser testá-los em um ambiente de preparo antes de entrar em funcionamento. Você pode usar o AEM com o ambiente Secure Test do Dynamic Media Classic para enviar ativos diretamente do Assets para o Dynamic Media Classic em um estado não publicado.
...
1. Entre em contato com o Atendimento ao cliente do Dynamic Media Classic (s7support@adobe.com) para ativar a visualização segura para sua conta do Dynamic Media Classic.
1. Siga as instruções para [configurar uma visualização segura para sua conta do Dynamic Media Classic.](https://help.adobe.com/en_US/scene7/using/WSd968ca97bf00cf72-5eeee3a113268dc80f5-8000.html)
...
* Se um ativo já estiver publicado no AEM e uma versão subsequente for criada, essa nova versão será publicada imediatamente ao vivo para produção. Publicar na ativação somente funciona com a publicação inicial de um ativo.

>[!NOTE]
>
>Se você quiser publicar ativos instantaneamente, a prática recomendada é manter **[!UICONTROL Ativar visualização]** segura definida como **[!UICONTROL Imediatamente]** e usar o recurso **[!UICONTROL Ativar upload]** automático.
...
>[!NOTE]
>
>Se um usuário publicar o ativo no AEM, ele acionará automaticamente o ativo S7 para o ativo de produção/ao vivo (o ativo não estará mais na visualização segura/não será publicado).

Para definir o estado dos ativos enviados para o Dynamic Media Classic como não publicados:

1. Toque no ícone AEM e navegue até **[!UICONTROL Implantação > Serviços]** em nuvem, toque em **[!UICONTROL Dynamic Media Classic]** e selecione a configuração no Dynamic Media Classic.
1. Tap the **[!UICONTROL Advanced]** tab. No menu suspenso **[!UICONTROL Ativar exibição]** segura, selecione **[!UICONTROL Na ativação]** de publicação do AEM para encaminhar ativos para o Dynamic Media Classic sem publicar. (Por padrão, esse valor é definido como **[!UICONTROL Imediatamente]**, onde os ativos do Dynamic Media Classic são publicados imediatamente.)
...
* Para ativar a lista suspensa de perfis de vídeo universais para a criação de página. Sem isso, o menu suspenso Predefinição **** de vídeo universal fica vazio e não pode ser definido.
...
1. Toque em Serviços **[!UICONTROL da]** Cloud e toque em **[!UICONTROL Adicionar configuração]** e selecione **[!UICONTROL Dynamic Media Classic]**.
...
1. Toque em Serviços da **[!UICONTROL Cloud > Adicionar serviços > Dynamic Media Classic]**.
1. Na lista suspensa **[!UICONTROL Adobe Dynamic Media Classic]** , selecione a configuração desejada e toque em **[!UICONTROL OK]**.
...
Você pode configurar a pasta para a qual os ativos são carregados no Dynamic Media Classic quando o ativo não está localizado na pasta de destino do CQ. Consulte Publicação de ativos fora da pasta de destino do CQ.
...
1. Especifique um parâmetro de trabalho de upload do Dynamic Media Classic no campo de valor **[!UICONTROL jobParam]** .
   ...
   Consulte a API [do Sistema de Produção de Imagem Clássica do](https://marketing.adobe.com/resources/help/en_US/s7/ips_api/) Adobe Dynamic Media para obter parâmetros adicionais de trabalho de upload que você pode usar.
   ...
   >
   >Certifique-se de que seu arquivo PSD tenha &quot;camadas&quot;. Se for apenas uma imagem ou uma imagem com máscara, ela só será processada como uma imagem porque não há camadas para processar.
...
* Verifique se o ativo que você está tentando carregar está na pasta de destino **[!UICONTROL do]** CQ (você especifica essa pasta na configuração da nuvem do Dynamic Media Classic).
...
* Verifique se você configurou a configuração em nuvem dessa página por meio das Propriedades **[!UICONTROL da]** página. As predefinições de vídeo estão disponíveis no componente de vídeo Dynamic Media Classic.
...
* Verifique se os ativos estão na pasta de destino do CQ. Somente os ativos que estão na pasta de destino do CQ são atualizados automaticamente (desde que você tenha configurado os ativos AEM para fazer upload automático dos ativos).
* Verifique se você configurou a configuração dos Serviços em nuvem para Ativar o carregamento automático e se atualizou e salvou o fluxo de trabalho do Ativo DAM para incluir o upload do Dynamic Media Classic.
* Ao fazer upload de uma imagem em uma subpasta da pasta de destino do Dynamic Media Classic, execute um dos procedimentos a seguir:

   * Certifique-se de que os nomes de todos os ativos, independentemente da localização, sejam exclusivos. Caso contrário, o ativo na pasta de destino principal será excluído e somente o ativo da subpasta permanecerá.
...
* Os ativos e pastas excluídos nos ativos AEM ainda são exibidos na pasta sincronizada no Dynamic Media Classic. Você deve excluí-los manualmente.

**Se o upload do vídeo falhar**

* Se o upload do seu vídeo falhar e você estiver usando o AEM para codificar o vídeo por meio da integração do Dynamic Media Classic, consulte [Adicionar tempo limite configurável ao fluxo de trabalho](#adding-configurable-timeout-to-scene-upload-workflow)de Upload do Dynamic Media Classic.
