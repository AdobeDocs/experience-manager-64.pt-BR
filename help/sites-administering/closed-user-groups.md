...
Etapas para criar um novo requisito de autenticação incluindo um caminho de logon. Observe que o requisito e a exclusão do caminho de logon só serão registrados com o Autenticador Apache Sling se o objeto `RequirementHandler` tiver sido configurado para a árvore que contém o nó de destino.
...
Etapas para remover um requisito de autenticação existente. O requisito só será cancelado do Apache Sling Authenticator se o `RequirementHandler` tiver sido configurado para a árvore que contém o nó de destino.
...
As principais diferenças do ponto de vista da autorização estão resumidas na lista seguinte:
