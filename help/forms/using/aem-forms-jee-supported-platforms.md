---
title: Plataformas compatíveis com AEM Forms em JEE
seo-title: Plataformas compatíveis com AEM Forms em JEE
description: Lista de componentes de infraestrutura necessários e suportados para instalar o AEM Forms no JEE
seo-description: Lista de componentes de infraestrutura necessários e suportados para instalar o AEM Forms no JEE
...
# Plataformas compatíveis com AEM Forms em JEE {#supported-platforms-for-aem-forms-on-jee}

## Plataformas compatíveis {#supported-platforms}

### Níveis de suporte {#support-levels}

O AEM Forms no servidor JEE pode ser configurado usando qualquer combinação de sistemas operacionais, servidores de aplicativos, bancos de dados, drivers de banco de dados, JDK, servidores LDAP e servidores de email suportados.

Este documento lista as plataformas de cliente e servidor suportadas para o AEM Forms no JEE. A Adobe oferece vários níveis de suporte, tanto para nossas configurações recomendadas quanto para outras configurações. O documento também lista outros softwares suportados e suas versões, exceções, definições de patches e políticas de suporte a patches de software de terceiros.

>[!NOTE]
>
>* Para obter uma lista completa das exceções às plataformas de servidor suportadas, consulte [Exceções às plataformas](#exceptions-to-supported-server-platforms)de servidor suportadas.
>* O AEM Forms em JEE suporta apenas versões em inglês, francês, alemão e japonês dos sistemas operacionais e aplicativos suportados.
>
...
A Adobe recomenda essas configurações e fornece suporte completo ou restrito como parte do contrato padrão de manutenção de software:
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>A Adobe oferece suporte e manutenção completos para essa configuração. Essa configuração é coberta pelo processo de garantia de qualidade da Adobe.</td> 
  </tr> 
  <tr> 
   <td>R: Suporte restrito</td> 
   <td>A Adobe fornece suporte total para essa configuração depois que determinados pré-requisitos forem atendidos. Entre em contato com o suporte empresarial da Adobe para saber mais sobre os pré-requisitos e solicitar o suporte.</td> 
  </tr> 
  <tr> 
   <td>L: Apoio limitado</td> 
   <td>A Adobe fornece suporte e manutenção completos para essas configurações após a satisfação de determinados pré-requisitos. Nem todos os recursos estão disponíveis na configuração. Entre em contato com o suporte empresarial da Adobe para saber mais sobre os pré-requisitos e solicitar o suporte.<br /> </td> 
  </tr> 
 </tbody> 
</table>
...
| Z:Não suportado | A configuração não é suportada. A Adobe não faz declarações sobre se a configuração funciona e não oferece suporte para ela. |

### Java Virtual Machines (JVM) {#java-virtual-machines-jvm}

O Adobe Experience Manager Forms requer uma máquina virtual Java para execução, fornecida pela distribuição do Java Development Kit (JDK). O Adobe Experience Manager opera com as seguintes versões das máquinas virtuais Java:
...
>* É recomendável rastrear os Boletins de segurança do fornecedor Java para garantir a segurança dos ambientes de produção e instalar as Atualizações Java mais recentes.
>
...
#### Suporte a persistência do AEM {#aem-persistence-support}
...
* O MongoDB é um software de terceiros e não está incluído no pacote de licenciamento do AEM. Para obter mais informações, consulte a página de política [de licenciamento do](https://www.mongodb.org/about/licensing/) MongoDB.

* Para obter o máximo de sua implantação do AEM, a Adobe recomenda licenciar a versão MongoDB Enterprise para se beneficiar do suporte profissional.
* O Atendimento ao cliente da Adobe ajudará a solucionar problemas relacionados ao uso do MongoDB com o AEM. Para obter mais informações, consulte a página [](https://www.mongodb.com/lp/contact/mongodb-adobe-experience-manager)MongoDB para Adobe Experience Manager.
* &#39;Sistema de arquivos&#39; inclui armazenamento em bloco compatível com POSIX. Isso inclui a tecnologia de armazenamento em rede. Observe que o desempenho do sistema de arquivos pode variar e influencia o desempenho geral. É recomendável carregar o AEM de teste em combinação com o sistema de arquivos remoto/de rede.
* Somente o MongoDB Storage Engine WiredTiger é compatível.
* O compartilhamento MongoDB não é compatível com o AEM.
* O AEM Forms em JEE não oferece suporte a persistência MySQL para RDBMK.
* O módulo do Document Security não usa o Repositório de conteúdo. Isso significa que, se você estiver usando apenas o Document Security e não planeja usar o HTML Workspace, formulários HTML5 ou formulários adaptáveis, não instale o Repositório de conteúdo.
* O AEM Forms no JEE oferece suporte à arquitetura Oracle Multilocatário.
...
<table> 
 <tbody> 
  <tr> 
   ...
   <td><p><strong>Nível de suporte do AEM Forms 6.4 em JEE</strong></p> </td> 
  </tr> 
  <tr> 
   <td>IBM DB2 11.1</td> 
   <td>Microkernel do repositório</td> 
...
Considere as seguintes exceções ao escolher uma plataforma para configurar seus formulários AEM no servidor JEE.

1. O AEM Forms em JEE não é compatível com Oracle WebLogic e JBoss® no IBM® AIX®.
1. O AEM Forms em JEE não é compatível com Oracle WebLogic e IBM® WebSphere® com MySQL.
1. O AEM Forms no JEE não é compatível com o Oracle Solaris™ com arquitetura Intel® (somente SPARC® é suportado).
1. O AEM Forms no JEE não é compatível com Oracle WebLogic e JBoss no SUSE Linux Enterprise Server 12. Somente o IBM WebSphere é suportado no SUSE Linux Enterprise Server 12.
...
1. O AEM Forms em JEE não é compatível com IBM® DB2 com JBoss®.
1. O repositório CRX suporta persistência do tipo TarMK, MongoDB e bancos de dados relacionais (RDBMK). Não é possível ter dois sistemas de banco de dados diferentes entre o servidor de aplicativos e o repositório CRX. No entanto, em um ambiente AEM Forms em JEE, você pode usar MongoMK com repositório CRX e um banco de dados relacional compatível com o servidor de aplicativos.
1. O AEM Forms no JEE não oferece suporte ao servidor de aplicativos WebSphere no CentOS.
...
* O AEM Forms no JEE é compatível com vários softwares de terceiros de acordo com nossa Política [de suporte a software de](#third-party-patch-support-policy)terceiros.
...
* O AEM Forms no JEE não oferece suporte para agrupamento vertical.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>A versão 8.5<strong> </strong>é compatível</p> </td> 
  </tr> 
  <tr> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><a href="https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html">Faixa clássica do Acrobat 2017</a></td> 
...
* Interface adaptativa de criação de formulários
* Interface do usuário de criação do Gerenciador de Formulários
* Interface do usuário de criação do Gerenciamento de correspondência
* IU do administrador (IU do Console de administração)

## Requisitos de sistema para o AEM Forms no JEE {#system-requirements-for-aem-forms-on-jee}
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Processador Intel® Xeon® E5-2680, 2,4 GHz ou VMWare ESX 5.1 ou posterior<br /><br /> RAM: 6 GB (SO de 64 bits com JVM de 64 bits)<br /> Espaço em disco livre: 15 GB de espaço temporário mais 22 GB<br /> para o AEM Forms no JEE</td> 
  </tr> 
  <tr> 
   <td>Sun Solaris</td> 
   <td>Processador<br /> UltraSPARC® IIIi, 1,5 GHz, Solaris Containers (Zones) particionando<br /> RAM: 6 GB (SO de 64 bits com JVM de 64 bits)<br /> Espaço em disco livre: 6 GB de espaço temporário mais 22 GB<br /> para o AEM Forms no JEE</td> 
  </tr> 
  <tr> 
   <td>IBM AIX</td> 
   <td>P6 pSeries 520 (Modelo 52A) 9131-52A, 1,8 GHz processador<br /> LPAR particionando<br /> RAM: 6 GB (SO de 64 bits com JVM de 64 bits)<br /> Espaço em disco livre: 6 GB de espaço temporário mais 22 GB<br /> para o AEM Forms no JEE</td> 
  </tr> 
  <tr> 
   <td>SUSE Linux Enterprise Server</td> 
   <td>Intel Xeon E5-2670v2, 1 vCPU, processador<br /> de 2,5 GHz AWS m3.medium (3 ECUs)<br /> RAM: 6 GB (SO de 64 bits com JVM de 64 bits)<br /> Espaço em disco livre: 6 GB de espaço temporário mais 22 GB<br /> para o AEM Forms no JEE</td> 
  </tr> 
  <tr> 
   <td>Red Hat Enterprise Linux</td> 
   <td>Intel Xeon E5-2670v2, 1 vCPU, processador<br /> de 2,5 GHz AWS m3.medium (3 ECUs)<br /> RAM: 6 GB (SO de 64 bits com JVM de 64 bits)<br /> Espaço em disco livre: 6 GB de espaço temporário mais 22 GB<br /> para o AEM Forms no JEE<br /> </td> 
  </tr> 
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li><strong></strong> Ambiente alimentado pela Sun SPARC: UltraSPARC V ou posterior</li> 
     <li><strong></strong> Ambiente alimentado pelo IBM AIX: Power6 ou posterior<br /> </li> 
...
* [Requisitos de sistema para um AEM Forms em cluster na implantação do JEE](https://www.adobe.com/go/learn_aemforms_sysreq_cluster_64)

## Clientes suportados para AEM Forms em JEE {#supported-clients-for-aem-forms-on-jee}
...
>[!NOTE]
>
>A família de produtos Acrobat DC apresenta duas faixas para o Acrobat e o Reader, que são essencialmente produtos diferentes: &quot;Clássico&quot; e &quot;Contínuo&quot;. Para obter detalhes e uma comparação das duas faixas, consulte [https://www.adobe.com/go/acrobatdctracks.](https://www.adobe.com/go/acrobatdctracks)
...
>[!NOTE]
>
>* O Portal de formulários é compatível somente com o Safari no iPad.
>
...
| Google Android | Android 4.4 (Androird Kit Kat) e acima do nível 19 *[da API e superior]*. O aplicativo AEM Forms é certificado em tablets Samsung Galaxy de 7 e 10 polegadas e tablet Google Nexus de 7 polegadas e smartphones populares. |
...
>[!NOTE]
>
>A Adobe irá [parar de atualizar e distribuir o Flash Player no final de 2020](https://theblog.adobe.com/adobe-flash-update/).

### Adobe Document Security Extension for Microsoft Office {#adobe-rights-management-extension-for-microsoft-office}

Clique [aqui](https://www.adobe.com/products/livecycle/rightsmanagement/extension/downloads.html) para ver os requisitos de sistema do Adobe Document Security Extension for Microsoft® Office.
...
Além disso, o AEM Forms no JEE oferece suporte a atualizações, patches e pacotes de correção sobre a versão principal e secundária especificada do software suportado. No entanto, a atualização para a próxima versão principal ou secundária não é suportada, a menos que especificado.
...
O AEM Forms em plataformas de referência de terceiros do JEE indica o nível de patch específico da infraestrutura de terceiros que estava atualizado durante o desenvolvimento e o lançamento do AEM Forms no JEE, e a partir do nível mínimo de patch/service pack da infraestrutura suportada por essa versão do AEM Forms no JEE.

A Adobe oferece suporte a patches urgentes ou recomendados emitidos por fornecedores terceirizados após o lançamento, desde que fornecedores terceirizados garantam compatibilidade retroativa com as versões compatíveis com o AEM Forms no JEE. A Adobe só oferecerá suporte a patches lançados após o nível mínimo de patch indicado na documentação do AEM Forms em JEE.

Em alguns casos, a Adobe não oferece suporte a atualizações de terceiros que alterem a funcionalidade principal e, portanto, não oferecem suporte à compatibilidade total com versões anteriores. Para obter detalhes sobre as atualizações compatíveis, consulte Definições [de patch](https://helpx.adobe.com/aem-forms/aem-forms-third-party-software-patch.html) suportadas para produtos de fornecedor específicos e os tipos de patch suportados pela Adobe.

Sob circunstâncias fora do controle da Adobe, patches de terceiros que exigem compatibilidade com versões anteriores podem ter impacto negativo nos produtos da Adobe ou nos ambientes do cliente. Nesses casos, a Adobe recomenda que os clientes avaliem o impacto de qualquer patch urgente de terceiros antes de aplicá-lo a sistemas críticos. A Adobe trabalhará com terceiros usando esforços comerciais razoáveis para resolver esses problemas, seja por meio de programas normais de suporte da Adobe ou por terceiros que corrigem o problema em seus patches. Isso não garante que um patch de terceiros recém-lançado que será suportado pela Adobe funcionará conforme documentado pelo fornecedor ou com o AEM Forms no JEE.

A Adobe reserva o direito de alterar as plataformas de referência de terceiros suportadas por um AEM Forms em uma versão JEE e suas definições de correção compatíveis em qualquer ponto determinado.
