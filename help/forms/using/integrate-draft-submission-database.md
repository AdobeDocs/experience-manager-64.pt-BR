...
O componente de rascunhos e envios do portal do AEM Forms permite que os usuários salvem seus formulários como rascunhos e enviem posteriormente de qualquer dispositivo. Além disso, os usuários podem exibir seus formulários enviados no portal. Para ativar essa funcionalidade, o AEM Forms fornece serviços de dados e metadados para armazenar os dados preenchidos por um usuário no formulário e os metadados associados aos rascunhos e formulários enviados. Esses dados são armazenados no repositório CRX, por padrão. No entanto, à medida que os usuários interagem com formulários por meio da instância de publicação do AEM, que geralmente está fora do firewall da empresa, as organizações podem desejar personalizar o armazenamento de dados para que ele seja mais seguro e confiável.

A amostra, discutida neste documento, é uma implementação de referência de dados personalizados e serviços de metadados para integrar os rascunhos e componentes de submissão a um banco de dados. O banco de dados usado na implementação de amostra é o **MySQL 5.6.24**. No entanto, você pode integrar o componente de rascunhos e envios a qualquer banco de dados de sua escolha.

>[!NOTE]
>
>* Os exemplos e as configurações explicados neste documento estão de acordo com o MySQL 5.6.24 e você deve substituí-los adequadamente para o seu sistema de banco de dados.
>* Verifique se você instalou a versão mais recente do pacote complementar AEM Forms. Para obter a lista de pacotes disponíveis, consulte o artigo de versões [do](https://helpx.adobe.com/aem-forms/kb/aem-forms-releases.html) AEM Forms.
>
...
1. Vá para o gerenciador de pacote AEM em https://[*host*]:[*port*]/crx/packmgr/.
...
1. Especifique os valores para as propriedades conforme descrito na tabela a seguir:
   ...
   |---|---|---|
   | Serviço de Dados de Rascunho do Portal de Formulários | Identificador do serviço de dados preliminar | formsportal.sampledataservice |
   ...
   | Serviço de dados de envio do Portal de formulários | Identificador para enviar serviço de dados | formsportal.sampledataservice |
   | Serviço de Metadados de Envio do Portal de Formulários | Identificador do serviço de metadados de envio | formsportal.samples emetadataservice |
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>jdbc:mysql://[<em>host</em>]:[<em>porta</em>]/[nome<em>_</em>esquema]</td> 
  </tr> 
  <tr> 
...
1. Se você já tiver as tabelas (dados, metadados e metadados adicionais) no esquema do banco de dados, execute as seguintes consultas de tabela futura:
   ...
   **Instrução SQL para alterar a tabela de metadados**
   ...
   >[!NOTE]
   >
   >A consulta de adição de metadados ALTER TABLE falhará se você já a executou e a coluna markedforDelete estiver presente na tabela.
...
A implementação de amostra agora está configurada, que você pode usar para listar seus rascunhos e envios enquanto armazena todos os dados e metadados em um banco de dados. Agora vamos ver como os dados e os serviços de metadados são configurados na amostra.
...
1. Clique em **[!UICONTROL Escolher arquivo]** e navegue para selecionar o arquivo mysql-Connector-java-5.1.39-bin.jar. Além disso, marque as caixas de seleção **[!UICONTROL Iniciar pacote]** e **[!UICONTROL Atualizar pacotes]** .
...
A implementação do banco de dados do Portal do Forms usa tabela de metadados adicional. A tabela tem uma chave primária composta com base nas colunas Chave e ID da tabela. O MySQL permite chaves primárias até o comprimento de 255 caracteres. Você pode usar o seguinte script de validação do lado do cliente para verificar a duração do nome de arquivo anexado ao widget de arquivo. A validação é executada quando um arquivo é anexado. O script fornecido no procedimento a seguir exibe uma mensagem, quando o nome do arquivo for maior que 150 (incluindo extensão). É possível modificar o script para verificar se há um número diferente de caracteres.
...
1. Adicione o seguinte código ao arquivo util.js e clique em **[!UICONTROL Salvar tudo]**. O comprimento de validação do código do nome do arquivo.
   ...
   >[!NOTE]
   >
   >O script é para o componente de widget anexo OTB (out-out). Se você personalizou o widget anexo OTB, altere o script acima para incorporar as respectivas alterações.
