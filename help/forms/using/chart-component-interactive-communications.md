...
# Uso de gráficos em Interative Communications {#using-charts-in-interactive-communications}
...
1. Toque no componente de gráfico no editor de Comunicações interativas e, na barra de ferramentas Componente, selecione **[!UICONTROL Configurar (]** ![configure_icon](assets/configure_icon.png)).
...
1. Configure as propriedades Básicas do gráfico para canal impresso e canal da Web. Além das propriedades comuns, há propriedades específicas para impressão e canal da Web e o tipo de gráfico.
   ...
   * Especifique o seguinte para **[!UICONTROL eixo x]** e **[!UICONTROL eixo y]**:
      ...
      * **[!UICONTROL Função]**: Para usar funções estatísticas para calcular os valores no eixo, selecione função para o eixo X / Y. Para obter mais informações sobre funções, consulte [Usar funções no gráfico](#usefunction) e [Exemplo 2: Aplicação de funções de soma e média em um gráfico de linha](#applicationsumfrequency).
...
1. (Somente canal de impressão) Nas Configurações do agente, especifique se é obrigatório que o agente use este gráfico. Se a opção i **[!UICONTROL t for obrigatória para o agente usar este gráfico]** não estiver selecionada, o agente poderá tocar no ícone de olho do gráfico na guia Conteúdo da interface do usuário do agente para mostrar/ocultar o gráfico.
...
* **[!UICONTROL Dica de]** ferramenta para exibir ao mouse sobre uma despesa (somente canal da Web) - neste exemplo  `${x}: $ ${y}`, que é exibido como  `[Expense Label: $ Amount]` (Exemplo: Visita ao parque de temas: US$ 315)

![Gráfico de colunas na saída de impressão de um ](assets/chartprintchannel.png)
**Communication interativoFigura:Gráfico de** *colunas na saída de impressão de uma Comunicação interativa*

**Eixo A.** Y - Quantia obtida da propriedade de modelo de dados de formulário e da propriedade Title definida como Quantia ($)  **B.Direção** do rótulo do eixo X definida como Inclinar Eixo  **C.** X Esquerdo - Descrição da despesa obtida da propriedade de modelo de dados de formulário e propriedade Título definida como Despesa
...
**A.** A propriedade Raio interno da rosca está definida como  **B.** Mostrar propriedade da legenda está selecionada e a propriedade Posição da legenda está definida como  **C. A** dica de ferramenta exibe o detalhe do item no mouse sobre - A dica de ferramenta está definida como ${x}: $ ${y}
...
**transações de &quot;Cama e Café da manhã&quot;Figura:Gráfico de** *linhas sem uma função com três transações de &quot;Cama e Café da manhã&quot;*
...
**** NomeUm identificador para o elemento de gráfico. O nome não está visível no gráfico, mas ajuda ao se referir ao elemento de outros componentes, scripts e expressões SOM.

**Título (Somente canal de impressão)** Especifica o título do gráfico.

**Tipo de** gráficoEspecifica o tipo de gráfico que você deseja gerar. As opções disponíveis são Pizza, Coluna, Rosca, Barra (somente canal da Web), Linha, Linha e Ponto, Ponto e Área. Para obter mais informações, consulte Exemplo 1: Saída de gráfico na impressão e na Web.

**Eixo X >** TítuloEspecifica o título do eixo x.

**Eixo X > Objeto do modelo de dados &amp;passado;** Especifique o nome do item de coleta do modelo de dados do formulário a ser representado no eixo x.
...
**Eixo Y >** TítuloEspecifica o título do eixo y.

**Eixo Y > Objeto do modelo de dados &amp;passado;** Especifica o item de coleta do modelo de dados do formulário a ser representado no eixo y. No canal Print, o objeto de modelo de dados para o eixo Y deve ser do tipo Number.
...
Para desativar a dica de ferramenta, deixe o campo Dica de ferramenta em branco. Essa opção não se aplica a gráficos de linha e de área. Por exemplo, consulte [Exemplo 1: Saída do gráfico em print e web](#chartoutputprintweb).
...
**Configurações específicas do gráficoAlém das configurações comuns,** as seguintes configurações específicas do gráfico estão disponíveis:
