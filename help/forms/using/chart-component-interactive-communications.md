...
Um gráfico ou um gráfico é uma representação visual dos dados. Ele condensa grandes quantidades de informações em um formato visual fácil de entender, permitindo que os destinatários da Comunicação interativa visualizem, interpretem e analisem melhor dados complexos.
...
1. Na barra lateral Componentes, arraste e solte o componente Gráfico em um dos seguintes canais de impressão ou da Web de uma Comunicação interativa:
...
1. Toque no componente gráfico no editor de Comunicação interativa e, na barra de ferramentas Componente, selecione **[!UICONTROL Configurar (]** configure_icon ![](assets/configure_icon.png)).
...
1. Configure as propriedades Básicas do gráfico para canal de impressão e canal da Web. Além das propriedades comuns, há propriedades específicas para impressão, canal da Web e tipo de gráfico.
   ...
   * Especifique o seguinte para os eixos **** x e **[!UICONTROL y]**:
   ...
   >[!NOTE]
   >
   >Para canal de impressão, no eixo X, o objeto de modelo de dados vinculado deve ser do tipo Número, String ou Data. No eixo Y, o objeto de modelo de dados vinculado deve ser do tipo Number. É recomendável usar a legenda do lado direito no canal de impressão.
...
1. Na barra lateral Propriedades, toque em ![done_icon](assets/done_icon.png).
   ...
   Visualizar para ver a aparência e os dados do gráfico. Retorne para reconfigurar as propriedades do gráfico, se necessário.
...
* **[!UICONTROL Título]** para os eixos X e Y (para o gráfico de tipo Coluna no canal de impressão somente neste exemplo) - neste exemplo, Quantia ($) para o eixo X e Despesa para o eixo Y.
...
![](assets/chartprintchannel.png) Gráfico de colunas na saída de impressão de uma **** Figura de comunicação interativa: Gráfico de *colunas na saída de impressão de uma comunicação interativa*
...
![](assets/chartwebchannel.png) Faça o donut do gráfico na saída da Web de uma **** Figura de comunicação interativa: Gráfico de *rosca na saída da Web de uma Comunicação Interativa*
...
![](assets/creditcarddatalinechartcopy.png) Gráfico de linhas sem uma função com três transações **&quot;Cama e Café da manhã&quot;** Figura: Gráfico de *linhas sem uma função com três transações de &quot;Cama e Café da manhã&quot;*
...
**** Eixo Y > Objeto do modelo de dados &amp;ast; Especifica o item de coleta do modelo de dados de formulário a ser representado no eixo y. No canal Impressão, o objeto de modelo de dados para o eixo Y deve ser do tipo Número.
...
Você pode configurar um gráfico para usar funções estatísticas para calcular valores dos dados de origem para plotar no gráfico. Ao aplicar funções em um gráfico, é possível plotar dados que não são fornecidos diretamente pelo modelo de dados do formulário.
...
1. Para exibir a função personalizada no menu suspenso Função, no CRXDe Lite, crie um `nt:unstructured` nó na pasta de aplicativos com as seguintes propriedades:
   ...
   * Adicione uma propriedade `qtip` com um valor que será uma breve descrição da função personalizada. Ela é exibida como uma dica de ferramenta ao passar o ponteiro sobre o nome da função na lista suspensa **Função** .
