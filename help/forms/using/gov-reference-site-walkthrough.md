...
seo-description: 'Consulte a apresentação do site de referência We.Gov para entender como o AEM Forms ajuda os governos a gerenciar informações individuais. '
...
* Dados da conta bancária do requerente em que este pode receber prestações de apoio a crianças
...
Depois que o aplicativo é aprovado, o requerente recebe um email do site We.Gov. A opção **Exibir documento** no email vincula um documento com os detalhes de inscrição do candidato.
...
* Sarah Rose, a mãe adotiva que pede apoio infantil
...
Um candidato pode verificar a qualificação para solicitar benefícios de suporte-filho. O site permite que os usuários respondam perguntas para permitir que determinem se seu aplicativo é elegível para benefícios. Sarah, uma mãe adotiva, é uma candidata potencial para isso. O formulário de elegibilidade faz parte dos serviços de Apoio à Criança do site We.Gov. Para verificar sua qualificação, Sarah clica em Suporte **** infantil no site We.Gov. Na página Suporte à criança, Sarah clica em **[!UICONTROL Verificar sua qualificação]**.
...
No seu navegador, abra `https://<hostname>:<PublishPort>/content/we-gov/en.html`. No site We.Gov, clique em Child Support. Na página Suporte à criança, clique em Verificar sua qualificação.
...
## Sarah inicia seu aplicativo para apoio infantil {#sarah-starts-her-application-for-child-support}

Sarah clica em **[!UICONTROL Iniciar aplicativo]** depois de ser informada sobre sua qualificação para suporte a crianças.\
...
A qualquer momento, Sarah pode clicar em **[!UICONTROL Voltar mais tarde]** e retomar o aplicativo. Quando ela clica em **[!UICONTROL Voltar mais tarde]**, seu progresso é salvo como um rascunho, e ela recebe uma opção para enviar o rascunho por email.

Ao clicar em **[!UICONTROL Enviar e-mail]**, ela recebe um e-mail com um link para o rascunho de seu formulário.
...
Depois que Sarah clicar em **[!UICONTROL Iniciar Aplicação]**, ela será direcionada para a seção Informações do Candidato da página Serviços de Suporte à Criança. Em Informações do Candidato, Sarah navega pelas guias e fornece informações pessoais para o aplicativo. Ela clica em **[!UICONTROL Próximo]** para navegar pelas guias.

Em Informação do Requerente, solicita-se à relatora que forneça informações detalhadas sob as seguintes guias:

* **[!UICONTROL Informações básicas]**

Em Basic Information, Sarah fornece sua prova de identidade e suas informações pessoais. As informações pessoais de Sarah incluem o nome, a ID de e-mail e o número da segurança social.
...
* **[!UICONTROL Informações adicionais]**
   ...
   Em Additional Information (Informações adicionais), Sarah insere um número de identificação, sua data de nascimento e endereço atual e número de telefone.

### Sarah fornece informações sobre filhos {#sarah-provides-child-information}

Depois que Sarah fornecer suas informações pessoais e clicar em **[!UICONTROL Avançar]**, ela será direcionada para a seção Informações da Criança.
...
Seu pedido é enviado ao site We.Gov.

### <strong>Sarah recebe um email</strong> de reconhecimento {#strong-sarah-receives-an-acknowledgement-email-strong}

Depois que Sarah paga a taxa de inscrição, ela recebe um email de reconhecimento do site We.Gov.\
We.Gov processa o pedido, e Sarah é informada que receberá uma compensação mensal depois que seu pedido for aprovado.

![sarah-ack-email](assets/sarah-ack-email.png)

### Como funciona {#how-it-works-1}

O aplicativo de suporte secundário usa uma combinação de layouts de painel, como a guia superior, o assistente e a tabela para criar a experiência. Ele usa um modelo de formulário chamado Modelo Filho We.Gov.
...
Em seu navegador, abra `https://<hostname>:<PublishPort>/content/forms/af/we-gov/child-support/css.html` e preencha as informações necessárias. Ao enviar a solicitação depois de preencher as informações necessárias, pagar as taxas e assinar o documento, você receberá o email de confirmação.
...
Gloria, chefe de aprovações em We.Gov, pode exibir, aprovar ou rejeitar solicitações enviadas. A Caixa de entrada do AEM permite que ela veja todos os aplicativos enviados em um único lugar.

### Como funciona {#how-it-works-2}

Quando Sarah preenche e envia o aplicativo de suporte filho, um PDF ou Documento de registro do aplicativo é criado e enviado para a caixa de entrada de Gloria Rios. Gloria pode ver o pedido apresentado e aceitá-lo ou rejeitá-lo.

### Veja você mesmo {#see-it-yourself-2}

Abrir página `https://<hostname***>:<PublishPort>/content/we-gov/en.html`. Na página, toque em **[!UICONTROL Fazer logon]**, marque a caixa de seleção **[!UICONTROL Fazer logon como representante]** , faça logon na caixa de entrada do AEM usando grios/senha como nome de usuário/senha para Gloria Rios. O aplicativo de suporte secundário é exibido. Para obter informações sobre como usar a Caixa de entrada do AEM para tarefas de fluxo de trabalho centradas em formulários, consulte [Gerenciar aplicativos e tarefas de formulários na Caixa de entrada](/help/forms/using/manage-applications-inbox.md)do AEM.
...
Gloria, chefe de aprovação na We.Gov, abre sua caixa de entrada AEM. Ela vê uma tarefa de revisão em sua lista de tarefas. Ela abre e exibe a tarefa de revisão.
...
O aplicativo de Sarah é um ponto de partida no fluxo de trabalho do AEM. Ele inicia o fluxo de trabalho do AEM quando o formulário de aplicativo de suporte secundário é enviado. O fluxo de trabalho do AEM cria uma tarefa para Gloria, que aparece na caixa de entrada do AEM. Quando Gloria solicita avaliação no local, uma nova tarefa é criada para o agente de campo.

### Veja você mesmo {#see-it-yourself-3}

Se a configuração for concluída, o fluxo de trabalho do AEM será iniciado imediatamente após o envio do formulário. Faça logon na caixa de entrada usando as credenciais de Gloria.
...
Na caixa de entrada do AEM, o aplicativo da Sarah é adicionado como uma tarefa de revisão. Selecione a tarefa e clique em **Avaliação necessária** para prosseguir para a próxima etapa.

### A placa recebe a tarefa Avaliação {#conard-assessment-task}

Quando Gloria clica em **[!UICONTROL Avaliação obrigatória]**, o Conard recebe a tarefa de revisão em sua Caixa de entrada do AEM. A tarefa é a próxima etapa no fluxo de trabalho do AEM definido no modelo de fluxo de trabalho. Ele vê a tarefa de revisão e a abre.

A Conard recebe a tarefa de avaliação do candidato, conforme mostrado abaixo.

![caixa de entrada](assets/conrad-inbox.png)

A avaliação de suporte secundário é um formulário associado à tarefa. Ele recebe os detalhes de Sarah, juntamente com os documentos de apoio (anexados em detalhes da tarefa). O cartão preenche o formulário de avaliação no campo em um dispositivo e envia para reavaliação.

O Conard verifica todos os detalhes que a Sarah forneceu e a Sarah assina a avaliação. O AEM Forms pode usar o local e o carimbo de data e hora e adicioná-los à assinatura.

![submeter a nova avaliação](assets/submit-for-re-evaluation.png)

A placa clica em **[!UICONTROL Enviar para reavaliação]** e o fluxo de trabalho do AEM envia a avaliação para a organização We.Gov.

### Como funciona {#how-it-works-4}

Quando Gloria solicita uma avaliação, a próxima etapa do fluxo de trabalho do AEM é iniciada e a tarefa de avaliação é adicionada na caixa de entrada do Conard. O Conard é o trabalhador de campo.
...
Abra a caixa de entrada do AEM em seu tablet e use as credenciais do Conard para fazer logon.
...
Você pode ver uma nova tarefa de Solicitação de Avaliação adicionada na caixa de entrada. Submeter a avaliação concluída e prosseguir para a próxima etapa.
...
Em Detalhes da Tarefa, Gloria vê Última Ação Tomada como &quot;Enviar para Reavaliação&quot; (pelo Conard). Gloria vê que a Conard Simms avaliou o pedido.
...
Abra a caixa de entrada e faça logon usando as credenciais de Gloria. Uma nova tarefa chamada Revisão é exibida na caixa de entrada.
...
O botão **[!UICONTROL Visualizar documento]** no e-mail vincula os detalhes de inscrição. Sarah clica em **[!UICONTROL Exibir documento.]**

![Approval-enrolment-kit-email](assets/approval-enrolment-kit-email.png)

O documento de inscrição lista detalhes como a ID de referência, a cobertura do filho, a data de início, o número da conta bancária, a frequência de pagamento e a quantia de pagamento.

![detalhes da inscrição em sarah](assets/sarah-enrollment-details.png)

Sarah pode exibir os documentos que carregou na mesma página.

![documentos carregados](assets/uploaded-documents.png)

### Como funciona {#how-it-works-6}

Quando Gloria aprova o aplicativo, Sarah recebe um email automático com um link para o documento de inscrição.

O documento de inscrição é uma comunicação interativa e pode ser exibido em qualquer dispositivo. Ele contém detalhes do serviço de suporte infantil e informações fornecidas pela Sarah.
...
Para que você possa visualizar e explorar o relatório de análise, estamos fornecendo dados semente para o aplicativo de serviços de suporte para crianças no site de referência. Antes de usar dados semente, consulte [Configurar o Analytics](/help/forms/using/setup-reference-sites.md#configureanalytics). Execute as seguintes etapas na instância do autor para exibir o relatório com os dados semente:
...
1. No navegador de endereços da lista CRXDE, digite: **/apps/we-gov/demo-artifacts/analyticsTestData/Child support service Dados de teste do Analytics**
...
1. No CRXDE, navegue até: **/content/dam/formsanddocuments/we-gov/child-support/css/jcr:content/analyticsdatanode/last-sete dias**
