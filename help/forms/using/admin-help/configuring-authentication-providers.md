...
# Configurar provedores de autenticação {#configuring-authentication-providers}
...
Se você ativar o SSO usando o SPNEGO, adicione um provedor de autenticação Kerberos com o SPNEGO ativado e um provedor LDAP como backup. Essa configuração permite a autenticação do usuário com uma ID de usuário e senha se o SPNEGO não estiver funcionando. (Consulte [Ativar SSO usando o SPNEGO](/help/forms/using/admin-help/enabling-single-sign-on-aem.md#enable-sso-using-spnego).)
...
1. Clique em um domínio existente na lista. Se você estiver adicionando autenticação para um novo domínio, consulte [Adicionar um domínio](/help/forms/using/admin-help/adding-domains.md#add-an-enterprise-domain) corporativo ou [Adicionar um domínio](/help/forms/using/admin-help/adding-domains.md#add-a-hybrid-domain)híbrido.
...
1. Escreva um container de serviço que implemente as interfaces IdentityCreator e AssignmentProvider. (Consulte [Programação com formulários](https://www.adobe.com/go/learn_aemforms_programming_63)AEM.)
...
1. Adicione provedores de autenticação. Ao adicionar provedores de autenticação, na tela Nova autenticação, selecione um Criador de identidade e Provedor de atribuição registrados. (Consulte [Configuração de provedores](configuring-authentication-providers.md#configuring-authentication-providers)de autenticação.)
1. Salve o domínio.
