...
seo-description: Saiba como adicionar, editar e excluir diretórios e configurar o gerenciamento de usuários para usar a exibição de lista virtual.
...
Para cada domínio corporativo configurado, especifique os diretórios que o provedor de autenticação consulta para obter informações do usuário. Você pode configurar vários diretórios para um domínio.

## Adicionar diretórios ou SPIs personalizados {#adding-directories-or-custom-spis}

Para cada domínio corporativo configurado, especifique os diretórios que o provedor de autenticação consulta para obter informações do usuário. Você pode adicionar um diretório a um domínio corporativo existente ou a um novo domínio corporativo que você está adicionando. Você pode configurar vários diretórios para um domínio. Você também pode configurar um domínio para usar uma SPI (Service Provider Interface) personalizada para sincronização.
...
***Observação**: Certifique-se de inserir um atributo que é garantido ser único na sua organização. Digitar um valor incorreto pode causar problemas sérios no sistema. *
...
***Observação **: Não inclua o DN do usuário nesta configuração. Para sincronizar um usuário específico, use a configuração Filtro de pesquisa.*
...
A quantidade de espaço usada para exibir o nome da chave do calendário comercial nas páginas do fluxo de trabalho dos formulários é limitada. Limite o nome da chave do calendário de negócios a menos de 53 caracteres para evitar que seja truncado nessas páginas.
...
**** Email principal: Atributo de esquema para o endereço de email principal do usuário.
...
**** Local: Atributo de esquema que contém as informações de localidade ISO. O valor é um código de idioma de duas letras ou um código de idioma e país.
...
**** Habilitar Controle de Exibição de Lista Virtual (VLV): Um controle LDAP que permite que formulários AEM recuperem dados em lotes do servidor de diretório. Se você estiver usando o Sun One como diretório LDAP e o diretório contiver muitos usuários, a ativação do VLV criará um índice que o Gerenciamento de usuários pode usar ao pesquisar usuários. Esse recurso é útil ao usar uma conta de usuário normal que pode sincronizar apenas uma quantidade limitada de dados. Você também pode ativar o VLV para grupos. Se você selecionar Ativar controle de exibição de lista virtual (VLV), especifique um nome na caixa Campo de classificação.

***Observação **: Para ativar o VLV, configure o Sun One. (Consulte[Configurar o Gerenciamento de Usuário para usar o Virtual List View (VLV)](configuring-directories.md#configure-user-management-to-use-virtual-list-view-vlv).)*

**** Campo de classificação: Se você selecionou Ativar controle de exibição de lista virtual (VLV), especifique o nome do atributo usado para classificar o índice. Esse nome de atributo (como uid) é aquele que você especificou quando criou um índice para VLV no servidor de diretório.

### Configurações do grupo {#group-settings}

**** Identificador exclusivo: (Obrigatório) Um atributo único e constante usado para identificar grupos. Use um atributo não DN como identificador exclusivo. Essa configuração depende do servidor de diretório. O valor é objectGUID para Ative Diretory 2003, nsuniqueID para Sun One e guid para eDirectory.
...
Se o DN de Membro for especificado com um atributo não DN, o Gerenciamento de Usuário usará o Identificador Exclusivo de Membro para consultar o LDAP para coletar o DN do usuário, pois ele corresponde a um valor identificador exclusivo.
...
**** Email principal: Atributo de esquema para o endereço de email principal do grupo

**** Email secundário: Atributo de esquema para o endereço de email secundário do grupo

**** Modificar carimbo de data e hora: Para ativar a sincronização de diretório delta, defina este valor para modificar o TimeStamp. (Consulte Ativar sincronização de diretório delta.)

**** Habilitar Controle de Exibição de Lista Virtual (VLV): Um controle LDAP que permite que formulários AEM recuperem dados em lotes do servidor de diretório. Se você estiver usando o Sun One como diretório LDAP e o diretório contiver muitos grupos, a ativação do VLV criará um índice que o Gerenciamento de usuários pode usar ao pesquisar grupos. Esse recurso é útil ao usar uma conta de usuário normal que pode sincronizar apenas uma quantidade limitada de dados. Você também pode ativar o VLV para usuários. Se você selecionar Ativar controle de exibição de lista virtual (VLV), especifique um nome de campo de classificação.

***Observação **: Para ativar o VLV, configure o Sun One. (Consulte[Configurar o Gerenciamento de Usuário para usar o Virtual List View (VLV)](configuring-directories.md#configure-user-management-to-use-virtual-list-view-vlv).)*

**** Nome do campo de classificação: Se você selecionou Ativar controle de exibição de lista virtual (VLV), especifique o nome do atributo usado para classificar o índice. Esse nome de atributo é aquele que você especificou ao criar um índice para VLV no servidor de diretório.

***Observação **: Clique em Testar para verificar se as configurações de usuário e grupo são coletadas com base no DN base e nos critérios de pesquisa. Se os usuários e grupos forem retornados, os resultados mostrarão os valores atribuídos a cada campo de acordo com o conjunto de atributos.*

***Observação **: O Gerenciamento de usuários não suporta IDs de usuário duplicadas em um domínio; somente um usuário com a ID de usuário é sincronizado.*

## Configurar o Gerenciamento de Usuários para usar o Virtual List View (VLV) {#configure-user-management-to-use-virtual-list-view-vlv}
...
O protocolo LDAP fornece um mecanismo para consultar grandes conjuntos de dados de forma paginada usando controles de solicitação. Ao usar o Microsoft Ative Diretory, a sincronização de banco de dados de formulários LDAP para AEM usa PagedResultsControl para recuperar dados em lotes de um tamanho específico. O servidor Sun ONE Diretory não suporta este controlo. Para concluir uma consulta paginada em relação ao Sun ONE Diretory Server, use o controle de Exibição de lista virtual (VLV). Esse controle envolve a configuração do servidor de diretório e a implementação do cliente.
...
>[!NOTE]
>
>As entradas de Objeto podem ser criadas usando a interface gráfica do usuário Sun ONE (GUI) ou por meio de um script de linha de comando. Para obter instruções sobre como criar entradas de Objeto usando a GUI, consulte a documentação do Sun ONE.
...
1. O script de amostra tem uma entrada LDAP chamada `lcuser`. Esta entrada destina-se à configuração relacionada ao VLV para sincronização de usuários em formulários AEM. Modifique as seguintes propriedades de acordo:
   ...
   **** Nome da entrada: O nome da entrada nesta amostra é `lcuser`. Se `lcuser` for alterado, deve ser alterado em todas as áreas do script de amostra.
