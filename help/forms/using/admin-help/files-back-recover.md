...
O local do GDS é determinado durante o processo de instalação dos formulários AEM ou posterior usando o console de administração. Além de manter um local de alta disponibilidade para o GDS, também é possível ativar o armazenamento do banco de dados para documentos. Consulte Opções [de backup quando o banco de dados é usado para o armazenamento](files-back-recover.md#backup-options-when-database-is-used-for-document-storage)do documento.
...
>[!NOTE]
>
>O modo de log binário padrão para MySQL é &quot;Instrução&quot;, o que é incompatível com tabelas usadas pelo Content Services (obsoleto). O uso do logon binário nesse modo padrão faz com que o Content Services (obsoleto) falhe. Se o seu sistema incluir o Content Services (obsoleto), use o modo de registro &quot;Misto&quot;. Para ativar o registro &quot;Misto&quot;, adicione o seguinte argumento ao arquivo my.ini:
>
...
O local padrão do diretório raiz do Armazenamento de conteúdo é raiz *[/lccs_data do]* GDS, onde raiz *[do]* GDS é o local descrito no local [do](files-back-recover.md#gds-location)GDS. Faça backup dos seguintes diretórios localizados no diretório raiz do Armazenamento de conteúdo:
...
>[!NOTE]
>
>Por padrão, as fontes Adobe instaladas com formulários AEM estão localizadas no diretório raiz [/fontes de formulários]aem.
