...
seo-description: Saiba como definir configurações de tipo de arquivo.
...
* Especifique quais configurações de tipo de arquivo usar por padrão (consulte [Importando e exportando arquivos de configuração do Gerador de PDF](/help/forms/using/admin-help/importing-exporting-pdf-generator-configuration.md))
...
* [Ativar suporte para PDF/A](/help/forms/using/admin-help/enable-pdf-a-support.md)
* [Excluir uma configuração de Tipo de arquivo](/help/forms/using/admin-help/enable-pdf-a-support.md)

>[!NOTE]
>
>As configurações de tipo de arquivo não estão disponíveis para os conversores de fallback, como Acrobat para conversão de HTML em PDF, Microsoft PowerPoint, Microsoft Word e Microsoft Excel.
...
1. No console de administração, clique em **[!UICONTROL Services]** > **[!UICONTROL PDF Generator]** > **[!UICONTROL Configurações de tipo de arquivo]**.
...
**Idioma do OCR principal:** o idioma usado pelo mecanismo do OCR para identificar os caracteres. O padrão é inglês (EUA).
...
**Reduzir a amostra de imagens:** Diminui o número de pixels em cores, em tons de cinza e em imagens monocromáticas. A amostragem de imagens digitalizadas é realizada após a conclusão do OCR. O padrão é Menor (600 dpi). Essa opção não estará disponível se o estilo de saída PDF for definido como Imagem pesquisável (Exata).
...
1. Preencha as informações necessárias nestas seções:
   ...
   Para ir para outra seção, clique no link na página da Web ou use os botões **[!UICONTROL Next]** ou **[!UICONTROL Previous]**.
...
1. No console de administração, clique em **[!UICONTROL Services > PDF Generator> File Type Settings]**.
...
**Idioma do OCR principal:** especifica o idioma usado pelo mecanismo do OCR para identificar os caracteres.
...
**ClearScan:** Sintetiza uma nova fonte Tipo 3 que aproxima o original e preserva o plano de fundo da página usando uma cópia de baixa resolução.

**Reduzir a amostra de imagens:** Diminui o número de pixels em cor, escala de cinza e imagens monocromáticas após a conclusão do OCR. Escolha o grau de amostragem decrescente a ser aplicado. As opções de números mais altos fazem menos downsampling, o que produz PDFs de resolução mais alta.
...
**Tente o Conversor de fallback:** o Gerador de PDF pode usar Java™ ou Acrobat para converter arquivos HTML em PDF. Quando essa opção é selecionada e uma conversão falha ou atinge o limite de tempo limite especificado, o Gerador de PDF tenta a conversão usando o método alternativo. Se um método alternativo falhar ou atingir o limite de tempo limite especificado, uma exceção será gravada no arquivo de log.
...
** O Spideringverifica as páginas da Web em busca de links para outras páginas da Web. Quando um link para outra página da Web é encontrado, a página de destino é buscada e incluída no documento PDF gerado. Ative essas opções para definir o número de níveis que serão buscados e convertidos em PDF:

**Obter somente níveis de X:** spiders e converte páginas para uma profundidade do nível especificado no URL da página base. Um valor de 1 converte somente o URL fornecido.

**Obter site inteiro:** converte o site inteiro, começando pelo URL fornecido.

**Permanecer no mesmo caminho:** todos os links que apontam para páginas que não estão no mesmo caminho relativo do URL de base não são convertidos durante a borda.

**Permanecer no mesmo servidor:** os links que apontam para páginas em servidores diferentes não são convertidos durante a criação de spiders. Somente os links que apontam para o mesmo servidor como o URL especificado são convertidos.
...
**Definir configurações da exibição inicial:** Permite configurar opções de documento, opções de janela e opções da interface do usuário. Essas configurações determinam como o conteúdo é exibido inicialmente.

### Opções de documento {#document-options}

Ative essas opções para especificar como exibir o conteúdo, como exibir páginas no documento PDF e como especificar o nível de ampliação:

**Mostrar:** Selecione os painéis a serem abertos no Acrobat quando o documento PDF for aberto.

**Layout de página:** Selecione o tipo de layout de página para o documento PDF.
...
**Abrir para número de página:** Especifique o número de página para o qual o PDF será aberto.
...
## Flash vídeos para configurações do PDF {#flash-videos-to-pdf-settings}

O PDF Generator oferece suporte à capacidade de enviar um vídeo para o Adobe Flash (arquivo SWF ou FLV) e criar um arquivo PDF com um vídeo para o Adobe Flash incorporado nele. Essa conversão não exige que o Adobe Flash Player seja instalado no servidor de formulários. Para obter instruções sobre como acessar essa opção, consulte [Criar ou editar configurações de tipo de arquivo](configuring-file-type-settings.md#create-or-edit-file-type-settings).

**Extensões de nome de arquivo:** lista separada por vírgulas de extensões de nome de arquivo que podem ser convertidas.

## Configurações XPS para PDF {#xps-to-pdf-settings}

A Especificação de Papel XML (XPS) é usada na máquina de impressão do Windows. Este é um formato Microsoft e pode ser criado a partir de qualquer aplicativo do Microsoft Office. AEM formulários fornece a capacidade de converter arquivos XPS em PDF.
...
1. Selecione uma destas opções:
   ...
   * Se necessário, selecione um diretório de fontes diferente no menu suspenso **Font source**. Este menu suspenso lista os diretórios de fontes especificados em **Início > Configurações > Sistema principal > Configurações principais**.
   * Selecione uma ou mais fontes na lista **Fontes disponíveis** e clique em **Adicionar**. Essas fontes são adicionadas à lista **Fonts to Unembed**.
   * Para desincorporar algumas fontes que não existem no servidor de formulários, digite os nomes dessas fontes na caixa **Adicionar fontes a não incorporar**. Clique em **Adicionar**.
...
1. Se quiser incorporar apenas os subconjuntos em uso das fontes incorporadas, selecione **Subdefinir todas as fontes incorporadas**.

   >[!NOTE]
   >
   >*Se você estiver usando essa opção em combinação com **Unembed some fonts**, as fontes na lista **Add fonts to**unembed ainda serão completamente desincorporadas.*
...
Se o seu documento PDF incluir arte-final que contém transparência, você poderá usar as configurações do Otimizador de PDF para aumentar a transparência e reduzir o tamanho do arquivo.

>[!NOTE]
>
>Se Acrobat 4.0 e posterior forem selecionadas como a versão do PDF de destino, todos os objetos transparentes serão nivelados. Para outras versões do Target PDF, a transparência é suportada e você pode definir as configurações de transparência.

Selecione **Transparency** para definir as configurações de transparência ao otimizar documentos PDF.

**Nível** de transparênciaEspecifica a quantidade de informações de vetor que será preservada. As configurações mais altas preservam mais objetos vetoriais, enquanto as configurações mais baixas rasterizam mais objetos vetoriais; as configurações intermediárias preservam áreas simples em forma de vetor e rasterizam áreas complexas. Selecione a configuração mais baixa para rasterizar toda a arte-final.

>[!NOTE]
>
>A quantidade de rasterização que ocorre depende da complexidade da página e dos tipos de objetos sobrepostos.

**Arte de linha e** TextoResolução à qual todos os objetos, incluindo imagens, arte vetorial, texto e gradientes, são rasterizados. Os valores suportados são de 1 pixels por polegada (ppi) a 9600 ppi.
...
**Gradiente e** Resolução de malhas, para os quais o gradiente e as malhas são rasterizados. Os valores compatíveis são de 1 ppi a 1200 ppi.
...
**Converter todo o texto em** contornosConverte todos os objetos de tipo (tipo de ponto, tipo de área e tipo de caminho) em contornos e descarta todas as informações de glifo de tipo nas páginas que contêm transparência. Essa opção garante que a largura do texto permaneça consistente durante o nivelamento. Observe que ativar essa opção fará com que as fontes pequenas pareçam ligeiramente mais espessas quando visualizadas no Acrobat ou impressas em impressoras de desktop de baixa resolução. Isso não afeta a qualidade do tipo impresso em impressoras ou imagens de alta resolução.

**Converter todos os traçados em** contornosConverte todos os traçados em caminhos preenchidos simples em páginas que contêm transparência. Essa opção garante que a largura dos traçados permaneça consistente durante o nivelamento. Observe que ativar essa opção faz com que os traçados finos pareçam um pouco mais grossos e podem degradar o desempenho do nivelamento.

**Recortar** regiões complexasGarante que os limites entre a arte-final vetorial e a arte-final rasterizada caiam ao longo de caminhos de objetos. Essa opção reduz os artefatos de costura que resultam quando parte de um log
...
* Selecione **Descartar objetos** para especificar objetos a serem removidos do PDF e otimizar linhas curvas em desenhos CAD.
...
* **Descarte as configurações** de impressão incorporadas: Remove as configurações de impressão incorporadas, como dimensionamento de página e modo duplex, do documento.
...
* **Descartar todas as imagens** alternativas: Remove todas as versões de uma imagem, exceto a versão destinada à visualização na tela. Alguns PDFs incluem várias versões da mesma imagem para diferentes propósitos, como visualização na tela de baixa resolução e impressão de alta resolução.
* **Descartar Tags** do Documento: Remove as tags do documento, o que também remove a acessibilidade e os recursos de refluxo do texto.
...
* **Descartar O Conteúdo Da Camada Oculta E Nivelar As Camadas** Visíveis: Diminui o tamanho do arquivo. O documento otimizado se parece com o PDF original, mas não contém informações de camada.
...
**Use Flate Para Codificar Fluxos Que Não Estão Codificados**: Aplica a compactação Flate a todos os fluxos que não estão codificados.

**Descartar marcadores** inválidos: Remove marcadores que apontam para páginas no documento que são excluídas.

**Descartar Destinos** Nomeados Não Referenciados: Remove destinos nomeados que não estão sendo referenciados internamente a partir do documento PDF. Essa opção não verifica links de outros arquivos PDF ou sites.

**Otimize O PDF Para A Exibição** Rápida Da Web: Reestrutura um documento PDF para download de página por vez (fornecimento de bytes) de servidores da Web.
...
**Habilite A Acessibilidade E O Reflow Com Adobe PDF** Marcado: Incorpora tags dentro do documento PDF para ativar a acessibilidade e o refluxo.
...
**[!UICONTROL Habilite A Acessibilidade E O Reflow Com Adobe PDF]** Marcado: Incorpora tags ao arquivo PDF. Por padrão, essa opção não é selecionada.

**[!UICONTROL Converter multimídia em multimídia]** em PDF: Converte multimídia em multimídia PDF, quando possível. Esta opção está selecionada por padrão.

**[!UICONTROL Converter Notas]** do Orador: Converte notas de alto-falante em PDF.
...
**[!UICONTROL Layout do PDF com base nas configurações]** da impressora do PowerPoint: Usa configurações de impressora do PowerPoint para dispor o documento PDF.
...
**[!UICONTROL Salve As Transições De Slides No Adobe PDF]**: Converte transições de slide. Esta opção está selecionada por padrão.
...
1. **[!UICONTROL Executar macros automaticamente]**: Executa todas as macros no documento do Microsoft Project (como uma macro que insere o tempo atual) antes de converter o documento.
...
**[!UICONTROL Habilite A Acessibilidade E O Reflow Com Adobe PDF]** Marcado: Incorpora tags ao arquivo PDF. Esta opção está selecionada por padrão.
...
**Abra O Painel Camadas Quando Exibido No Adobe Acrobat**: Se as camadas do Visio não estiverem niveladas, abre uma janela onde é possível especificar as camadas que são preservadas no arquivo PDF quando abertas com o Acrobat. Esta opção está selecionada por padrão.

**Criar arquivo** compatível com PDF/A-1b: Força o uso da Configuração do Adobe PDF em PDF/A-1b:2005 (RGB).

**Converter Comentários Em Comentários** Do Adobe PDF: Converte observações do Visio em comentários PDF.
...
**[!UICONTROL Abrir Painel De Camadas Quando Exibido No Adobe Acrobat]**: Mostra a estrutura de camadas quando o PDF é aberto no Acrobat.
...
**[!UICONTROL Converter todas as camadas]**: Por padrão, o Gerador de PDF converte somente a camada padrão de arquivos AutoCAD em PDF, em vez de todas as camadas dentro do arquivo. Selecione essa opção para converter todas as camadas do arquivo.
...
**[!UICONTROL Lista De Layouts Do AutoCAD A Serem Convertidos]**: Um desenho do AutoCAD pode ter vários layouts. Quando esta caixa estiver vazia, todos os layouts no desenho do AutoCAD são incluídos no documento PDF gerado. Para converter seletivamente um subconjunto dos layouts, forneça uma lista separada por vírgulas de nomes de layout.
...
**Tente o PDFMaker como Conversor** de Fallback: Quando essa opção é selecionada e uma conversão usando o OpenOffice falha ou atinge o limite de tempo limite especificado, o Gerador de PDF tenta a conversão usando o PDFMaker. Se a conversão usando o PDFMaker falhar ou atingir o limite de tempo limite especificado, uma exceção será gravada no arquivo de log.
...
**Exportar Notas**: Converte as notas nos documentos do OpenOffice em notas no documento PDF gerado.
