...
O Utilitário de migração converte os [Adaptive Forms assets](/help/forms/using/introduction-forms-authoring.md), [cloud settings](/help/sites-developing/extending-cloud-config.md) e [Correspondence Management assets](/help/forms/using/cm-overview.md) do formato usado nas versões anteriores para o formato usado no AEM 6.4 Forms. Quando você executa o utilitário de migração, as seguintes etapas são migradas:
...
Você pode [atualizar](/help/forms/using/upgrade.md) para a versão mais recente do AEM Forms 6.4 do AEM Forms 6.3 ou 6.2 ou executar uma nova instalação. Dependendo de você ter atualizado sua instalação anterior ou executado uma nova instalação, é necessário executar um dos seguintes procedimentos:
