...
Um administrador pode configurar uma pasta de rede, conhecida como pasta ** assistida, para que quando um usuário colocar um arquivo (como um arquivo PDF) na pasta assistida, uma operação pré-configurada seja iniciada e manipule o arquivo. Após a operação especificada ser executada, a operação salva o arquivo modificado em uma pasta de saída especificada. Para obter informações detalhadas sobre como administrar uma pasta assistida, consulte a Ajuda [](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md)da Administração.
...
* Você pode usar um Serviço, Fluxo de trabalho ou um Script para automatizar uma operação com uma pasta assistida. Certifique-se de que o Serviço, o Fluxo de Trabalho ou um Script correspondente esteja criado e pronto para ser executado. Para obter informações sobre como criar um Serviço, Fluxo de trabalho e Script, consulte [Vários métodos de processamento de arquivos](/help/forms/using/watched-folder-in-aem-forms.md#various-methods-for-processing-files).
* Uma pasta assistida tem várias propriedades, consulte Propriedades [da pasta](/help/forms/using/watched-folder-in-aem-forms.md#main-pars-header-1)assistida.
...
1. Toque em **Ferramentas** > **Formulários** > **Configurar pasta assistida.** Será exibida uma lista de pastas monitoradas já configuradas.
1. Toque em **Novo**. Uma lista de campos necessários para criar a pasta assistida é exibida:
   ...
   * **Processar arquivos usando**: O tipo do processo a ser iniciado. Você pode especificar fluxo de trabalho, script ou serviço.
...
1. Toque em **Avançado**. A guia avançada contém mais campos. A maioria desses campos contém um valor padrão.
   ...
   * **Excluir arquivos preparados com tempo limite quando limitados**: Se ativado, o mecanismo **Tempo limite de arquivos preparados após** ser ativado é ativado somente quando a limitação é ativada para a pasta assistida.
   ...
   * **** Tamanho do lote: Especifique o número de arquivos ou pastas a serem coletados por varredura. Evita uma sobrecarga no sistema; a verificação de muitos arquivos ao mesmo tempo pode causar uma falha. O valor padrão é 2.
      ...
      Se o intervalo de digitalização for pequeno, os threads normalmente digitalizam a pasta de entrada. Se os arquivos forem descartados com frequência na pasta assistida, mantenha o intervalo de verificação pequeno. Se os arquivos forem descartados com pouca frequência, use um intervalo de verificação maior para que outros serviços possam usar os threads.
