...
#### Configurar permissões do repositório e controles de acesso (ACLs) do repositório {#setup-repository-permissions-and-access-control-lists-acls}
...
**Autorização após o processo:** Você pode usar a autorização pós-processo para validar e controlar os resultados antes de retornar os resultados para o solicitante. Você também pode filtrar, ameaçar e inserir dados adicionais aos resultados.
...
* Há um conjunto diferente de grupos predefinidos com privilégios específicos para cada persona. Atribuir usuários ao grupo.
   ...
   * Um usuário de autores de modelo e de usuários avançados de modelo pode pré-visualização e criar um modelo.
...
Este documento fornece instruções específicas para o ambiente AEM Forms. É necessário garantir que sua instalação AEM subjacente esteja segura quando implantada. Para obter instruções detalhadas, consulte [AEM documentação da Lista](/help/sites-administering/security-checklist.md) de verificação de segurança.
