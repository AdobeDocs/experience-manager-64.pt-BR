...
# Gerenciamento de usuários da Forms | Tratamento de dados do usuário {#forms-user-management-handling-user-data}
...
O gerenciamento de usuários armazena dados de usuários em um banco de dados, como My Sql, Oracle, MS SQL Server e IBM DB2. Além disso, qualquer usuário que tenha feito logon pelo menos uma vez em aplicativos Forms em `https://[*server*]:[*host*]/lc` AEM autor, o usuário será criado AEM repositório. Portanto, o gerenciamento de usuários é armazenado nos seguintes armazenamentos de dados:
...
Os dados de gerenciamento de usuários para usuários que acessaram pelo menos uma vez os aplicativos Forms em `https://[*server*]:[*host*]/lc` também são armazenados AEM repositório.
...
Para exportar ou excluir dados do usuário do banco de dados de gerenciamento de usuários, é necessário conectar-se ao banco de dados usando um cliente de banco de dados e descobrir a ID principal com base em alguma PII do usuário. Por exemplo, para recuperar a ID principal de um usuário usando uma ID de logon, execute o seguinte comando `select` no banco de dados.
...
Execute os seguintes comandos de banco de dados para exportar dados de gerenciamento de usuários para uma ID principal das tabelas de banco de dados. No comando `select`, substitua `<principal_id>` pela ID principal do usuário cujos dados você deseja exportar.
...
1. Execute os seguintes comandos de banco de dados para excluir dados de gerenciamento de usuários de uma ID principal das tabelas de banco de dados. No comando `Delete`, substitua `<principal_id>` pela ID principal do usuário cujos dados você deseja excluir.
...
Para visualização do usuário criado no repositório AEM, faça logon em `https://[*server*]:[*port*]/lc/useradmin` com AEM credenciais de administrador. Observe que `*server*` e `*port*` no URL são os da instância do autor AEM. Aqui, você pode procurar usuários com seu nome de usuário. Clique em um duplo para visualização com informações como propriedades, permissões e grupos para o usuário. A propriedade `Path` de um usuário especifica o caminho para o nó do usuário criado AEM repositório.
...
1. Procure um usuário e clique no duplo e clique no nome de usuário para abrir as propriedades do usuário. Copie a propriedade `Path`.
