...
* **[Clique na expressão](/help/forms/using/adaptive-form-expressions.md#p-click-expression-p)**: para lidar com ações no evento click de um botão.
* **[Script de inicialização](/help/forms/using/adaptive-form-expressions.md#p-initialization-script-p):** execute uma ação na inicialização de um campo.
...
* **[Script de confirmação de valor](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p):** para alterar os componentes de um formulário após o valor de um campo ser alterado.
...
* **[Expressão](/help/forms/using/adaptive-form-expressions.md#p-step-completion-expression-p)** de conclusão de etapa: para impedir que um usuário vá para a próxima etapa de um assistente.
...
Sempre que o valor do campo **marital_status** for alterado, a expressão será reacionada. Também é possível preencher a lista suspensa em um serviço REST. Para obter informações detalhadas, consulte [Preencher dinamicamente listas suspensas](/help/forms/using/dynamically-populate-dropdowns.md).
...
>[!NOTE]
>
>Você pode desativar a execução do Script de confirmação de valor quando o valor de um campo é alterado programaticamente. Para fazer isso, vá para `https://[server]:[port]/system/console/configMgr and change` **Adaptive Forms Version for Compatibility** para **AEM Forms 6.1**. Consequentemente, o script de confirmação de valor é executado somente quando o usuário altera o valor do campo da interface do usuário.
...
Para tornar um componente obrigatório, na caixa de diálogo **[!UICONTROL Editar]** do componente, é possível selecionar a opção **[!UICONTROL Título e Texto > Obrigatório]**. Você também pode adicionar a **mensagem necessária** (opcional) apropriada. .
