...
* **[Script](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p)de confirmação de valor:**para alterar os componentes de um formulário após a alteração do valor de um campo.
...
A validação de um campo também pode ser calculada usando expressões em campos diferentes. Essas expressões são gravadas no campo Script **[!UICONTROL de]** validação da guia **[!UICONTROL Script]** da caixa de diálogo **[!UICONTROL Editar]** do componente. O status de validação de um campo depende do valor retornado pela expressão. Para obter informações sobre como gravar tais expressões, consulte [Validar Expressão](/help/forms/using/adaptive-form-expressions.md#p-validate-expression-p).
