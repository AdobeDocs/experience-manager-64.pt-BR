...
1. Especifique **create_first_ic** no campo **Title** e **Nome**. Selecione **FDM_Create_First_IC** como Modelo de dados de formulário e toque em **Próximo**.
1. No assistente **Canais**:

   1. Especifique **create_first_ic_print_template** como o modelo de impressão e toque em **Selecionar**. Certifique-se de que a caixa de seleção **Use Print as Principais for Web Canal** não esteja selecionada.
   1. Especifique **Create_First_IC_models** pasta > **Create_First_IC_Web_Template** como o modelo Web e toque em **Selecionar**.
   1. Toque em **Criar**.
...
1. Vá para a guia **Ativos** e aplique o filtro para exibir apenas os fragmentos de documento no painel esquerdo.
...
1. Toque na área de público alvo **Gráficos** e toque em **+** para adicionar um componente **Chart**.
1. Toque no componente Gráfico e selecione ![configure_icon](assets/configure_icon.png) (Configurar). As propriedades do gráfico são exibidas no painel esquerdo:
   ...
   1. Selecione a propriedade **calltype** no tipo de objeto de modelo de dados **calls** na seção **X-axis**. Toque em ![done_icon](assets/done_icon.png).
   1. Selecione **Frequency** na lista suspensa **Function**.
   1. Selecione a propriedade **calltype** no tipo de objeto de modelo de dados **calls** na seção **Y-axis**. Toque em ![done_icon](assets/done_icon.png).
   1. Toque em ![done_icon](assets/done_icon.png) para salvar as propriedades do gráfico.

1. Vá para a guia **Ativos** e aplique o filtro para exibir somente os fragmentos de layout no painel esquerdo. Arraste e solte o fragmento de layout **table_lf** na área de público alvo **Chamadas detalhadas**.
...
1. Selecione **Objeto de Modelo de Dados** na lista suspensa **Tipo de Vínculo** e selecione **chamadas** > **calldate**. Toque duas vezes em ![done_icon](assets/done_icon.png) para salvar as propriedades.
   ...
   Da mesma forma, crie vínculos com **calltime**, **callnumber**, **callduration** e **callrates** para campos de texto em **Time**, **Number**, &lt;a1 2/>Colunas Duration **e** Charges **, respectivamente.**

1. Toque na área de público alvo **PayNow** e toque em **+** para adicionar um componente **Image**.
1. Toque no componente de Imagem e selecione ![configure_icon](assets/configure_icon.png) (Configurar). As propriedades da imagem são exibidas no painel esquerdo:
   ...
   1. Toque em **Carregar**, selecione a imagem guardada no sistema de ficheiros local e toque em **Abrir**.
...
1. Toque em **Resumo de Encargos** na área de público alvo e toque em **+** para adicionar um componente **Chart**.
1. Toque no componente Gráfico e selecione ![configure_icon](assets/configure_icon.png) (Configurar). As propriedades do gráfico são exibidas no painel esquerdo:
...
1. Selecione a guia **Fontes de dados** no painel esquerdo e arraste e solte o objeto de modelo de dados **chamadas** para a área de público alvo **Chamadas detalhadas**. Todas as propriedades no objeto de modelo de dados **calls** são exibidas como colunas de tabela na área de público alvo **Chamadas detalhadas** no painel direito.
...
1. Selecione o cabeçalho da coluna da tabela **Mobilenum** e selecione **Mais opções** > **Eliminar coluna**. Da mesma forma, exclua a coluna **Calltype**.
1. Selecione o cabeçalho da coluna da tabela **Calldate** e toque em ![edit](assets/edit.png) (Editar) para renomear o texto para **Data da Chamada**. Da mesma forma, renomeie outros cabeçalhos de coluna na tabela.
1. Com base no caso de uso, insira o botão **Pagar agora** no Interative Communication que fornece ao usuário uma opção para fazer o pagamento clicando no botão. Execute as seguintes etapas para inserir o botão:

   1. Toque na área de público alvo **Pagar Agora** e toque em **+** para adicionar um componente **Texto**.
   1. Toque no componente de texto e toque em ![edit](assets/edit.png) (Editar).
...
1. Toque na área de público alvo **Pagar Agora** e toque em **+** para adicionar um componente **Image**.
1. Toque no componente de Imagem e selecione ![configure_icon](assets/configure_icon.png) (Configurar). As propriedades da imagem são exibidas no painel esquerdo:

   1. Especifique **PayNow** como o nome da imagem no campo **Name**.
   1. Toque em **Carregar**, selecione a imagem **PayNowWeb** guardada no sistema de ficheiros local e toque em **Abrir**.
   1. Toque em ![done_icon](assets/done_icon.png) para salvar as propriedades da imagem.

1. Com base no caso de uso, insira um botão **Assinar** no Interative Communication que fornece ao usuário uma opção para assinar os serviços de valor agregado clicando no botão.

   Repita as etapas de 13 a 17 para adicionar um botão **Assinar** à área de público alvo **Serviços de valor agregado** e adicione a imagem **ServiçosWeb** do ValueAddedWeb.
...
1. No assistente **Canais**:
   ...
   1. Marque a caixa de seleção **Usar Imprimir como Principal para o Canal da Web**.
   1. Especifique **Create_First_IC_models** pasta > **Create_First_IC_Web_Template** como o modelo Web e toque em **Selecionar**.
   1. Toque em **Criar**.
...
1. Toque em **Editar** para abrir a Comunicação interativa no painel direito.
1. Execute as etapas 6 - 15 da seção [Criar comunicação interativa para canal de impressão](#create-interactive-communication-for-print-channel).
...
1. Como a caixa de seleção **Usar Imprimir como Principal para o Canal da Web** está selecionada na etapa 4, o conteúdo e os vínculos são gerados automaticamente para o canal da Web a partir do canal Imprimir.
   ...
   Se você cancelou a herança de um componente, poderá reativá-la. Para reativar a herança, passe o mouse sobre o limite da área do público alvo relevante, que inclui o componente, e toque ![reabilitar herança](assets/reenableinheritance.png).
...
1. Repita as etapas 13 - 18 de [Criar comunicação interativa para o canal Web](#create-interactive-communication-for-web-channel) para inserir os hiperlinks **Pagar agora** e **Assinar** no canal Web da Comunicação interativa.
