...
description: Os workflows AEM Forms JEE fornecem ferramentas para projetar, criar e gerenciar processos comerciais. Saiba mais sobre como acessar e excluir dados do usuário, identificar IDs de instância do processo quando o iniciador ou participante do fluxo de trabalho for conhecido, identificar IDs de instância do processo quando os dados do usuário forem armazenados em variáveis primitivas, expurgar dados do usuário de instâncias do fluxo de trabalho com base em IDs de instância do processo e trabalhar com tarefas órfãs.
...
Para obter mais informações sobre como criar o processo de fluxo de trabalho do AEM Forms JEE, consulte [Ajuda do Workbench](https://helpx.adobe.com/content/dam/help/en/experience-manager/6-4/forms/pdf/WorkbenchHelp.pdf).
...
1. Crie uma instância do cliente público `ProcessManager` ( `com.adobe.idp.workflow.client.ProcessManager`) usando uma instância `ServiceClientFactory` com as configurações de conexão corretas.
   ...
   Para obter mais informações, consulte a referência da API Java para [Class ProcessManager](https://helpx.adobe.com/experience-manager/6-4/forms/ProgramLC/javadoc/com/adobe/idp/workflow/client/ProcessManager.html).
