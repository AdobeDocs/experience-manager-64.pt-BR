...
Você pode configurar uma ação de envio na seção **[!UICONTROL Submission]** das propriedades do Container de formulário adaptável, na barra lateral.

![Configurar a ](assets/thank-you-setting.png)
**ação EnviarFigura:** *Configurar ação de envio*
...
>[!NOTE]
>
>Certifique-se de que [AEM_Installation_Diretory]\crx-quickstart\temp\datamanager\ASM folder exists. O diretório é necessário para armazenar temporariamente anexos. Se o diretório não existir, crie-o.

>[!CAUTION]
>
>Se você [prefill](/help/forms/using/prepopulate-adaptive-form-fields.md) um modelo de formulário, modelo de dados de formulário ou formulário adaptável com base em schemas com queixa de dados XML ou JSON para um schema (schema XML, schema JSON, modelo de formulário ou modelo de dados de formulário) que sejam dados, os dados não conterão tags &lt;afData>, &lt;afBoundData> e &lt;/afUnboundData>, então os dados de campos (campos não vinculados são campos de formulário adaptáveis sem a propriedade [bindref](/help/forms/using/prepopulate-adaptive-form-fields.md)) do formulário adaptável é perdida.

Você pode gravar uma ação de envio personalizada para formulários adaptáveis para atender ao seu caso de uso. Para obter mais informações, consulte [Gravar ação de envio personalizado para formulários adaptáveis](/help/forms/using/custom-submit-action-form.md).

## Enviar para o ponto de extremidade REST {#submit-to-rest-endpoint}

A opção de envio **[!UICONTROL Enviar para o ponto de extremidade REST]** transmite os dados preenchidos no formulário para uma página de confirmação configurada como parte da solicitação de GET HTTP. É possível adicionar o nome dos campos a serem solicitados. O formato da solicitação é:
...
Você também pode **[!UICONTROL Habilitar solicitação de POST]** e fornecer um URL para postar a solicitação. Para enviar dados ao servidor AEM que hospeda o formulário, use um caminho relativo correspondente ao caminho raiz do servidor AEM. Por exemplo, /content/forms/af/SampleForm.html. Para enviar dados a qualquer outro servidor, use o caminho absoluto.
...
A ação **[!UICONTROL Enviar e-mail]** enviar envia um e-mail para um ou mais recipient após o envio bem-sucedido do formulário. O email gerado pode conter dados de formulário em um formato predefinido.
...
A ação de envio **[!UICONTROL Enviar PDF por email]** envia um email com um PDF contendo dados de formulário para um ou mais recipient após o envio bem-sucedido do formulário.
...
A opção **[!UICONTROL Enviar para o fluxo de trabalho da Forms]** enviar envia um xml de dados e anexos de arquivo (se houver) para um LiveCycle Adobe ou AEM Forms em um processo JEE.

Para obter informações sobre como configurar a ação de envio do fluxo de trabalho Enviar para formulários, consulte [Enviar e processar seus dados de formulário usando workflows de formulário](/help/forms/using/submit-form-data-livecycle-process.md).
...
Para obter informações sobre o modelo de dados de formulário, consulte [Integração de dados da AEM Forms](/help/forms/using/data-integration.md).
...
Para obter mais informações sobre o Forms Portal e a ação de envio, consulte [Componente de rascunhos e envios](/help/forms/using/draft-submission-component.md).

## Chamar um fluxo de trabalho AEM {#invoke-an-aem-workflow}

A ação de envio **[!UICONTROL Chamar um fluxo de trabalho AEM]** associa um formulário adaptável a um fluxo de trabalho AEM. Quando um formulário é submetido, o fluxo de trabalho associado é start automaticamente no nó de processamento. Além disso, coloca o arquivo de dados, os anexos e o documento de Registro, se aplicável, no local da carga do fluxo de trabalho.

Antes de usar a ação de envio **[!UICONTROL Chamar um Fluxo de Trabalho AEM]**, [defina as configurações AEM DS](/help/forms/using/configuring-the-processing-server-url-.md). Para obter informações sobre como criar um Fluxo de trabalho AEM, consulte [workflows centrados em formulários no OSGi](/help/forms/using/aem-forms-workflow.md).
...
**validação no lado do servidorFigura:** *Ativação da validação no lado do servidor*
