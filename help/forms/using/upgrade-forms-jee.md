...
A atualização direta do LiveCycle ES2, LiveCycle ES3, AEM 6.0 Forms, AEM 6.1 Forms para AEM 6.4 Forms não está disponível. Você pode fazer uma atualização intermediária para uma ou mais versões do LiveCycle ou AEM Forms e, em seguida, atualizar do AEM 6.4 Forms. Para obter a lista de versões intermediárias e as instruções de atualização correspondentes, consulte [Escolher um caminho](/help/forms/using/upgrade.md#main-pars-header)de atualização.
...
1. Antes de atualizar:
   ...
   1. Decida o tipo Repositório de conteúdo (Repositório CRX) a ser usado. Somente alguns formulários AEM em recursos JEE usam a persistência do Repositório de conteúdo para armazenar conteúdo e ativos. Instale e configure o Repositório de conteúdo somente se planeja usar esses formulários AEM nos recursos JEE:
      ...
      * No LiveCycle ES4 SP1, os recursos de Gerenciamento de correspondência, Portal de formulários, Espaço de trabalho HTML e Relatório de processos usam o Repositório de conteúdo. Se você não usou esses recursos no LiveCycle ES4 e planeja não usá-los no AEM Forms, o Repositório de conteúdo não é necessário.
      * Em Formulários AEM, Formulários adaptáveis, Gerenciamento de correspondência, Formulários HTML5 (conhecidos como Formulários móveis no LiveCycle ES4 SP1), Portal de formulários, Espaço de trabalho HTML, Relatórios de processos, Fluxos de trabalho centrados no Forms no OSGi, os recursos usam o Repositório de conteúdo. Se você planeja usar o AEM Forms para esses recursos, o Repositório de conteúdo é necessário.
      * Não é necessário o Repositório de conteúdo para o AEM Forms Document Security.
   ...
   1. Certifique-se de que sua plataforma existente (ou seja, servidor de aplicativos, banco de dados, sistema operacional, Adobe Acrobat, aplicativos de terceiros e hardware) seja compatível com o AEM 6.4 Forms no JEE. Para obter informações sobre hardware e software suportados, consulte o documento Combinações [de plataforma](/help/forms/using/aem-forms-jee-supported-platforms.md) suportadas.
      ...
      >[!NOTE]
      >
      >Se você estiver usando o formato de persistência RDBMK, use um único banco de dados para a persistência do repositório e serviços de documento em execução no AEM Forms em JEE.


1. Execute a atualização:

   1. Instale o AEM 6.4 Forms no JEE em um novo servidor executando o programa de instalação. O instalador coloca todos os arquivos necessários no computador, dentro de uma estrutura de diretório de instalação.
   1. Depois que a instalação for concluída, execute o **Configuration Manager** para configurar vários módulos AEM Forms e definir as configurações apropriadas. Juntamente com a configuração de configurações, permite especificar o caminho do Global Data Storage (GDS) e do repositório crx.
   ...
   1. (Não é necessário para o módulo de segurança de documento do AEM Forms) Importe conteúdo para o repositório de conteúdo (CRX-Repository) para o servidor de formulários do AEM 6.4.
...
1. Execute as tarefas de pós-implantação para verificar as credenciais de logon, configurar os serviços de documento, o gerenciamento de correspondência, a segurança do documento e muito mais, dependendo do caso de uso.
...
**Dependendo do ambiente existente e do servidor de aplicativos, escolha um dos seguintes documentos e siga as instruções detalhadas:**
...
* [Atualizar do LiveCycle ES4 SP1 para o AEM 6.4 Forms for WebSphere](assets/upgrade-websphere-livecycle.pdf)
...
1. Antes de atualizar:
   ...
   1. Decida o tipo Repositório de conteúdo (Repositório CRX) a ser usado. Somente alguns formulários AEM em recursos JEE usam a persistência do Repositório de conteúdo para armazenar conteúdo e ativos. Instale e configure o Repositório de conteúdo somente se planeja usar esses formulários AEM nos recursos JEE:
      ...
      * Em Formulários AEM, Formulários adaptáveis, Gerenciamento de correspondência, Formulários HTML5, Portal de formulários, Espaço de trabalho HTML, Relatório de processos e Fluxos de trabalho centrados no Forms em recursos OSGi usam Repositório de conteúdo. Se você planeja usar o AEM Forms para esses recursos, o Repositório de conteúdo é necessário.
      * Não é necessário o Repositório de conteúdo para o AEM Forms Document Security.
      Além disso, os tipos de repositório do LiveCycle e do AEM Forms são diferentes. Para obter os tipos de repositório disponíveis e informações relacionadas, consulte [Escolhendo um tipo de persistência para uma instalação](/help/forms/using/choosing-persistence-type-for-aem-forms.md)do AEM Forms.

   1. Crie um backup do banco de dados do LiveCycle ES3, do Global Data Storage (GDS) e do Repositório de conteúdo (não é necessário para segurança de documentos). Se você estiver atualizando para persistência MongoMK ou RDBMK, exporte ativos de gerenciamento de correspondência do LiveCycle ES3 como um arquivo.
   1. Certifique-se de que sua plataforma existente (ou seja, servidor de aplicativos, banco de dados, sistema operacional, Adobe Acrobat, aplicativos de terceiros e hardware) seja compatível com o AEM 6.4 Forms no JEE. Para obter informações sobre hardware e software suportados, consulte o documento Combinações [de plataforma](/help/forms/using/aem-forms-jee-supported-platforms.md) suportadas.

      Se você criar uma nova instância do banco de dados, importe os dados cujo backup foi feito na etapa 3 para o banco de dados. Para obter informações sobre como importar dados para um banco de dados, consulte a documentação do fornecedor do banco de dados correspondente.

      >[!NOTE] Se você estiver usando o formato de persistência RDBMK, use um único banco de dados para a persistência do repositório e serviços de documento em execução no AEM Forms em JEE.
...
1. Execute a atualização:
   ...
   1. (Não é necessário para o módulo de segurança de documento do AEM Forms) Atualize e importe o repositório do CRX para o servidor do AEM 6.4 Forms.
