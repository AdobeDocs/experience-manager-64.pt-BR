...
1. Toque **[!UICONTROL Criar]** para escrever uma nova regra. O editor visual da interface do usuário do editor de regras é aberto por padrão quando você inicia o editor de regras pela primeira vez.
   ...
   [Clique para ampliar](assets/rule-editor-ui-1.png)
 
...
Além das funções prontas para uso como *Soma de, *que estão listadas em Saída de funções, você pode gravar funções personalizadas que precisa com frequência. Certifique-se de que a função gravada seja acompanhada pela função `jsdoc` acima.
...
* **Parâmetro**
   ...
   Todos os outros tipos de parâmetros são categorizados em um dos acima. Nenhum é suportado. Certifique-se de selecionar um dos tipos acima. Os tipos não fazem distinção entre maiúsculas e minúsculas. Espaços não são permitidos no parâmetro `name`. `<Parameter Descrption>`
...
1. Toque **Criar regra**. Usando as opções mostradas abaixo, crie uma regra para salvar o valor quadrado da entrada no campo Saída do formulário.
   [ ![Uso de funções personalizadas para criar uma regra](assets/add-custom-rule.png)](assets/add-custom-rule-1.png)Toque **Concluído**. Sua função personalizada é adicionada.
