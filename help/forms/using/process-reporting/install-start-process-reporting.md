...
Para obter detalhes sobre os relatórios predefinidos, consulte [Relatórios predefinidos no Relatórios de processamento](pre-defined-reports-in-process-reporting.md).

**Query Ad-hoc:** Este item contém opções para executar pesquisa baseada em filtro para processos e tarefas.

Para obter detalhes sobre query ad-hoc, consulte [Query Ad-hoc no Relatórios Process](adhoc-queries-in-process-reporting.md).
