...
seo-title: Preparando o AEM Forms para backup
...
seo-description: Saiba como usar o serviço de Backup e Restauração para entrar e sair do modo de Backup do servidor AEM Forms usando a API Java e a API do serviço da Web.
...
>[!NOTE]
>
>Para obter mais informações sobre o que considerar ao executar backups para o AEM Forms, consulte [ajuda de administração](https://www.adobe.com/go/learn_aemforms_admin_63).
...
Para obter informações sobre a localização desses arquivos, consulte [Incluindo arquivos da biblioteca Java do AEM Forms](/help/forms/developing/invoking-aem-forms-using-java.md#including-aem-forms-java-library-files).
...
1. Entre no modo de backup
   ...
   Entre no modo de backup, chamando o método `enterBackupMode` com os seguintes parâmetros:

   * Um valor `String` que especifica um rótulo exclusivo legível por humanos que identifica a sessão do modo de backup. É recomendável não usar espaços ou caracteres que não possam ser codificados no formato XML.
   ...
   * Um valor `Boolean` que especifica se deve estar no modo de backup contínuo. Um valor `True` especifica estar no modo de backup contínuo. Quando no modo de backup contínuo, o valor especificado para o número de minutos para permanecer no modo de backup é ignorado.

      O modo de backup contínuo significa que uma nova sessão de modo de backup é iniciada após a conclusão da atual. Um valor `False` significa que o modo de backup contínuo não é usado e, após sair do modo de backup, a limpeza de arquivos do GDS é retomada.

1. Recuperar informações sobre a sessão do modo de backup no servidor

   Recupere as informações usando o objeto `BackupModeEntryResult` que é retornado após chamar o método `enterBackupMode`. As informações que podem ser recuperadas depois de entrar no modo de backup podem ser úteis para integração com seus procedimentos de backup. Por exemplo, o rótulo, a ID de backup e a hora de início podem ser úteis como entrada para nomes de arquivo para o procedimento de backup.
...
1. Incluir arquivos de projeto
   ...
   * Crie um conjunto de clientes Microsoft .NET que consome o WSDL da API do Serviço de Backup e Restauração.
...
1. Entre no modo de backup
   ...
   * Um valor `Boolean` que especifica se deve estar no modo de backup contínuo. Um valor `True` especifica estar no modo de backup contínuo. Quando no modo de backup contínuo, o valor especificado para o número de minutos para permanecer no modo de backup é ignorado. O modo de backup contínuo significa que uma nova sessão de modo de backup é iniciada após a conclusão da atual.

      Um valor `False` significa que o modo de backup contínuo não é usado e, após sair do modo de backup, a limpeza de arquivos do GDS é retomada.
...
Antes de gravar aplicativos para entrar no modo de licença, é recomendável compreender os procedimentos de backup usados com o AEM Forms. Para obter mais informações sobre o que considerar ao executar backups para o AEM Forms, consulte [ajuda de administração](https://www.adobe.com/go/learn_aemforms_admin_63).
...
1. Recuperar informações sobre a sessão do modo de backup no servidor
   ...
   Recupere informações sobre a operação usando o objeto `BackupModeResult` retornado. As informações que podem ser recuperadas depois de entrar no modo de backup podem ser úteis para integração com seus procedimentos de backup. Por exemplo, o rótulo, a ID de backup e a hora de início podem ser úteis como entrada para nomes de arquivo para o procedimento de backup.
...
1. Entre no modo de backup
   ...
   Deixe o modo de backup chamando a operação de serviço da Web `leaveBackupMode`.
