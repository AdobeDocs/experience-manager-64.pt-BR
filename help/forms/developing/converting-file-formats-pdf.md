...
* adobe-livecycle-native-jboss-x86_win32.ear > adobe-Native2PDFSvc.war\WEB-INF\lib > adobe-native.jar > Native2PDFSvc-native.jar\bin > adobe-appmondata.jar\com\adobe\appmon. O arquivo adobe-livecycle-native-jboss-x86_win32.ear está na pasta de exportação em *[Diretório de instalação do AEM forms]\*configurationManager. (se o AEM Forms for implantado em outro servidor de aplicativos J2EE, substitua o arquivo adobe-livecycle-native-jboss-x86_win32.ear pelo arquivo EAR que corresponde ao servidor de aplicativos J2EE.)
* adobe-generatepdf-dsc.jar > adobe-appmondata.jar\com\adobe\appmon (o arquivo adobe-appmondata.jar está no arquivo adobe-generatepdf-dsc.jar). O arquivo adobe-generatepdf-dsc.jar está no *[Diretório de instalação do AEM forms]*\implantar pasta.
...
>[!NOTE]
>
>Neste contexto, o termo adicional significa o conteúdo do aplicativo.[nome do aplicativo].adição.[locale]Arquivo .xml. Esse arquivo especifica substituições e adições ao arquivo XML da caixa de diálogo.
...
O nome do arquivo que identifica um arquivo XML de diálogo adicional é appmon.*[appname]*.adição.*[locale]*.xml. Um exemplo é appmon.excel.add.en_US.xml.

O nome do arquivo XML da caixa de diálogo adicional deve usar o formato appmon.*[nome do aplicativo]*.adição.*[locale]*.xml, onde *nome do aplicativo* deve corresponder exatamente ao nome do aplicativo usado no arquivo de configuração XML e no script.
...
Crie uma variável de ambiente que especifique a localização do executável do aplicativo nativo. A variável deve usar o formato *[nome do aplicativo]*_PATH, em que *nome do aplicativo* deve corresponder exatamente ao nome do aplicativo usado no arquivo de configuração XML e no script, e onde o caminho contém o caminho para o executável em aspas duplas. Um exemplo dessa variável de ambiente é `Photoshop_PATH`.
...
1. Na caixa de diálogo Nova variável do sistema, na **Nome da variável** digite um nome que use o formato *[nome do aplicativo]*_CAMINHO.
