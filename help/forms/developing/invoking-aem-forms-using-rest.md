...
# Chamada do AEM Forms usando solicitações REST {#invoking-aem-forms-using-rest-requests}
...
Para chamar um serviço Forms (um processo se torna um serviço quando o processo é ativado) usando REST, configure um terminal REST. (Consulte &quot;Gerenciando pontos de extremidade&quot; na ajuda [](https://www.adobe.com/go/learn_aemforms_admin_63)administrativa.)
...
O `ServiceName` valor obrigatório é o nome do serviço Forms a ser chamado. O `OperationName` valor opcional é o nome da operação do serviço. Se esse valor não for especificado, esse nome assumirá `invoke`, que é o nome da operação que start o processo. O `ServiceVersion` valor opcional é a versão codificada no formato X.Y. Se esse valor não for especificado, a versão mais recente será usada. O `enctype` valor também pode ser `application/x-www-form-urlencoded`.
...
* Objetos de coleção como `java.util.List` e `java.util.Map`
   ...
   Se um serviço Forms exigir um `com.adobe.idp.Document` parâmetro, o serviço só poderá ser chamado usando o método POST HTTP. Se o serviço exigir um `com.adobe.idp.Document` parâmetro, o corpo da solicitação HTTP se tornará o conteúdo do objeto de Documento de entrada.
...
Um serviço AEM Forms pode ser chamado de forma assíncrona, substituindo-o `services` pelo URL `async_invoke` de invocação, como mostrado no exemplo a seguir.
...
## Serviços AEM Forms que suportam invocação REST {#aem-forms-services-that-support-rest-invocation}
...
O exemplo HTML a seguir chama um processo AEM Forms chamado `MyApplication/EncryptDocument` que requer um documento PDF. Para obter informações sobre esse processo, consulte [Invocar o AEM Forms usando o MTOM](/help/forms/developing/invoking-aem-forms-using-web.md#invoking-aem-forms-using-mtom).
...
Você também pode enviar dados de formulário como XML para um processo Forms. Para enviar dados XML, verifique se a `Submit As` lista suspensa especifica XML. Como o valor de retorno do processo deve ser um documento PDF, o documento PDF é exibido no Acrobat.
