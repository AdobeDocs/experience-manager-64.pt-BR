...
1. Defina as opções de tempo de execução da criptografia.
   ...
   * Especifique a opção de compatibilidade do Acrobat, chamando o `PasswordEncryptionOptionSpec` método do objeto `setCompatability` e transmitindo um valor de enumeração que especifica o nível de compatibilidade do Acrobat. For example, you can specify `PasswordEncryptionCompatability.ACRO_7`.
