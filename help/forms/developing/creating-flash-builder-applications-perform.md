...
>[!NOTE]
>
>Antes de ler esta seção, é recomendável que você esteja familiarizado com Chamar o AEM Forms usando o Remoting. (Consulte [Invocando o AEM Forms usando o AEM Forms Remoting](/help/forms/developing/invoking-aem-forms-using-remoting.md#invoking-aem-forms-using-remoting).)
...
1. Se o servlet de segurança autenticar o usuário, ele será `ISSOManager` despachado `SSOEvent.AUTHENTICATION_SUCCESS`. Como resposta, o aplicativo cliente mostra a página principal. Neste exemplo, a página principal chama o processo AEM Forms de curta duração chamado MyApplication/EncryptDocument.
...
* `SSOStandalone.mxml`: O arquivo principal MXML que representa o aplicativo cliente. (Consulte [Criação do arquivo](creating-flash-builder-applications-perform.md#creating-the-ssostandalone-mxml-file)SSOStandalone.mxml.)
...
* `views/remoting.mxml`: Representa a visualização que chama o processo de duração curta da AEM Forms chamado MyApplication/EncryptDocument usando remoting. (Consulte [Criação do arquivo](creating-flash-builder-applications-perform.md#creating-the-remoting-mxml-file)remoting.mxml.)
...
Nessa situação, o usuário tenta fazer logon de um aplicativo cliente para a AEM Forms pela primeira vez. (nenhuma sessão anterior envolvendo o usuário existe.) No `applicationComplete` evento, é chamado o `SSOManager.singleSignOn` método que envia uma solicitação para o Gerenciador de usuários.
