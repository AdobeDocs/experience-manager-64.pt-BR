...
* Também é possível melhorar o desempenho do serviço Forms renderizando um formulário no cliente. (Consulte [Renderização do Forms no Cliente](/help/forms/developing/rendering-forms-client.md).)
...
1. Definir opções de tempo de execução de desempenho
   ...
   * Defina a opção linearizada chamando o `PDFFormRenderSpec` método do `setLinearizedPDF` objeto e passando `true.`
