...
O serviço Endpoint Registry fornece a capacidade de gerenciar programaticamente os pontos de extremidade. Por exemplo, é possível adicionar os seguintes tipos de pontos finais a um serviço:
...
* Gerenciador de tarefas
   ...
   ***Observação **: SOAP, EJB e (obsoleto para formulários AEM no JEE) Os pontos finais remotos são criados automaticamente para cada serviço ativado. Os endpoints SOAP e EJB habilitam SOAP e EJB para todas as operações de serviço.*
   ...
   Os pontos finais Email, Gerenciador de tarefas e Pasta assistida expõem apenas uma operação específica do serviço. A adição desses pontos finais requer uma segunda etapa de configuração para selecionar um método para chamar, definir parâmetros de configuração e especificar mapeamentos de parâmetros de entrada e saída.

   Você pode organizar pontos de extremidade do TaskManager em grupos chamados *categorias*. Essas categorias são então expostas ao Workspace por meio do TaskManager, com os usuários finais visualizando os pontos finais do TaskManager conforme são categorizados. No Workspace, os usuários finais visualizam essas categorias no painel de navegação. Os pontos de extremidade dentro de cada categoria são exibidos como cartões de processo na página Iniciar processos no Workspace.

   Você pode realizar essas tarefas usando o serviço Endpoint Registry:

* Adicionar pontos de extremidade EJB. (Consulte [Adicionando pontos finais](programmatically-endpoints.md#adding-ejb-endpoints)EJB.)
...
* Adicionar pontos de extremidade de email.  (Consulte [Adicionar Pontos de Extremidade](programmatically-endpoints.md#adding-email-endpoints)de Email.)
...
* **Identificador** do serviço: Especifica o serviço ao qual o terminal pertence.
...
Depois de criar um novo terminal, é necessário ativá-lo. Depois de ativar o terminal, ele pode ser usado para chamar o serviço. Depois de ativar o terminal, você pode visualizá-lo no console de administração.
...
1. Defina atributos de ponto de extremidade EJB.
   ...
   * Especifique a descrição do ponto de extremidade chamando o método do `CreateEndpointInfo` `setDescription` objeto e transmitindo um valor de string que descreve o ponto de extremidade.
...
1. Ative o terminal.
   ...
   Ative o terminal chamando o método enable do `EndpointRegistryClient` objeto e transmitindo o `Endpoint` objeto retornado pelo `createEndpoint` método.
...
* **Descrição**: Especifica a descrição do ponto de extremidade.
* **Nome**: Especifica o nome do ponto final.
* **Valor** do identificador de serviço: Especifica o serviço ao qual o terminal pertence.
...
Depois de criar um novo terminal, é necessário ativá-lo. Quando o terminal está ativado, ele pode ser usado para chamar o serviço. Depois de ativar o terminal, você pode visualizá-lo no console de administração.
...
Para criar um terminal de Pasta assistida para um serviço, especifique os seguintes valores:

* **Identificador** do conector: Especifica o tipo de endpoint criado. Para criar um ponto final de Pasta assistida, especifique `WatchedFolder`.
* **Descrição**: Especifica a descrição do ponto final.
* **Nome**: Especifica o nome do ponto final.
* **Identificador** do serviço: Especifica o serviço ao qual o terminal pertence. Por exemplo, para adicionar um endpoint de Pasta assistida ao processo que é introduzido nesta seção (um processo se torna um serviço quando ativado usando o Workbench), especifique `EncryptDocument`.
...
A lista a seguir especifica os valores de configuração que são definidos ao adicionar programaticamente um ponto final de Pasta assistida a um serviço:
...
* **againCount**: O número de vezes que uma Pasta assistida verifica a pasta ou o diretório. O valor -1 indica uma varredura indefinida. O valor padrão é -1.
...
* **userName**: O nome de usuário usado ao chamar um serviço de destino da Pasta assistida. Este valor é obrigatório. O valor padrão é SuperAdmin.
...
* **overwriteDuplicateFilename**: Quando definido como true, os arquivos na pasta de resultados e a pasta preserve são substituídos. Quando definido como falso, os arquivos e pastas com sufixo de índice numérico são usados para o nome. O valor padrão é false.
...
Depois de criar um terminal de Pasta assistida, é necessário ativá-lo. Quando o terminal está ativado, ele pode ser usado para chamar o serviço. Depois de ativar o terminal, você pode visualizá-lo no console de administração.
...
1. Especifique os valores de configuração.
   ...
   * Um valor de string que especifica o nome do valor de configuração. Ao definir o valor `url` de configuração, especifique `url`.
...
1. Ative o terminal.
   ...
   Ative o terminal chamando o `EndpointRegistryClient` método do `enable` objeto e transmitindo o `Endpoint` objeto retornado pelo `createEndpoint` método.
...
* **Nome**: Especifica o nome do ponto final.
* **Valor** do identificador de serviço: Especifica o serviço ao qual o terminal pertence. Por exemplo, para adicionar um terminal de Email ao processo que é introduzido nesta seção (um processo se torna um serviço quando ativado usando o Workbench), especifique `EncryptDocument`.
...
* **againCount**: Número de vezes que o ponto de extremidade de email verifica a pasta ou o diretório. O valor -1 indica uma varredura indefinida. O valor padrão é -1.
...
* **startDelay**: O tempo de espera para a verificação depois que o agendador é iniciado. A hora padrão é 0.
...
* **userName**: O nome de usuário usado ao chamar um serviço de destino do email. O valor padrão é `SuperAdmin`.
...
* **filePattern**: Especifica os padrões de anexo de arquivo de entrada aceitos pelo provedor. Isso inclui arquivos com extensões de nome de arquivo específicas (&amp;ast;.dat, &amp;ast;.xml), arquivos com nomes específicos (dados) e arquivos com expressões compostas no nome e na extensão (&amp;ast;.[dD][A][Tt]). O valor padrão é `*`.
* **receiptSuccessfulJob**: Um endereço de email para o qual as mensagens são enviadas para indicar trabalhos bem-sucedidos. Por padrão, uma mensagem de trabalho bem-sucedida é sempre enviada ao remetente. Se você digitar `sender`, os resultados do email serão enviados ao remetente. Há suporte para até 100 destinatários. Especifique destinatários adicionais com endereços de email, cada um separado por vírgula. Para desativar essa opção, deixe esse valor em branco. Em alguns casos, talvez você queira acionar um processo e não desejar uma notificação por email do resultado. O valor padrão é `sender`.
* **receiptFailedJob**: Um endereço de email para o qual são enviadas mensagens para indicar trabalhos com falha. Por padrão, uma mensagem de falha é sempre enviada ao remetente. Se você digitar `sender`, os resultados do email serão enviados ao remetente. Há suporte para até 100 destinatários. Especifique destinatários adicionais com endereços de email, cada um separado por vírgula. Para desativar essa opção, deixe esse valor em branco. O valor padrão é `sender`.
...
* **smtpPassword**: A senha da conta SMTP. Alguns servidores de email não exigem senha SMTP.
...
Depois de criar um terminal de email, é necessário ativá-lo. Quando o terminal está ativado, ele pode ser usado para chamar o serviço. Depois de ativar o terminal, você pode visualizá-lo no console de administração.
...
1. Especifique os valores de configuração.
   ...
   Para que cada valor de configuração seja definido para o ponto de extremidade Email, é necessário chamar o método do `CreateEndpointInfo` objeto `setConfigParameterAsText` . Por exemplo, para definir o valor de `smtpHost` configuração, chame o método `CreateEndpointInfo` do objeto `setConfigParameterAsText` e passe os seguintes valores:

   * Um valor de string que especifica o nome do valor de configuração. Ao definir o valor `smtpHost` de configuração, especifique `smtpHost`.
...
* **Descrição**: Especifica a descrição do ponto final.
* **Nome**: Especifica o nome do ponto final.
* **Valor** do identificador de serviço: Especifica o serviço ao qual o terminal pertence. Por exemplo, para adicionar um terminal Remoto ao processo que é introduzido nesta seção (um processo se torna um serviço quando é ativado no Workbench), especifique `EncryptDocument`.
...
1. Defina atributos de ponto de extremidade do TaskManager.
...
* **Nome**: Especifica o nome do ponto final.
* **Identificador** do serviço: Especifica o serviço ao qual o terminal pertence.
* **Categoria**: Especifica um valor identificador de categoria associado ao ponto final do TaskManager.
...
Depois de criar um novo terminal, é necessário ativá-lo. Quando o terminal está ativado, ele pode ser usado para chamar o serviço a partir do Workspace. Depois de ativar o terminal, você pode visualizá-lo no console de administração.
...
1. Crie uma categoria para o terminal.
   ...
   * Crie a categoria chamando o método do `EndpointRegistryClient` objeto `createEndpointCategory` e transmitindo o `CreateEndpointCategoryInfo` objeto. Esse método retorna um `EndpointCategory` objeto que representa a nova categoria.
...
1. Defina atributos de ponto de extremidade do TaskManager.
   ...
   * Especifique o nome do ponto de extremidade chamando o método do `CreateEndpointInfo` `setName` objeto e transmitindo um valor de string que especifica o nome.
   * Especifique o serviço ao qual o ponto de extremidade pertence, chamando o método do `CreateEndpointInfo` objeto `setServiceId` e transmitindo um valor de string que especifica o nome do serviço.
   * Especifique a categoria à qual o ponto final pertence, chamando o método do `CreateEndpointInfo` objeto `setCategoryId` e transmitindo um valor de string que especifica o valor do identificador da categoria. Você pode chamar o método do `EndpointCategory` objeto para obter o valor identificador `getId` dessa categoria.
...
## Modificando Pontos Finais {#modifying-endpoints}
...
1. Especifique novos valores de configuração.
   ...
   * Para cada valor de configuração a ser definido, chame o `ModifyEndpointInfo` método do `setConfigParameterAsText` objeto. Por exemplo, para definir o valor de configuração de url, chame o método do `ModifyEndpointInfo` objeto `setConfigParameterAsText` e passe os seguintes valores:
...
Você pode remover programaticamente um terminal de um serviço usando a API Java de formulários AEM. Após remover um terminal, o serviço não poderá ser chamado usando o método de invocação que o terminal ativou. Por exemplo, se você remover um terminal SOAP de um serviço, não será possível chamar o serviço usando o modo SOAP.
...
1. Recupere o terminal a ser removido.
   ...
   * Recupere uma lista de todos os pontos de extremidade aos quais o usuário atual (especificado nas propriedades de conexão) tem acesso, chamando o método do `EndpointRegistryClient` objeto `getEndpoints` e transmitindo um `PagingFilter` objeto que atua como filtro. Você pode passar `(PagingFilter)null` para retornar todos os pontos finais. Esse método retorna um `java.util.List` objeto no qual cada elemento é um `Endpoint` objeto.
   * Iterar pelo `java.util.List` objeto para determinar se ele tem pontos de extremidade. Se houver pontos de extremidade, cada elemento será uma `EndPoint` instância.
   * Determine o serviço que corresponde a um ponto final chamando o método do `EndPoint` objeto `getServiceId` . Esse método retorna um valor de string que especifica o nome do serviço.
...
1. Recuperar valores de configuração.
   ...
   * Recupere os valores de configuração associados a esse ponto de extremidade, chamando o método do `Endpoint` objeto `getConfigParameters` . Esse método retorna uma matriz de `ConfigParameter` objetos.
