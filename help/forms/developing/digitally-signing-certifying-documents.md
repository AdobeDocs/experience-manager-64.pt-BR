...
O serviço de assinatura permite que sua organização proteja a segurança e a privacidade de documentos Adobe PDF que ele distribui e recebe. Esse serviço usa assinaturas digitais e certificação para garantir que somente os destinatários pretendidos possam alterar documentos. Como os recursos de segurança são aplicados ao próprio documento, ele permanece protegido e controlado por todo o seu ciclo de vida. Um documento permanece protegido além do firewall, quando é baixado offline e quando é enviado de volta à sua organização.
...
* Assine digitalmente documentos PDF. (Consulte Assinatura [digital de documentos](digitally-signing-certifying-documents.md#digitally-signing-pdf-documents)PDF.)
* Certifique documentos PDF. (Consulte [Certificando documentos](digitally-signing-certifying-documents.md#certifying-pdf-documents)PDF.)
* Valide as assinaturas digitais localizadas em um documento PDF. (Consulte [Verificação de assinaturas](#unresolvedlink-lc-si)digitais.)
* Valide todas as assinaturas digitais localizadas em um documento PDF. (Consulte [Verificando várias assinaturas](#unresolvedlink-lc-si)digitais.)
* Remova uma assinatura digital de um campo de assinatura. (Consulte [Remoção de assinaturas](digitally-signing-certifying-documents.md#removing-digital-signatures)digitais.)
   ...
   ***Observação **: Para obter mais informações sobre o serviço de assinatura, consulte Referência de[serviços para formulários](https://www.adobe.com/go/learn_aemforms_services_63)AEM.*
...
1. Obtenha um documento PDF ao qual um campo de assinatura seja adicionado.
...
[Início rápido da API de serviço de assinatura](/help/forms/developing/signature-service-java-api-quick.md#signature-service-java-api-quick-start-soap)
...
1. Salvar o documento PDF como um arquivo PDF
   ...
   * Crie uma matriz de bytes que armazene o conteúdo do `BLOB` objeto retornado pelo `addSignatureField` método. Preencha a matriz de bytes obtendo o valor do membro de `BLOB` dados do `binaryData` objeto.
...
[Início rápido (modo SOAP): Recuperar nomes de campos de assinatura usando a API Java](/help/forms/developing/signature-service-java-api-quick.md#quick-start-soap-mode-retrieving-signature-field-names-using-the-java-api)
...
Um dicionário *de bloqueio de* campo especifica uma lista de campos que são bloqueados quando o campo de assinatura é assinado. Um campo bloqueado impede que os usuários façam alterações no campo. Um dicionário *de valor* semente contém informações de restrição usadas no momento em que a assinatura é aplicada. Por exemplo, você pode alterar permissões que controlam as ações que podem ocorrer sem invalidar uma assinatura.

Ao modificar um campo de assinatura existente, você pode fazer alterações no documento PDF para refletir as necessidades comerciais em mudança. Por exemplo, um novo requisito comercial pode exigir o bloqueio de todos os campos do documento depois que o documento é assinado.
...
Para modificar um campo de assinatura, atribua valores ao dicionário de bloqueios de campos ou ao dicionário de valores semente. A especificação de valores de dicionário de bloqueio de campo de assinatura envolve a especificação de campos de documento PDF que estão bloqueados quando o campo de assinatura é assinado. (Esta seção discute como bloquear todos os campos.)
...
* **Carimbo** de data/hora: Especifica opções de carimbo de data e hora. Por exemplo, você pode definir o URL do servidor de carimbo de data e hora usado.
...
1. Definir valores do dicionário
   ...
   * Não permita alterações no documento PDF invocando o `PDFSeedValueOptionSpec` método do objeto `setMdpValue` e transmitindo o valor de `MDPPermissions.NoChanges` enumeração.
   ...
   * Bloqueie todos os campos no documento PDF invocando o `FieldMDPOptionSpec` método do objeto `setMdpValue` e transmitindo o valor de `FieldMDPAction.ALL` enumeração.
...
1. Definir valores do dicionário
   ...
   * Crie um `PDFSeedValueOptionSpec` objeto usando seu construtor. Esse objeto permite que você defina valores de dicionário de valor semente.
   * Não permita alterações no documento PDF atribuindo o valor de `MDPPermissions.NoChanges` enumeração ao membro de `PDFSeedValueOptionSpec` dados do objeto `mdpValue` .
   * Crie um `FieldMDPOptionSpec` objeto usando seu construtor. Esse objeto permite definir valores de dicionário de bloqueio de campo de assinatura.
   * Bloqueie todos os campos no documento PDF atribuindo o valor de `FieldMDPAction.ALL` enumeração ao membro de `FieldMDPOptionSpec` dados do objeto `mdpValue` .
...
Assinaturas digitais podem ser aplicadas a documentos PDF para fornecer um nível de segurança. As assinaturas digitais, como assinaturas manuscritas, fornecem um meio pelo qual os signatários se identificam e fazem declarações sobre um documento. A tecnologia usada para assinar documentos digitalmente ajuda a garantir que o assinante e os destinatários sejam claros sobre o que foi assinado e confiem que o documento não foi alterado desde que foi assinado.

Os documentos PDF são assinados por meio de tecnologia de chave pública. Um assinante tem duas chaves: uma chave pública e uma chave privada. A chave privada é armazenada em uma credencial do usuário que deve estar disponível no momento da assinatura. A chave pública é armazenada no certificado do usuário que deve estar disponível para os destinatários para validar a assinatura. As informações sobre certificados revogados estão disponíveis nas CRLs (certificate Revolutions lists) e nas respostas OCSP (Online Certificate Status Protocol, Protocolo de Status de Certificado Online) distribuídas pelas ACs (Autoridades de Certificação). O tempo de assinatura pode ser obtido de uma fonte confiável conhecida como uma autoridade de carimbo de data e hora.
...
Para obter informações sobre como lidar com as credenciais de segurança, consulte o guia* Installing and Deploying AEM Forms* para seu servidor de aplicativos.
...
Você pode definir a ordem do servidor CRL e OCSP que o serviço de assinatura usa com aplicativos e serviços da Adobe. Por exemplo, se o servidor OCSP for definido primeiro em Aplicativos e Serviços da Adobe, então o servidor OCSP será marcado, seguido pelo servidor CRL. (Consulte &quot;Gerenciamento de certificados e credenciais usando o Armazenamento de confiança&quot; na Ajuda do AAC).
...
>[!NOTE]
>
>No Java e no serviço da Web, percorra as seções e as inicializações rápidas correspondentes, a verificação de revogação é usada. Como nenhuma informação CRL ou OCSP do servidor é especificada, as informações do servidor são obtidas do certificado usado para assinar digitalmente o documento PDF.
...
### Assinar documentos PDF digitalmente usando a API Java {#digitally-sign-pdf-documents-using-the-java-api}
...
1. Assinar o documento PDF
   ...
   * Um `CRLPreferences` objeto que armazena as preferências da lista de revogação de certificados (CRL). Se a verificação de revogação não for feita, esse parâmetro não será usado e você poderá especificar `null`.
...
1. Salvar o documento PDF assinado
   ...
   * Crie uma matriz de bytes que armazene o conteúdo do `BLOB` objeto retornado pelo `sign` método. Preencha a matriz de bytes obtendo o valor do membro de `BLOB` dados do `MTOM` objeto.
   * Crie um `System.IO.BinaryWriter` objeto chamando seu construtor e transmitindo o `System.IO.FileStream` objeto.
   * Grave o conteúdo da matriz de bytes em um arquivo PDF chamando o método do `System.IO.BinaryWriter` objeto `Write` e transmitindo a matriz de bytes.
...
* É possível unir um formulário PDF com base em XFA criado usando o Designer e dados de formulário localizados em um documento XML usando o serviço de Formulários. O servidor Forms renderiza um formulário interativo.
* Você assina o formulário interativo usando a API de serviço de assinatura.

O resultado é um formulário PDF interativo assinado digitalmente. Ao assinar um formulário PDF baseado em um formulário XFA, salve o arquivo PDF como um formulário PDF estático da Adobe. Se você tentar assinar um formulário PDF salvo como um formulário Adobe Dynamic PDF, ocorrerá uma exceção. Como você está assinando o formulário que é retornado pelo serviço de Formulários, verifique se o formulário contém um campo de assinatura.

>[!NOTE]
>
>Antes de poder assinar digitalmente um formulário interativo, é necessário adicionar o certificado ao AEM Forms. Um certificado é adicionado usando o console de administração ou de forma programática usando a API do Trust Manager. (Consulte [Importação de credenciais usando a API](/help/forms/developing/credentials.md#importing-credentials-by-using-the-trust-manager-api)do Gerenciador de confiança.)

Ao usar a API do serviço de formulários, defina a opção de tempo de `GenerateServerAppearance` execução como `true`. Essa opção de tempo de execução garante que a aparência do formulário gerado no servidor permaneça válida quando aberto no Acrobat ou Adobe Reader. É recomendável definir essa opção de tempo de execução ao gerar um formulário interativo para assinar usando a API do Forms.

>[!NOTE]
>
>Antes de ler Digitalmente Signing Interative Forms, recomenda-se que você esteja familiarizado com a assinatura de documentos PDF. (Consulte Assinatura [digital de documentos](digitally-signing-certifying-documents.md#digitally-signing-pdf-documents)PDF.)
...
>[!NOTE]
>
>A seção Início rápido associado à assinatura digital de formulários interativos chama o `renderPDFForm2` método.
...
Depois que o serviço de assinatura assinar digitalmente o documento PDF, é possível salvá-lo como um arquivo PDF. O arquivo PDF pode ser aberto no Acrobat ou no Adobe Reader.
...
1. Obter o formulário interativo usando o serviço Forms
   ...
   * Chame o método do `FormsServiceClient` objeto `renderPDFForm2` e passe os seguintes valores:
      ...
      * Um `com.adobe.idp.Document` objeto que contém dados a serem unidos ao formulário.
      * Um `PDFFormRenderSpec` objeto que armazena opções de tempo de execução.
      * Um `URLSpec` objeto que contém valores de URI exigidos pelo serviço Forms. Você pode especificar `null` para esse valor de parâmetro.
...
1. Obter o formulário interativo usando o serviço Forms
   ...
   * Chame o método do `FormsServiceClient` objeto `renderPDFForm2` e passe os seguintes valores:
      ...
      * Um `BLOB` objeto que contém dados a serem unidos ao formulário.
      * Um `PDFFormRenderSpec` objeto que armazena opções de tempo de execução.
      * Um `URLSpec` objeto que contém valores de URI exigidos pelo serviço Forms. Você pode especificar `null` para esse valor de parâmetro.
      * Um `java.util.HashMap` objeto que armazena anexos de arquivo. Esse é um parâmetro opcional e você pode especificar `null` se não deseja anexar arquivos ao formulário.
...
>[!NOTE]
>
>Ao certificar e assinar o mesmo documento PDF, se a assinatura certificada não for confiável, um triângulo amarelo será exibido ao lado da primeira assinatura ao abrir o documento PDF no Acrobat ou Adobe Reader. A assinatura de certificação deve ser confiável para evitar essa situação.
...
* **Configurações** OCSP: Configurações para suporte ao protocolo OCSP (Online Certificate Status Protocol), que fornece informações sobre o status da credencial usada para certificar o documento PDF. Por exemplo, você pode especificar o URL do servidor que fornece informações sobre a credencial que você está usando para fazer logon no documento PDF.
* **Configurações** de CRL: Configurações das preferências da lista de revogação de certificado (CRL) se a verificação de revogação estiver concluída. Por exemplo, você pode especificar para sempre verificar se uma credencial foi revogada.
...
1. Certificar o documento PDF
   ...
   * Um `OCSPPreferences` objeto que armazena preferências para suporte ao Protocolo de status de certificado online (OCSP), que fornece informações sobre o status da credencial usada para certificar o documento PDF. Se a verificação de revogação não for feita, esse parâmetro não será usado e você poderá especificar `null`.
   * Um `CRLPreferences` objeto que armazena as preferências da lista de revogação de certificados (CRL). Se a verificação de revogação não for feita, esse parâmetro não será usado e você poderá especificar `null`.
...
1. Salvar o documento PDF certificado como um arquivo PDF
   ...
   * Crie uma matriz de bytes que armazene o conteúdo do `BLOB` objeto retornado pelo `certify` método. Preencha a matriz de bytes obtendo o valor do membro de `BLOB` dados do `binaryData` objeto.
   * Crie um `System.IO.BinaryWriter` objeto chamando seu construtor e transmitindo o `System.IO.FileStream` objeto.
   * Grave o conteúdo da matriz de bytes em um arquivo PDF chamando o método do `System.IO.BinaryWriter` objeto `Write` e transmitindo a matriz de bytes.
...
Suponha que a identidade do assinante seja desconhecida. Quando você abre o documento PDF no Acrobat, uma mensagem de aviso informa que a identidade do assinante é desconhecida, como mostra a ilustração a seguir.
...
Para verificar uma assinatura usada para assinar ou certificar digitalmente um documento PDF, obtenha um documento PDF que contenha uma assinatura.
...
Como parte da configuração dessas opções, você pode especificar o tempo de verificação. Por exemplo, você pode selecionar a hora atual (a hora no computador do validador), que indica o uso da hora atual. Para obter informações sobre os diferentes valores de hora, consulte o valor de `VerificationTime` enumeração na Referência [da API do](https://www.adobe.com/go/learn_aemforms_javadocs_63_en)AEM Forms.

Também é possível especificar se a verificação de revogação deve ser executada como parte do processo de verificação. Por exemplo, você pode executar uma verificação de revogação para determinar se o certificado foi revogado. Para obter informações sobre as opções de verificação de revogação, consulte o valor de `RevocationCheckStyle` enumeração na Referência [da API do](https://www.adobe.com/go/learn_aemforms_javadocs_63_en)AEM Forms.

Para executar a verificação de revogação em um certificado, especifique um URL para um servidor de lista de revogação de certificado (CRL) usando um `CRLOptionSpec` objeto. Entretanto, se você não especificar um URL para o servidor CRL, o serviço de assinatura obterá o URL do certificado.
...
Você pode definir a ordem do servidor CRL e OCSP que o serviço de assinatura usa usando Aplicativos e Serviços da Adobe. Por exemplo, se o servidor OCSP for definido primeiro em Aplicativos e Serviços da Adobe, então o servidor OCSP será marcado, seguido pelo servidor CRL.
...
[Verificar assinaturas digitais usando a API Java](#unresolvedlink-lc-si)

[Verificar assinaturas digitais usando a API de serviço da Web](#unresolvedlink-lc-si)
...
1. [Criar um cliente de assinatura](#unresolvedlink-lc-si)
...
1. Definir opções de tempo de execução de PKI
   ...
   * Defina o tempo de verificação chamando o `PKIOptions` método do `setVerificationTime` objeto e transmitindo um valor de `VerificationTime` enumeração que especifica o tempo de verificação.
   * Defina a opção de verificação de revogação chamando o método do `PKIOptions` objeto `setRevocationCheckStyle` e transmitindo um valor de `RevocationCheckStyle` enumeração que especifica se a verificação de revogação deve ser executada.
...
1. Determinar a identidade do assinante
   ...
   * Chame o método do `IdentityInformation` objeto `getStatus` para determinar a identidade do assinante. Esse método retorna um valor de `IdentityStatus` enumeração que especifica a identidade. Por exemplo, se o assinante for confiável, esse método retornará `IdentityStatus.TRUSTED`.

**Consulte também:**

[Verificação de assinaturas digitais](#unresolvedlink-lc-si)
...
1. Definir opções de tempo de execução de PKI
   ...
   * Crie um `PKIOptions` objeto usando seu construtor.
   * Defina o tempo de verificação atribuindo ao membro de `PKIOptions` dados do `verificationTime` objeto um valor de `VerificationTime` enumeração que especifica o tempo de verificação.
   * Defina a opção de verificação de revogação atribuindo ao membro de `PKIOptions` dados do `revocationCheckStyle` objeto um valor de `RevocationCheckStyle` enumeração que especifica se a verificação de revogação deve ser executada.
...
1. Determinar a identidade do assinante
   ...
   * Recupere o membro de `IdentityInformation` dados do `status` objeto para determinar a identidade do assinante. Esse membro de dados retorna um valor de `IdentityStatus` enumeração que especifica a identidade. Por exemplo, se o assinante for confiável, esse membro retornará `IdentityStatus.TRUSTED`.
...
Para executar a verificação de revogação em um certificado, especifique um URL para um servidor de lista de revogação de certificado (CRL) usando um `CRLOptionSpec` objeto. Entretanto, se você não especificar um URL para um servidor CRL, o serviço de assinatura obterá o URL do certificado.
...
Iterar por cada assinatura. Ou seja, para cada assinatura, verifique a assinatura digital e verifique a identidade do assinante e o status de cada assinatura. (Consulte [Verificação de assinaturas](#unresolvedlink-lc-si)digitais.)
...
[Verificar várias assinaturas digitais usando a API Java](#unresolvedlink-lc-si)

[Verificação de várias assinaturas digitais usando a API de serviço da Web](#unresolvedlink-lc-si)
...
1. Obtenha o documento PDF que contém as assinaturas para verificar
   ...
   * Crie um `java.io.FileInputStream` objeto que represente o documento PDF que contém várias assinaturas digitais para verificação usando seu construtor. Passe um valor de string que especifica o local do documento PDF.
   * Crie um `com.adobe.idp.Document` objeto usando seu construtor e transmitindo o `java.io.FileInputStream` objeto.
...
1. Definir opções de tempo de execução de PKI
   ...
   * Crie um `PKIOptions` objeto usando seu construtor.
   * Defina o tempo de verificação chamando o `PKIOptions` método do `setVerificationTime` objeto e transmitindo um valor de `VerificationTime` enumeração que especifica o tempo de verificação.
   * Defina a opção de verificação de revogação chamando o método `PKIOptions` do objeto `setRevocationCheckStyle` e transmitindo um valor de `RevocationCheckStyle` enumeração que especifica se a verificação de revogação deve ser executada.
...
1. Iterar por todas as assinaturas
   ...
   * Iterar por todas as assinaturas chamando o `PDFDocumentVerificationInfo` método do `getVerificationInfos` objeto. Esse método retorna um `java.util.List` objeto no qual cada elemento é um `PDFSignatureVerificationInfo` objeto. Use um `java.util.Iterator` objeto para percorrer a lista de assinaturas.
   * Usando o `PDFSignatureVerificationInfo` objeto, é possível executar tarefas como determinar o status da assinatura chamando o `PDFSignatureVerificationInfo` `getStatus` método do objeto. Este método retorna um `SignatureStatus` objeto cujo membro de dados estáticos informa sobre o status da assinatura. Por exemplo, se a assinatura for desconhecida, esse método retornará `SignatureStatus.DocumentSignatureUnknown`.

**Consulte também:**

[Verificando várias assinaturas digitais](#unresolvedlink-lc-si)
...
Depois que o serviço de assinatura remover uma assinatura digital de um campo de assinatura, você poderá salvar o documento PDF como um arquivo PDF para que os usuários possam abri-lo no Acrobat ou Adobe Reader.
...
1. Salvar o documento PDF como um arquivo PDF
   ...
   * Crie um `java.io.File` objeto e verifique se a extensão do arquivo é .pdf.
   * Chame o `com.adobe.idp.Document` método do `copyToFile` objeto. Passe o `java.io.File` objeto para copiar o conteúdo do `com.adobe.idp.Document` objeto para o arquivo. Certifique-se de usar o `Document` objeto retornado pelo `clearSignatureField` método.
