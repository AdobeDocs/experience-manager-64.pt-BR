...
Retorna um armazenamento de sessão com um nome especificado. Consulte também [Acesso a um Armazenamento de Sessão](/help/sites-developing/client-context.md#accessing-session-stores).
...
Um objeto CQ_Analytics.SessionStore que representa o armazenamento de sessão do nome fornecido. Retorna `null` quando não existe nenhum armazenamento do nome especificado.
...
#### onStoreInitialized(storeName, callback, atraso) {#onstoreinitialized-storename-callback-delay}
...
* atraso: Booleano ou número. O tempo para atrasar a chamada para a função de retorno de chamada, em milissegundos. Um valor booleano de `true` usa o atraso padrão de `200 ms`. Um valor booleano `false` ou um número negativo não faz com que nenhum atraso seja usado.
...
Registra uma função de retorno de chamada que é chamada quando um armazenamento de sessão é registrado. O evento de registro ocorre quando uma loja é registrada em [CQ_Analytics.ClientContextMgr](#cq-analytics-clientcontextmgr).
...
Apaga o armazenamento de sessão e o inicializa com a propriedade de inicialização. Define o sinalizador de inicialização como `true` e dispara os eventos `initialize` e `update`.
...
Registra um ouvinte para um evento. Consulte também [Criação de um ouvinte para reagir a uma atualização do armazenamento de sessão](/help/sites-developing/client-context.md#creating-a-listener-to-react-to-a-session-store-update).
...
Um valor booleano `true` se a propriedade for persistente e um valor `false` se o valor não for uma propriedade persistente.

#### persist() {#persist}

Persiste no armazenamento da sessão. O modo de persistência padrão usa o navegador `localStorage` usando `ClientSidePersistence` como o nome ( `window.localStorage.set("ClientSidePersistance", store);`)
...
Remove todas as propriedades de dados do armazenamento e mantém o armazenamento. Opcionalmente, não dispara o evento `udpate` após a conclusão.

**Parâmetros**

* deferEvent: Um valor true impede que o evento `update` seja disparado. Um valor `false` faz com que o evento de atualização seja disparado.
...
Retorna os dados de armazenamento. Opcionalmente, exclui propriedades de nomes dos dados. Chama o método `init` se a propriedade de dados do armazenamento não existir.
...
O valor da propriedade de dados. Retorna `null` se o armazenamento da sessão não contiver nenhuma propriedade do nome fornecido.
...
Retorna o valor de uma propriedade. O valor é retornado como a propriedade bruta ou o valor filtrado por XSS. Chama o método `init` se a propriedade de dados do armazenamento não existir.
...
Retorna os nomes das propriedades que o armazenamento de sessão contém. Chama o método `init` se a propriedade de dados do armazenamento não existir.
...
Um valor `true` se o armazenamento for inicializado e um valor `false` se o armazenamento não for inicializado.
...
Remove uma propriedade do armazenamento de sessão. Aciona o evento `update` após a conclusão. Chama o método `init` se a propriedade de dados do armazenamento não existir.
...
Define os valores de várias propriedades. Aciona o evento `update` após a conclusão. Chama o método `init` se a propriedade de dados do armazenamento não existir.
...
Define o valor de uma propriedade. Aciona o evento `update` após a conclusão. Chama o método `init` se a propriedade de dados do armazenamento não existir.
