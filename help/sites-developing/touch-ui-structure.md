---
title: Estrutura da interface habilitada para toque do AEM
seo-title: Estrutura da interface habilitada para toque do AEM
...
seo-description: A interface otimizada para toque, conforme implementada no AEM, tem vários princípios subjacentes e é composta de vários elementos chave
...
# Estrutura da interface habilitada para toque do AEM{#structure-of-the-aem-touch-enabled-ui}

A interface do usuário habilitada para toque do AEM tem vários princípios subjacentes e é composta de vários elementos chave:

## Consoles {#consoles}

### Layout básico e redimensionamento {#basic-layout-and-resizing}

A interface do usuário atende a dispositivos móveis e de desktop, mas, em vez de criar dois estilos, a Adobe decidiu usar um estilo que funciona para todas as telas e dispositivos.
...
* o logotipo e o produto/solução específicos que você está usando no momento; para o AEM, isso também forma um link para a Navegação global
* Pesquisar 
...
* o ícone do usuário, juntamente com um link para o gerenciamento de perfil

### Barra de ferramentas {#toolbar}

Isso é contextual para as ferramentas de localização e superfície relevantes para controlar a exibição ou os ativos na página abaixo. A barra de ferramentas é específica do produto, mas há alguma compatibilidade com os elementos.
...
O quadro do editor é um contêiner (abstrato) para todos os elementos *de criação de* página. Ele fica sobre o quadro de conteúdo e inclui:
...
Ele contém duas guias padrão para permitir que você selecione ativos e componentes; eles podem ser arrastados daqui e soltos na página.
...
>[!NOTE]
>
>As camadas são um conceito poderoso que afeta a exibição do usuário e a interação com o conteúdo da página. Ao desenvolver suas próprias camadas, é necessário garantir que a camada seja limpa quando ela for fechada.
...
Cada instância de um componente revelará sua barra de ferramentas quando clicada (uma vez ou com um clique duplo lento). A barra de ferramentas contém as ações específicas (por exemplo, copiar, colar, editor aberto) que estão disponíveis para a instância do componente (editável) na página.
...
Para obter mais detalhes sobre os conceitos em torno da interface habilitada para toque, continue com o artigo [Conceitos da interface do usuário](/help/sites-developing/touch-ui-concepts.md)habilitada para toque do AEM.
