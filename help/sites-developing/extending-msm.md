...
* **`LiveRelationship`** O `LiveRelationship` especifica a conexão (relação) entre um recurso na ramificação da live copy e seu recurso equivalente de origem/blueprint.
...
1. Repita a etapa anterior para criar a variável `/apps/wcm/core` árvore de pastas. Criar um nó do tipo `sling:Folder` no núcleo chamado `resources`. <!-- ![chlimage_1-39](assets/chlimage_1-39.png) -->
