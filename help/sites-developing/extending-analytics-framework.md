...
# Personalização do Adobe Analytics Framework{#customizing-the-adobe-analytics-framework}
...
1. Na coluna da direita, digite um valor para a variável, por exemplo `CONSTANT`.
...
### Adicionar javascript personalizado a um Adobe Analytics Framework {#adding-custom-javascript-to-an-adobe-analytics-framework}
...
### Adicionando código personalizado global para todas as estruturas Adobe Analytics {#adding-global-custom-code-for-all-adobe-analytics-frameworks}
...
Portanto, seu javascript deve definir `s.usePlugins` como `true` para que qualquer código na função `s_doPlugins` seja executado. Para personalizar o código, sobreponha o arquivo config.js.jsp com um que use seu próprio javascript. Se o javascript precisar usar plug-ins do Adobe Analytics, [integre-os](/help/sites-administering/adobeanalytics.md) no AEM.
...
1. Clique com o botão direito do mouse na pasta `components` que você acabou de criar e clique em Criar > Criar componente. Especifique os seguintes valores de propriedade:
...
1. Adicione o código javascript que você deseja executar na função `s_doPlugins` e clique em Salvar tudo.
...
### Usando plug-ins do Adobe Analytics em AEM {#using-adobe-analytics-plugins-in-aem}
