...
Para habilitar um modelo existente para seu site e desejar que ele seja exibido na caixa de diálogo **Criar página** ao criar uma página sob **Sites** a partir do console **Sites**, defina a propriedade allowPaths do nó modelo como: **/content(/.&amp;ast;)?**
...
1. Copie um modelo existente (de preferência, com uma definição o mais próxima possível do que você deseja obter) para um novo nó.
   ...
   >[!NOTE]
   >
   >A lista dos modelos disponíveis depende do local da nova página e das restrições de posicionamento especificadas em cada modelo. Consulte [Disponibilidade do modelo](/help/sites-developing/templates.md#template-availability).
