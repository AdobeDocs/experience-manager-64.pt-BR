...
Aplicativos de página única (SPA) podem oferta ao usuário de experiências ricas e dinâmicas que reagem e se comportam de maneiras familiares, normalmente como aplicativos nativos. [Isso é feito contando com o cliente para carregar o conteúdo antecipadamente e, em seguida, fazer um pesado levantamento da manipulação da ](/help/sites-developing/spa-walkthrough.md#how-does-a-spa-work) interação do usuário, minimizando assim a quantidade de comunicação necessária entre o cliente e o servidor, tornando o aplicativo mais reativo.
...
* [Fluxo de comunicação direcionado ao tempo de execução da Adobe I/O](#adobe-io-driven-communication-flow)
...
AEM deve saber onde o conteúdo renderizado remotamente pode ser recuperado. Independentemente de [qual modelo você escolher implementar para SSR](#adobe-io-runtime), será necessário especificar para AEM como acessar esse serviço de renderização remota.

Isso é feito por meio do serviço **RemoteContentRenderer - Configuration Fatory** OSGi. Procure a string &quot;RemoteContentRenderer&quot; no console Configuração do console da Web em `http://<host>:<port>/system/console/configMgr`.
...
Ao usar o SSR, o [fluxo de trabalho de interação do componente](/help/sites-developing/spa-overview.md#workflow) do SPA no AEM inclui uma fase na qual o conteúdo inicial do aplicativo é gerado pela Adobe I/O Runtime.
...
A seção [Fluxo de comunicação orientado por AEM](#aem-driven-communication-flow) descreve a implementação padrão e recomendada da renderização do lado do servidor em relação ao SPA no AEM, onde AEM executa o carregamento de inicialização e a disponibilização de conteúdo.
...
|---|---|---|
| via AEM | AEM gerencia bibliotecas injetáveis onde necessário<br>Os recursos só precisam ser mantidos no AEM | Possivelmente estranho ao desenvolvedor SPA |
| via Adobe I/O Runtime | Mais familiar para desenvolvedores SPA | Os recursos clientlib exigidos pelo aplicativo, como CSS e JavaScript, precisarão ser disponibilizados pelo desenvolvedor AEM por meio da propriedade [`allowProxy`](/help/sites-developing/clientlibs.md#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet)<br>Os recursos devem ser sincronizados entre AEM e Adobe I/O Runtime<br>Para habilitar a criação do SPA, pode ser necessário um servidor proxy para Adobe I/O Runtime |
...
Ao considerar implementar a renderização do lado do servidor para o seu SPA, é necessário revisar quais partes do aplicativo ele exigirá SSR.
