...
>[!NOTE]
>
>Ative *Registro de depuração ContentSync* através das configurações de registro OSGI no pacote *com.day.cq.contentsync*. Isso permite rastrear quais manipuladores foram executados e se eles atualizaram o cache e reportaram a atualização do cache.
...
Se o nó `cq:ContentSyncConfig` não tiver nenhuma propriedade `updateuser`, o usuário padrão `anonymous` atualizará o cache.
...
**conteúdoRenderize conteúdo usando o processamento**  de solicitação  [ ](/help/sites-developing/the-basics.md#sling-request-processing)Sling padrão.
