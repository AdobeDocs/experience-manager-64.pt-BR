...
description: Práticas recomendadas para monitorar o ambiente e o desempenho de seu [!DNL Experience Manager] após a implantação.
contentOwner: AG
...
Do ponto de vista do Adobe Experience Manager Assets, o monitoramento deve incluir a observação e os relatórios sobre os seguintes processos e tecnologias:
...
Normalmente, [!DNL Assets] pode ser monitorizado de duas formas, monitoramento ao vivo e monitoramento a longo prazo.
...
* [VM visual](https://visualvm.github.io/): A VM visual permite que você visualize informações detalhadas da VM Java, incluindo uso da CPU, uso da memória Java. Além disso, ele permite exemplificar e avaliar o código que é executado em uma instância.
...
* [!DNL Experience Manager] arquivo error.log: Você pode investigar o [!DNL Experience Manager] error.log para obter detalhes de erros registrados no sistema. Use o comando `tail -F quickstart/logs/error.log` para identificar erros que você deve investigar.
* [Console de fluxo de trabalho](../sites-administering/workflows.md): Aproveite o console do workflow para monitorar os workflows que ficam atrasados ou travados.

Normalmente, você usa essas ferramentas em conjunto para obter uma ideia abrangente sobre o desempenho de sua [!DNL Experience Manager] instância.
...
Monitorização a longo prazo de um [!DNL Experience Manager] A instância envolve o monitoramento por um período mais longo das mesmas partes que são monitoradas ao vivo. Também inclui definir alertas específicos para o seu ambiente.

### Agregação de logs e relatórios {#log-aggregation-and-reporting}

Há várias ferramentas disponíveis para agregar logs, por exemplo Splunk(TM) e Elastic Search/Logstash/Kabana (ELK). Para avaliar o tempo de atividade de seu [!DNL Experience Manager] , é importante entender os eventos de log específicos do seu sistema e criar alertas com base neles. Um bom conhecimento das práticas de desenvolvimento e operações pode ajudá-lo a entender melhor como ajustar o processo de agregação de log para gerar alertas críticos.
...
**[!DNL Experience Manager]monitoramento**
...
* Limiar de alarme: Quando o valor de `QueueBlocked` é verdadeiro ou o valor de `QueueNumEntries` for superior a 150% da linha de base.
...
**Observação**: Para os parâmetros MBean e URL, substitua `<AGENT_NAME>` com o nome do agente de replicação que você deseja monitorar.
...
No processo de monitoramento, se você encontrar problemas, veja algumas tarefas de solução de problemas que você pode executar para resolver problemas comuns com o [!DNL Experience Manager] instâncias:
...
* Verificar `OutOfMemoryError` logs. Para obter mais informações, consulte [Analisar problemas de memória](https://helpx.adobe.com/experience-manager/kb/AnalyzeMemoryProblems.html).
...
* Investigue os pontos de saída da rede do cliente e a entrada aponta para o [!DNL Experience Manager] rede de instâncias, incluindo o dispatcher. Frequentemente, essas são áreas de gargalo. Para obter mais informações, consulte [Considerações sobre a rede de ativos](assets-network-considerations.md).
* Atualize seu [!DNL Experience Manager] servidor. Pode ter um tamanho inadequado [!DNL Experience Manager] instância. O Suporte ao cliente do Adobe pode ajudar você a identificar se o servidor está com menos tamanho.
