...
description: Discute considerações de rede ao projetar uma implantação da AEM Assets.
contentOwner: AG
...
No uplink de uma rede corporativa para a Internet, pode haver outros serviços usando a largura de banda. É importante entender quanto da largura de banda pode ser dedicada ou priorizada para a AEM Assets. Por exemplo, se um link de 1 Gbps já estiver com 80% de utilização, você só poderá alocar um máximo de 20% da largura de banda para ativos AEM.
...
Este é o menor ponto de estrangulamento voltado para o cliente. No entanto, você pode avaliar se há uma alteração ou lista de permissões com o grupo de operações de rede responsável por esse firewall.
...
A criação da topologia do ambiente AEM requer conhecimento detalhado da configuração do sistema e de como a rede é conectada dentro do ambiente do usuário.

O cenário de amostra inclui um farm de publicação com cinco servidores, uma loja binária S3 e mídia dinâmica configurada.

O dispatcher compartilha sua conexão de 100 Mbps com duas entidades, o mundo externo e a instância AEM. Para operações simultâneas de upload e download, você deve dividir esse número por dois. O armazenamento externo conectado usa uma conexão separada.
