...
* Certifique-se de que o agente de replicação **[!UICONTROL Dynamic Media Asset Ativation (scene7)]** esteja ativado.

   Esse agente de replicação é encontrado em **[!UICONTROL Agents]** no Author.
...
Erros e problemas de sincronização são registrados em `error.log` (AEM diretório do servidor `/crx-quickstart/logs/`). O registro em log é suficiente para determinar a causa raiz da maioria dos problemas, no entanto, é possível aumentar o registro em DEBUG no pacote `com.adobe.cq.dam.ips` por meio do Console do Sling ([http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog)) para coletar mais informações.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     <li>Verifique se o modo de execução é <span class="kbd">dynamicmedia_scene7</span>.</li> 
   ...
   <td> 
    <ol> 
     <li>Verifique a instância do AEM com <span class="kbd">-r dynamicmedia_scene7</span></li> 
  ...
  <tr> 
   ...
   <td><p>Para determinar se a codificação de vídeo ainda está em andamento ou se entrou em um estado de falha:</p> 
    <ul> 
     ...
     <li>Monitore o vídeo no console <code>http://localhost:4502/libs/cq/workflow/content/console.html</code> do workflow, nas guias Instâncias, Arquivamento e Falhas.</li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>Vá para a página de diagnóstico do gerenciador de exemplo: <code>http://localhost:4502/libs/dam/gui/content/s7dam/samplemanager/samplemanager.html</code></p> <p>Observe valores calculados. Ao operar corretamente, você deve ver:</p> <p><code class="code">_DMSAMPLE status: 0 unsyced assets - activation not necessary
  ...
  <tr> 
   ...
   <td><p>Se os ativos de amostra ou a arte-final predefinida do visualizador não tiverem sido sincronizados ou publicados, reinicie todo o processo de cópia/sincronização:</p> 
    <ol> 
     ...
     <li>Navegue até o gerenciador de pacotes do CRX: <code>http://localhost:4502/crx/packmgr/</code><a href="http://localhost:4502/crx/packmgr/"></a> 
      <ol> 
       <li>Pesquise o pacote do visualizador na lista (ele começa com <span class="kbd">cq-dam-scene7-viewers-content</span>)</li> 
     ...
     <li>Em Cloud Services, navegue até a página Configuração do Dynamic Media e abra a caixa de diálogo de configuração da sua configuração Dynamic Media - S7. 
      <ul> 
       <li>Não faça alterações, clique em <strong>Save</strong>. Isso aciona a lógica novamente para criar e sincronizar os ativos de amostra, o CSS predefinido do visualizador e o trabalho artístico.<br /> <br /> </li> 
      </ul> </li> 
    </ol> </td> 
  </tr> 
 </tbody> 
