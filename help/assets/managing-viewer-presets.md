...
Você pode adicionar, editar, excluir, publicar, cancelar a publicação e pré-visualização predefinições do visualizador no AEM tocando em **[!UICONTROL Ferramentas > Ativos > Predefinições do visualizador]**.
...
* Android Smartphone
* Tablet Android
* Para vídeo, é fornecido suporte adicional para a reprodução MP4 para [Blackberry](https://developer.blackberry.com/devzone/develop/supported_media/bb_media_support_at_a_glance.html#kba1328730952678) e [Windows Phone 8](https://msdn.microsoft.com/library/windows/apps/ff462087%28v=vs.105%29.aspx).
...
|  | Para que uma imagem carregada seja qualificada como um panorama esférico, ela deve ter um ou ambos os seguintes: <ul><li>Uma proporção largura/altura de 2:1.</li><li>Marcado com as palavras-chave equirectangular ou esférico e panorama, ou esférico e panorâmico. Consulte [Usando tags](../sites-authoring/tags.md).</li></ul> |
...
Consulte *Notas de versão dos visualizadores* no sumário do [Guia de Referência dos Visualizadores](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/home.html).
...
| Gesto | Zoom de menu suspenso | Zoom | Rotação |
|---|---|---|---|
...
1. No canto superior esquerdo do AEM, toque no logotipo AEM e, em seguida, no painel esquerdo, toque em **[!UICONTROL Ferramentas > Ativos > Predefinições do visualizador]**.
...
1. Na caixa de diálogo **[!UICONTROL Nova predefinição do visualizador]**, no campo **[!UICONTROL Nome da predefinição]**, digite o nome da nova predefinição. Escolha um nome com cuidado. Eles não são editáveis depois que você toca em **[!UICONTROL Criar]**.

   Quando você salvar a predefinição posteriormente nessas etapas, o nome será exibido na página Predefinições do visualizador sob o cabeçalho da coluna **[!UICONTROL Título da predefinição]**.
...
1. Faça uma das seguintes opções:
   ...
   * No menu suspenso **[!UICONTROL Tipo selecionado]**, selecione um componente cujo design visual você deseja personalizar. Como alternativa, você pode tocar em qualquer elemento visual no visualizador para selecioná-lo para configuração.
      ...
      As propriedades de estilização de CSS para cada tipo de predefinição do visualizador estão descritas no tópico da Ajuda &quot;Personalizando *&lt;nome_do_visualizador>* Visualizador&quot; no [Guia de Referência do Visualizador](https://experienceleague.adobe.com/docs/dynamic-media-developer-resources/library/home.html).
...
1. (Opcional) Próximo à parte superior da página **[!UICONTROL Editar predefinição do visualizador]**, toque em **[!UICONTROL Área de trabalho]**, **[!UICONTROL Tablet]** ou **[!UICONTROL Telefone]** para definir exclusivamente estilos visuais para diferentes tipos de dispositivos e telas.
...
Ao criar ou editar uma predefinição do visualizador de vídeo interativo, você tem a opção de qual **[!UICONTROL Configuração do Modo de exibição]** usar ao selecionar `InteractiveSwatches` no menu suspenso **[!UICONTROL Componente selecionado]** na guia **[!UICONTROL Comportamento]**. O modo de exibição escolhido afeta como e quando as miniaturas aparecem enquanto o vídeo está sendo reproduzido. Você pode escolher um `segment`modo de exibição (padrão) ou um `continuous`modo de exibição.

| Modo de exibição | Descrição |
|---|---|
| [!UICONTROL Segmento] | [!UICONTROL O modo de exibição padrão para o visualizador de vídeo interativo está ] segmentado nas predefinições do visualizador de vídeo interativo Shoppable_Video_light e Shoppable_Video_black e em qualquer predefinição do visualizador de vídeo interativo que você mesmo criar. |
...
| [!UICONTROL Contínuo] | No modo de exibição [!UICONTROL Contínuo], se o número de miniaturas em um segmento for menor que o número visível no painel, o visualizador incluirá automaticamente a exibição de miniaturas do próximo segmento ou do segmento anterior, nos casos em que a última miniatura for exibida. |
...
## Ativar ou desativar predefinições do visualizador do Dynamic Media {#activating-or-deactivating-viewer-presets}
...
1. No canto superior esquerdo do AEM, toque no logotipo AEM e, em seguida, no painel esquerdo, toque em **[!UICONTROL Ferramentas > Ativos > Predefinições do visualizador]**.
1. Na página **[!UICONTROL Predefinição do visualizador]**, no cabeçalho da coluna **[!UICONTROL Estado]**, toque na alternância para ativar ou desativar uma predefinição do visualizador.
...
Ativar (ou ativar *Ativar*) o estado de uma predefinição do visualizador significa que ela está visível no componente Dynamic Media, no componente de Mídia interativa e sempre que você visualização um ativo.

Entretanto, para fornecer um ativo com uma predefinição do visualizador, a predefinição do visualizador também deve ser publicada. Todas as predefinições do visualizador devem ser ativadas *e* publicadas para obter URL ou código incorporado para um ativo. Ative e publique todas as predefinições do visualizador prontas para uso que acompanham o Dynamic Media. As predefinições do visualizador personalizado criadas e adicionadas são ativadas automaticamente, mas também devem ser publicadas.
...
**Para classificar predefinições** do visualizador do Dynamic Media:
...
1. Na página **[!UICONTROL Editar predefinição do visualizador]** faça as alterações que você deseja para a predefinição do visualizador.
1. Faça uma das seguintes opções:

   * Toque em **[!UICONTROL Salvar]** para salvar suas alterações e retornar à página **[!UICONTROL Predefinição do visualizador]**.
   * Toque em **[!UICONTROL Cancelar]** para anular quaisquer alterações efetuadas e regressar à página **[!UICONTROL Predefinição do visualizador]**.
...
1. No canto superior esquerdo do AEM, toque no logotipo AEM e, em seguida, no painel esquerdo, toque em **[!UICONTROL Ferramentas > Ativos > Predefinições do visualizador]**.
1. Na página **[!UICONTROL Predefinições do visualizador]**, verifique um **[!UICONTROL Título da predefinição]** e toque no ícone **[!UICONTROL Lixeira]**.
...
Para obter os URLs das predefinições do visualizador, consulte [Vincular URLs à sua Aplicação web](linking-urls-to-yourwebapplication.md). Consulte também [Incorporar o Visualizador de vídeo em uma página da Web](embed-code.md).

Se você estiver usando AEM como seu WCM, poderá adicionar ativos usando as predefinições do visualizador diretamente na página. Consulte [Adicionar ativos Dynamic Media às páginas](adding-dynamic-media-assets-to-pages.md).
