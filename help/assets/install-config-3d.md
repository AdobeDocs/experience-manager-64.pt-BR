...
Consulte também [AEM notas](/help/release-notes/aem3d-release-notes.md) de versão dos ativos 3D para obter pré-requisitos, navegadores compatíveis e outras importantes informações de versão.

Consulte também [Trabalhar com o componente](using-the-3d-sites-component.md)Sites 3D.

>[!NOTE]
>
>Antes de baixar e instalar o pacote 3D, verifique se você instalou todos os pacotes de pré-requisitos AEM com êxito. See the [AEM 3D Release Notes.](install-config-3d.md)
...
>[!NOTE]
>
>Antes de continuar com a instalação e configuração do AEM 3D, o Adobe recomenda que você implante todos os service packs e outros recursos relacionados aplicáveis. See [AEM 3D Release Notes](/help/release-notes/aem3d-release-notes.md).
...
1. Clique em **[!UICONTROL Instalar]** para iniciar a instalação do pacote.
   ...
   O arquivo `sample-3D-content.zip` é colocado na pasta raiz **[!UICONTROL Assets]** . Consulte [Validação da configuração AEM 3D](#validating-the-setup-of-aem-d) para obter mais informações.
...
1. Na tela Atualizar ativo **** DAM, no painel flutuante AEM, clique no ícone **[!UICONTROL Mais]** à direita de Fluxo de trabalho para expandir a lista. Selecione Etapa **[!UICONTROL do]** processo na lista.
...
1. Reinicie o AEM.
   ...
   Continue com [Validando a configuração AEM 3D](#validating-the-setup-of-aem-d).

## Validação da configuração de AEM 3D {#validating-the-setup-of-aem-d}

1. Em AEM, clique em **[!UICONTROL Ferramentas > Ativos]**, baixe `sample-3D-content.zip`e expanda o arquivo baixado. (Agora você pode excluir `sample-3D-content.zip` em AEM.)
...
1. Como Administrador, faça logon na instância do autor AEM onde deseja instalar as credenciais e abra o **[!UICONTROL CRXDE Lite]**.
...
1. Especifique o formato da representação de `.glb` conversão, fazendo o seguinte no **[!UICONTROL CRXDE Lite]**:
   ...
   1. Navegue até `/libs/settings/dam/v3D/services/dncr` o **[!UICONTROL CRXDE Lite]**.
   1. Defina a `outputFormat` propriedade como `Dn` ou `generic`.
      ...
      Quando definida como `generic`, a execução `.glb` é genérica sem extensões específicas para Adobe. Essa configuração permite que ele seja usado em aplicativos de terceiros, enquanto a visualização com o visualizador 3D AEM será visualmente insuficiente.
