...
Esta página mantém uma lista de problemas conhecidos lançados pelo Adobe Experience Manager 6.4 em abril de 2018. Para obter mais informações sobre problemas conhecidos, [entre em contato com o suporte](https://helpx.adobe.com/br/support/experience-manager.html).
...
* Verifique se as configurações são `0` e `false`.
* Reinicie o navegador.

Se você estiver usando o Microsoft Edge como navegador:

* Digite `about:flags` na barra de endereços e pressione Return.
* Role até Recursos experimentais e depois **[!UICONTROL Touch]**.
* Clique em **[!UICONTROL Ativar eventos de toque]**.
* Selecione **[!UICONTROL Sempre Desligado]**.
* Reinicie o navegador.

## Plataforma {#platform}

* **Painel de operações: a barra** de progresso não é exibida quando a extensão .zip está ausente no arquivo de backup. (GRANITE-10713)
* **HTL:objeto** Java Use com espaço em branco à direita na declaração do pacote congela o SightlyJavaCompilerService (GRANITE-20836)
* **Arquivo Apache Felix/Sling:** Config ainda presente no repositório mesmo após a configuração.delete() (GRANITE-20618)
...
* **Segurança:** Melhorar a classificação JAAS padrão de SSO, Módulos de logon externos e padrão (GRANITE-20590
* **Ferramenta - CRX DE Lite:** Cordilheira de propriedades que a visualização continua subindo (GRANITE-12040)
* **Ferramenta - CRX DE Lite:** Não é possível salvar alterações em Tipos de Valor &quot;Longo&quot;, a menos que você clique com o duplo em Nome da Propriedade (GRANITE-12351)

* **Ferramenta - A pesquisa CRX DE Lite:** ctrl+F em arquivos de texto abertos fica travada na pesquisa RegExp (GRANITE-5996)

* **Ferramenta - propriedade CRX DE Lite:** Node não exibida após renomear o nó (GRANITE-7160)
* **IU:** Pulldown &quot;more...&quot; não mostra todos os elementos quando abertos em um elemento de portátil no IE e no Firefox (GRANITE-16326)
...
* **IU: erro** sem tratamento ao representar como um usuário que não existe (GRANITE-23228). Solução alternativa da [implementação de um manipulador de erros](/help/sites-developing/customizing-errorhandler-pages.md) para personalizar a mensagem de erro.

* **Omnisearch:Exceção de** pesquisa com barra invertida (GRANITE-11769)
* **Omnisearch:** Abrir &quot;Configurações de Visualização&quot; na visualização faz com que o filtro de pesquisa mude (GRANITE-16524)
* **Omnisearch:** lista incorreta de configurações de coluna mostradas ao fazer a pesquisa de Ativos a partir de Sites (GRANITE-16527)

* **Omnisearch:predicados do painel** esquerdo estão acompanhando a solicitação do servidor Omnisearch (GRANITE-20524)
...
* (Somente AEM Forms JEE) Ao inicializar o servidor de aplicativos JBoss durante a execução do Configuration Manager, retorna erros de invocação e inicialização do EJB. Entretanto, você pode ignorá-los. (Ref# CQ-4229793)
* Quando o AEM Forms é iniciado, o aviso `SAX Security Manager could not be setup` é exibido. (CQ-4297403)
...
* As configurações de autenticação para serviços da Web baseados em SOAP não estão visíveis e, portanto, não podem ser configuradas nos serviços em nuvem. Para resolver o problema:
   ...
   1. No console CRXDE Lite, vá para o nó a seguir.\
...
* O scheduler Adobe Sign para de funcionar intermitentemente e, portanto, os formulários pendentes não são movidos para o envio. Para resolver o problema, reinicie o pacote **Suporte ao Scheduler Apache Sling** AEM console da Web em https://[*server*]:[*port*]/system/console/bundles.

### Criação adaptável de Forms
