...
AEM 3D suporta os tipos de ativos Adobe Dimension (Dn) e glTF. A implementação para esses tipos de ativos é substancialmente diferente da dos tipos 3D tradicionais descritos nesta documentação. Consulte [Trabalhar com ativos da Adobe Dimension](/help/assets/working-dimension-assets.md).
...
Consulte [Integração com o Autodesk Maya](/help/assets/integrate-maya-with-3d.md).

Consulte também [Instalar e configurar AEM 3D Assets](/help/assets/install-config-3d.md) e [Configurações avançadas](/help/assets/advanced-config-3d.md).

Consulte também [Trabalhar com ativos 3D](/help/assets/assets-3d.md).
...
Consulte [Instalar e configurar AEM 3D](/help/assets/install-config-3d.md).
...
| * | Formatos de entrada 3D adicionais suportados pelo Autodesk Maya podem ser ativados. Consulte [Ativando Formatos Adicionais Suportados por Maya](/help/assets/integrate-maya-with-3d.md#enabling-additional-formats-supported-by-maya). | Somente Windows | O Autodesk Maya é necessário no nó Autor para ativar esses formatos de arquivo. Consulte [Integração AEM 3D com o Autodesk Maya](/help/assets/integrate-maya-with-3d.md). |
| MAX | Máx. de Autodesk 3ds Nativo | Somente Windows | O Autodesk 3ds Max é necessário no nó do autor para ativar esse formato de arquivo. Consulte [Integrar AEM 3D com Autodesk 3ds Max](/help/assets/integrating-aem-3d-with-autodesk-3ds-max.md). |
...
* **Download de ativos 3D com dependentes e execuções** - Ao baixar ativos 3D de AEM com  **** Dependências e  **Representações** marcadas, o download inclui não apenas as representações do ativo 3D principal, mas também as representações de todos os dependentes.
...
* **Arquivos**  Max (.max) de 3ds do Autodesk - Se o Autodesk 3ds Max estiver instalado e configurado nos nós do Autor, AEM oferece suporte à ingestão e conversão de arquivos .max. O uso de arquivos .max como estágios não é suportado no momento.
...
* **Brilho dos estágios**  do IBL - A análise automática da imagem IBL pode resultar em uma cena que é muito escura ou muito brilhante. Para ajustar o brilho da iluminação dos estágios IBL, navegue até **Propriedades básicas** e ajuste o valor **bright** de **Iluminação do Ambiente** conforme necessário.

### Componente 3D da AEM Sites {#aem-sites-d-component}
...
* **visualização 3D ausente ao visualizar em Sites**  - ao usar  **** Visualizar sites, a página deve ser recarregada no navegador para inicializar totalmente o visualizador 3D. Esse não é um problema quando você visualização a página da Web diretamente (ou seja, quando `edit.html` é removido do caminho) nos nós Autor ou Publicar.
...
* **Configuração**  do componente 3D - você deve instalar o 3D Feature Pack em todos os nós do Publish ativos e cada nó deve ser configurado com o  **CRXDE** Litto para as mesmas opções de configuração em  `/libs/settings/dam/v3D/WebGLSites`.

* **Texturas ausentes, plano de fundo ou iluminação após a publicação**  - O mecanismo de  **** publicação no AEM Sites publica automaticamente as dependências primárias da página, incluindo o modelo 3D e o estágio 3D referenciado pelo componente 3D. Os estágios 3D e os modelos 3D normalmente dependem de ativos secundários para imagens IBL e mapas de textura, que o mecanismo de publicação de sites não publica automaticamente. Solução: publicar todos os ativos 3D dos Ativos antes de publicar a página da Web dos Sites. Isso garante que todas as dependências para ativos 3D estejam disponíveis nos nós Publicar.
