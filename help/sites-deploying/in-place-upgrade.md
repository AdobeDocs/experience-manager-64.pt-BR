...
Antes de executar a atualização, há várias etapas que devem ser concluídas. Consulte [Atualizando código e personalizações](/help/sites-deploying/upgrading-code-and-customizations.md) e [Tarefas de manutenção de pré-atualização](/help/sites-deploying/pre-upgrade-maintenance-tasks.md) para obter mais informações. Além disso, verifique se o sistema atende aos requisitos para a nova versão do AEM. Veja como o Detector de padrões pode ajudá-lo a estimar a complexidade de sua atualização e também consulte a seção Escopo e requisitos de atualização de [Planejando sua atualização](/help/sites-deploying/upgrade-planning.md) para obter mais informações.

## Pré-requisitos de Migração {#migration-prerequisites}

* **Versão mínima necessária do Java:** a ferramenta de migração só funciona com versões 7 e posteriores do Java. Observe que para AEM 6.3 e superior, o Oracle JRE 8 e o JRE 7 e 8 da IBM são as únicas versões compatíveis.
...
1. Baixe o novo arquivo jar AEM e use-o para substituir o arquivo antigo fora da pasta `crx-quickstart`.
...
Essa migração não é necessária se você estiver atualizando do AEM 6.3. Para versões anteriores à 6.3, o Adobe fornece uma ferramenta que pode ser usada para migrar o repositório para a nova versão da barra de segmentos Oak presente no AEM 6.3. Ele é fornecido como parte do pacote de início rápido e é obrigatório para qualquer atualização que esteja usando o TarMK. As atualizações de ambientes que usam MongoMK não exigem migração de repositório. Para obter mais informações sobre os benefícios do novo formato de barra de segmentos, consulte [Migração para Oak Segment Tar FAQ](/help/sites-deploying/revision-cleanup.md#online-revision-cleanup-frequently-asked-questions).

A migração real é realizada usando o arquivo jar AEM quickstart padrão, executado com uma nova opção `-x crx2oak` que executa a ferramenta crx2oak para simplificar a atualização e torná-la mais robusta.
...
Em que `<<YOUR_PROFILE>>` e `<<ADDITIONAL_FLAGS>>` são substituídos pelo perfil e sinalizadores listados na tabela a seguir:
...
* Se você estiver executando a atualização em um sistema Windows em que o mapeamento da memória Java não é manipulado corretamente, adicione o parâmetro `--disable-mmap` ao comando.

* Se você estiver usando o Java 7, adicione o parâmetro `-XX:MaxPermSize=2048m` logo após o parâmetro `-Xmx`.

Para obter instruções adicionais sobre como usar a ferramenta crx2oak, consulte Usar a [Ferramenta de migração CRX2Oak](/help/sites-deploying/using-crx2oak.md). O JAR auxiliar do crx2oak pode ser atualizado manualmente se necessário, substituindo-o manualmente por versões mais recentes depois de desempacotar o início rápido. Sua localização na pasta de instalação AEM é: `<aem-install>/crx-quickstart/opt/extensions/crx2oak.jar`. A versão mais recente da ferramenta de migração CRX2Oak está disponível para download no Repositório de Adobe em: [https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/crx2oak/](https://repo.adobe.com/nexus/content/groups/public/com/adobe/granite/crx2oak/)

Se a migração tiver sido concluída com êxito, a ferramenta sairá com um código de saída zero. Além disso, verifique se há mensagens WARN e ERROR no arquivo `upgrade.log`, localizado em `crx-quickstart/logs` no diretório de instalação AEM, pois isso pode indicar erros não fatais que ocorreram durante a migração.

Verifique os arquivos de configuração abaixo da pasta `crx-quickstart/install`. Se uma migração for necessária, ela será atualizada para refletir o repositório do público alvo.
...
Em que `/path/to/datastore` representa o caminho para o armazenamento de dados do arquivo.

## Realização Da Atualização {#performing-the-upgrade}

**Se estiver usando S3:**

1. Remova quaisquer frascos abaixo de `crx-quickstart/install` associados a uma versão anterior do conector S3.
...
### Determinando o comando correto do start de atualização {#determining-the-correct-upgrade-start-command}

Para executar a atualização, é importante start AEM usando o arquivo jar para exibir a instância. Para atualizar para a versão 6.4, veja também outras opções de reestruturação e migração de conteúdo em [Migração de conteúdo ociosa](/help/sites-deploying/lazy-content-migration.md) que você pode escolher com o comando de atualização.
...
## Implantar base de códigos atualizada {#deploy-upgraded-codebase}

Depois que o processo de atualização no local for concluído, a base de código atualizada deverá ser implantada. As etapas para atualizar a base de código para funcionar na versão de público alvo do AEM podem ser encontradas na página [Código de atualização e Personalizações](/help/sites-deploying/upgrading-code-and-customizations.md).

## Execute verificações e solução de problemas pós-atualização {#perform-post-upgrade-check-troubleshooting}

Consulte [Verificações e solução de problemas de pós-atualização](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md).
