...
>[!NOTE]
>
>O recurso Cold Standby destina-se a proteger cenários em que a alta disponibilidade é necessária em **autor** instâncias. Para situações em que a alta disponibilidade é necessária em **publicar** instâncias que usam o Tar Micro Kernel, o Adobe recomenda usar um farm de publicação.
>
>Para obter informações sobre mais implantações disponíveis, consulte o [Implantações recomendadas](/help/sites-deploying/recommended-deploys.md) página.

>[!NOTE]
>
>Quando a instância Standby é configurada ou derivada do nó Primary , ela só permite acesso ao seguinte console (para atividades relacionadas à administração):
>
>* Console da Web OSGI
>
...
Para criar uma configuração TarMK de standby frio, primeiro é necessário criar as instâncias de standby executando uma cópia do sistema de arquivos de toda a pasta de instalação do principal em um novo local. Você pode iniciar cada instância com um modo de execução que especificará sua função ( `primary` ou `standby`).

Abaixo está o procedimento que precisa ser seguido para criar uma configuração com uma instância principal e uma standby:

1. Instalar AEM.

1. Desligue sua instância e copie sua pasta de instalação para o local onde a instância do standby frio será executada. Mesmo que seja executado de máquinas diferentes, atribua a cada pasta um nome descritivo (como *aem-primary* ou *aem-standby*) para diferenciar as instâncias.
1. Vá para a pasta de instalação da instância primária e:
   ...
   1. Crie uma pasta chamada `install.primary` under `aem-primary/crx-quickstart/install`
   ...
   1. Se estiver usando uma instância AEM TarMK com um armazenamento de dados externo, crie uma pasta chamada `crx3` under `aem-primary/crx-quickstart/install` nomeado `crx3`
...
1. Crie um novo Apache Sling Logging Logger para a **org.apache.jackrabbit.oak.segment** pacote. Defina o nível de log como &quot;Depurar&quot; e aponte sua saída de log para um arquivo de log separado, como */logs/tarmk-coldstandby.log*. Para obter mais informações, consulte [Registro](/help/sites-deploying/configure-logging.md).
1. Vá para o local da variável **secundário** e inicie-a executando o jar.
...
1. Em seguida, prepare a instância de standby. Você pode fazer isso executando as mesmas etapas da instância primária:
   ...
   1. Exclua quaisquer arquivos que você possa ter em `aem-standby/crx-quickstart/install`.
   1. Crie uma nova pasta chamada `install.standby` under `aem-standby/crx-quickstart/install`
   1. Crie dois arquivos de configuração chamados:

      * `org.apache.jackrabbit.oak.segment.SegmentNodeStoreService.config`
      * `org.apache.jackrabbit.oak.segment.standby.store.StandbyStoreService.config`
   1. Crie uma nova pasta chamada `crx3` under `aem-standby/crx-quickstart/install`
   1. Criar a configuração do armazenamento de dados e colocá-la em `aem-standby/crx-quickstart/install/crx3`. Neste exemplo, o arquivo que você precisa criar é:
...
1. Inicie o **secundário** usando o modo de execução standby:
...
>[!NOTE]
>
>Você pode verificar a função de uma instância a qualquer momento verificando a presença do **principal** ou **secundário** runmodes no Console Web de Definições do Sling.
>
>Isso pode ser feito indo para *http://localhost:4502/system/console/status-slingsettings* e verificar **&quot;Modos de execução&quot;** linha.
...
Além disso, ao executar com um `FileDataStore`, mensagens como as seguintes confirmarão que os arquivos binários estão sendo transmitidos corretamente:
...
* **Porta (porta):** a porta a utilizar para comunicação. O padrão é `8023`.

* **Host principal (`primary.host`):** - o host da instância primária. Esta configuração só é aplicável para o modo de espera.
...
* **Tempo limite de leitura em espera (`standby.readtimeout`):** Tempo limite para solicitações emitidas da instância de standby em milissegundos. O valor padrão usado é 60000 (um minuto).
...
>
>A melhor maneira de garantir que isso seja abordado é excluindo o *sling.id* no standby e reinicie a instância.
...
1. Faça backup do `crx-quickstart` pasta da pasta de instalação standby. Pode ser usado como ponto de partida ao configurar um novo standby.

1. Reinicie a instância usando o `primary` runmode:
...
1. Crie e inicie uma nova instância de standby. Para obter mais informações, consulte o procedimento acima em [Criando uma configuração AEM TarMK Cold Standby](/help/sites-deploying/tarmk-cold-standby.md#creating-an-aem-tarmk-cold-standby-setup).
...
1. Pare o processo de sincronização na instância do standby remoto indo para o Console JMX e usando o **org.apache.jackrabbit.oak: Status (&quot;Standby&quot;)** feijão. Para obter mais informações sobre como fazer isso, consulte a seção sobre [Monitoramento](#monitoring).
...
1. Reconfigure o clone recém-criado para atuar como uma instância de espera fria. Para obter mais detalhes, consulte [Criando uma configuração AEM TarMK Cold Standby.](/help/sites-deploying/tarmk-cold-standby.md#creating-an-aem-tarmk-cold-standby-setup)
1. Inicie as instâncias de standby primário e frio.

## Monitoramento {#monitoring}

O recurso expõe informações usando JMX ou MBeans. Fazendo isso, você pode inspecionar o estado atual do standby e o principal usando o [Console JMX](/help/sites-administering/jmx-console.md). As informações podem ser encontradas em um MBean de `type org.apache.jackrabbit.oak:type="Standby"`nomeado `Status`.
...
* `SecondsSinceLastSuccess:` o número de segundos desde a última comunicação bem-sucedida com o servidor. Ele será exibido `-1` se não tiver sido feita qualquer comunicação bem-sucedida.
...
1. Pare o processo de standby na instância de standby indo para o Console JMX e usando o **org.apache.jackrabbit.oak: Status (&quot;Standby&quot;)** feijão. Para obter mais informações sobre como fazer isso, consulte a seção acima em [Monitoramento](/help/sites-deploying/tarmk-cold-standby.md#monitoring).
...
1. Execute o `cleanup()` na instância de standby, usando o mesmo bean JMX descrito na primeira etapa.
...
1. Execute a manutenção do repositório de espera passiva conforme descrito na seção [above](/help/sites-deploying/tarmk-cold-standby.md#cold-standby-repository-maintenance).
1. Após a conclusão do processo de manutenção e reinício das instâncias:
   ...
   * No standby, a coleta de lixo do armazenamento de dados fica disponível somente por meio do **BlobGarbageCollection** MBean - `startBlobGC()`. O **RepositoryManagement** O MBean não está disponível no modo de espera.
