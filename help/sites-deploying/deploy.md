...
* **Servidor** de Aplicações Web: O AEM pode ser implantado no modo independente (inclui um servidor Web Jetty integrado) ou como um aplicativo da Web em um servidor de aplicativos de terceiros (WebLogic, WebSphere etc).
...
1. Para iniciar a instância em um ambiente GUI, basta clicar duas vezes no **`cq-quickstart-6.4.0.jar`** arquivo.
