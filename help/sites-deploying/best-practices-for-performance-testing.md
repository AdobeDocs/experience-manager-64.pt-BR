...
* Quando se trata de testar sites móveis ou responsivos, um conjunto separado de ferramentas precisa ser usado. Eles funcionam diminuindo a largura de banda da rede, simulando conexões móveis mais lentas, como 3G ou EDGE. Entre as ferramentas mais utilizadas estão:
   ...
   * **[Condicionador de links de rede](https://nshipster.com/network-link-conditioner/)** - fornece uma interface de usuário fácil de usar e funciona em um nível relativamente baixo na pilha de rede. Ele inclui versões para OS X e iOS; [](https://nshipster.com/network-link-conditioner/)
   * [**Charles**](https://www.charlesproxy.com/) - um aplicativo proxy de depuração da Web que, além de várias outras utilizações, fornece controle de rede. As versões são fornecidas para Windows, OS X e Linux. [](https://www.charlesproxy.com/)
...
O **ObservationQueueMaxLength** A seção mostrará o número de eventos na fila de observação do Oak nas últimas horas, minutos, segundos e semanas. Encontre o maior número de eventos na seção &quot;por hora&quot;. Esse número precisa ser comparado ao `oak.observation.queue-length` , que pode ser encontrada na variável **SlingRepositoryManager** no [Console do OSGi](/help/sites-deploying/web-console.md). Se o número mais elevado mostrado para a fila de observação exceder o valor de `queue-length` , entre em contato com o Suporte do Adobe para obter ajuda com a elevação da configuração. A configuração padrão é 1.000, mas a maioria das implantações geralmente precisa aumentá-la para 20.000 ou 50.000.
