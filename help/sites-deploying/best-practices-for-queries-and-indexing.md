...
Além disso, leia a documentação do [Oak sobre como escrever query e índices](/help/sites-deploying/queries-and-indexing.md). Além de os índices serem um novo conceito no AEM 6, há diferenças sintáticas nos query Oak que precisam ser levadas em conta ao migrar o código de uma instalação AEM anterior.
...
* Por meio das APIs [do](/help/sites-developing/querybuilder-api.md) QueryBuilder (recomendado)
...
>[!NOTE]
>
>Ao usar o QueryBuilder, ele determinará a contagem de resultados por padrão, que é mais lenta no Oak em comparação às versões anteriores do Jackrabbit. Para compensar isso, você pode usar o parâmetro [thinkTotal](/help/sites-developing/querybuilder-api.md#using-p-guesstotal-to-return-the-results).

### A ferramenta Explicar Query {#the-explain-query-tool}

Como em qualquer idioma do query, a primeira etapa para otimizar um query é entender como ele será executado. Para ativar essa atividade, você pode usar a ferramenta [](/help/sites-administering/operations-dashboard.md#explain-query) Explicar Query que faz parte do Painel Operações. Com essa ferramenta, um query pode ser conectado e explicado. Será exibido um aviso se o query causar problemas em um repositório grande, bem como no tempo de execução e nos índices que serão usados. A ferramenta também pode carregar uma lista de query lentos e populares que podem ser explicados e otimizados.

### Registro DEBUG para Query {#debug-logging-for-queries}

Para obter informações adicionais sobre como o Oak está escolhendo qual índice usar e como o mecanismo de query está executando um query, uma configuração de registro **DEBUG** pode ser adicionada aos seguintes pacotes:
...
Para obter mais informações sobre como fazer isso, consulte a documentação [de](/help/sites-deploying/configure-logging.md)registro.
...
Para estatísticas de query, dê uma olhada nas estatísticas **do Query** Oak do MBean.

Se você quiser acessar seus índices usando uma ferramenta como [Luke](https://code.google.com/p/luke/), será necessário usar o console Oak para descarregar o índice do diretório `NodeStore` para um diretório filesystem. Para obter instruções sobre como fazer isso, leia a documentação [do](https://jackrabbit.apache.org/oak/docs/query/lucene.html)Lucene.
...
* Se o query usar a classificação, tenha uma definição de propriedade explícita para a propriedade classificada e defina `ordered` para `true` ela. Isso permitirá que os resultados sejam ordenados como tal no índice e economizados em operações de classificação dispendiosas no momento da execução do query.
...
>[!NOTE]
>
>Para obter mais informações sobre oak-mongo.js, consulte a seção [Ferramentas de linha de](https://jackrabbit.apache.org/oak/docs/command_line.html) comando da documentação do Oak.

## Indexação novamente {#re-indexing}

Esta seção descreve os **únicos** motivos aceitáveis para reindexar os índices Oak.

Fora das razões descritas abaixo, iniciar reíndices de índices Oak **não** alterará o comportamento ou resolverá problemas e aumentará desnecessariamente a carga em AEM.
...
* Como resolver:
   ...
   * [Indexar](/help/sites-deploying/best-practices-for-queries-and-indexing.md#how-to-re-index) novamente o índice de lúcene
...
* Como resolver:
   ...
   * Versões de carvalho anteriores à versão 1.6:
      ...
      * [Indexar](#how-to-re-index) novamente o índice de lúcene
   * Oak versões 1.6+
      ...
      * Caso contrário, [reindexe](#how-to-re-index) o índice de lúcene
...
Se ocorrer um problema em AEM que não corresponda aos critérios descritos abaixo, **não** indexe novamente nenhum índice, pois isso não resolverá o problema.
...
* Como resolver:
   ...
   * Caso contrário, [indexe](#how-to-re-index) novamente *todos* os índices de lúcene
   * Nota:
...
* Como resolver:
   ...
   * Remover a cópia local do índice de lúcene
      ...
      1. Parar AEM
      1. Excluir a cópia local do índice de lucene em `crx-quickstart/repository/index`
      1. Reiniciar AEM
   * Se isso não resolver o problema e as `AsyncIndexUpdate` exceções persistirem, então:

      1. [Voltar a indexar](#how-to-re-index) o índice de erro
      1. Também arquivar um ticket de suporte [a](https://helpx.adobe.com/support.html) Adobe
...
>[!NOTE]
>
>No AEM 6.4, [oak-run.jar é o ÚNICO método](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) suportado para reindexação em repositórios MongoMK ou RDBMK.
...
#### Indexação dos índices de propriedade do Lucene {#re-indexing-lucene-property-indexes}

* Use [oak-run.jar para indexar](/help/sites-deploying/oak-run-indexing-usecases.md#usecase3reindexing) novamente o índice da propriedade Lucene.
...
>[!NOTE]
>
>A seção anterior resume e enquadra a orientação de reindexação do Oak a partir da documentação [do](https://jackrabbit.apache.org/oak/docs/query/indexing.html#reindexing) Apache Oak no contexto do AEM.
...
* A pré-extração de texto tem um impacto positivo alto quando a reindexação de texto completo de binários com texto pesado (PDF, Doc, TXT etc.), em que um repositório de imagens não terá a mesma eficiência, pois as imagens não contêm texto extraível.
...
Indexação novamente de um índice de luceno **existente** com extração binária ativada
...
Suporte à implantação de um **novo** índice de lúcene para AEM com extração binária ativada
...
1 bis. Execute `oak-run.jar --generate` para criar uma lista de nós que terá seu texto pré-extraído.
...
3-A. [O reíndice](#how-to-re-index) de índices Lucene é chamado em AEM
