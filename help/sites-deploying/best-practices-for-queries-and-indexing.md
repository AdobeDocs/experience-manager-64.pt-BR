...
seo-title: Práticas recomendadas para consultas e indexação
description: Este artigo fornece orientações sobre como otimizar seus índices e consultas.
seo-description: Este artigo fornece orientações sobre como otimizar seus índices e consultas.
...
Além disso, leia a documentação do [Oak em consultas e índices](/help/sites-deploying/queries-and-indexing.md)de gravação. Além de os índices serem um novo conceito no AEM 6, há diferenças sintáticas nas consultas do Oak que precisam ser levadas em conta ao migrar o código de uma instalação anterior do AEM.

## Quando usar consultas {#when-to-use-queries}
...
* Como resolver:
   ...
   * Se isso não resolver o problema e as `AsyncIndexUpdate` exceções persistirem, então:
      ...
      1. Também é possível arquivar um ticket de suporte [da](https://helpx.adobe.com/support.html) Adobe
...
>[!NOTE]
>
>No AEM 6.4, [oak-run.jar é o ÚNICO método](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) compatível para reindexação em repositórios MongoMK ou RDBMK.
...
#### Quando pode ser usada a pré-extração de texto? {#when-can-text-pre-extraction-be-used}
...
#### Quando pode a pré-extração de texto NÃO ser usada? {#when-can-text-pre-extraction-not-be-used}
...
3-A. [O reíndice](#how-to-re-index) de índices Lucene é chamado no AEM
