...
Abaixo está a lista completa de `CodeUpgradeTasks` introdução em 6.4:
...
| `Cq62TargetCampaignsContentUpgrade` | &lt; 6.2 | Imediato | Introduz uma camada de container na hierarquia (Áreas) e ajusta as referências. |
| `Cq62TargetContentUpgrade` | &lt; 6.2 | Imediato | Define os nomes de localização fixos para os componentes do público alvo. |
...
| `CQ63AssetsSearchFacetsUpdate` | &lt; 6.3 | Imediato | Move ativos e aspectos de pesquisa personalizados de **/apps** para **/conf** e converte os formulários de schema de metadados e perfis de metadados de coral2 para coral3. |
| `CQ63InboxItemsUpgrade` | &lt; 6.3 | Imediato | Atualiza InboxItems para ordenar itens de caixa de entrada (ajustando metadados para classificação eficiente) |
...
| `CQ63ProcessingProfileConfigUpdate` | &lt; 6.3 | Imediato | Traduz a propriedade processingProfile (usada até 6.1) em Ativos para corresponder à estrutura 6.3 e posterior. Além disso, ajusta os caminhos relativos do perfil para **/conf** no lugar de **/aplicativos**. |
| `CQ63ToolsMenuEntriesContentUpgrade` | &lt; 6.3 | Imediato | tarefa de atualização que remove entradas de menu obsoletas do CRXDE Lite e do Console da Web no caso de uma atualização. |
