...
description: Uma lista das plataformas de cliente e servidor suportadas para o AEM.
seo-description: Uma lista das plataformas de cliente e servidor suportadas para o AEM.
...
* Arquivo de início rápido do Experience Manager (JAR independente ou WAR de implantação de aplicativos da Web)
...
Este documento lista as plataformas de cliente e servidor suportadas para o Adobe Experience Manager. A Adobe fornece vários níveis de suporte, tanto para configurações recomendadas quanto para outras configurações.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Para garantir o sucesso do projeto de nossos clientes, a Adobe fornece suporte completo dentro de um programa de suporte restrito, que exige que condições específicas sejam atendidas. O suporte de nível R requer uma solicitação formal do cliente e uma confirmação da Adobe. Para obter mais informações, entre em contato com o Atendimento ao cliente da Adobe.</td> 
  </tr> 
 </tbody> 
</table>
...
1. O suporte e a distribuição do JDK do Oracle Java SE, incluindo todas as atualizações de manutenção das versões LTS além do final das atualizações públicas, serão suportados pela Adobe diretamente para todos os clientes do AEM que fizerem uso da tecnologia Oracle Java SE. Consulte o suporte [Oracle Java para perguntas e respostas](assets/adobe-oracle-java-license-agreement.pdf) do Adobe Experience Manager para obter mais informações.

### Armazenamento e persistência {#storage-persistence}

Existem várias opções para implantar o repositório do Adobe Experience Manager. Consulte a lista a seguir para obter as tecnologias e opções de armazenamento suportadas.
...
1. O MongoDB Storage Engine WiredTiger é compatível apenas.
1. Não é compatível com o AEM Forms.
1. O MongoDB Enterprise 3.6 é compatível a partir do AEM versão 6.4.2.0.

>[!NOTE]
>
>Consulte [Implantação de comunidades](/help/communities/deploy-communities.md) para obter mais informações sobre a capacidade do AEM Communities.
...
>
>O Atendimento ao cliente da Adobe ajudará a solucionar problemas relacionados ao uso do MongoDB com o AEM.
>
...
>[!NOTE]
>
>Os bancos de dados relacionais compatíveis, conforme listados acima, são softwares de terceiros e não estão incluídos no pacote de licenciamento do AEM.
>
>Para executar o AEM 6.4 com um banco de dados relacional suportado, é necessário um contrato de suporte separado com um fornecedor de banco de dados. O Atendimento ao cliente da Adobe ajudará a solucionar problemas relacionados ao uso de bancos de dados relacionais com o AEM 6.4.
>
...
1. O Linux Kernel 2.6, 3.x e 4.x inclui derivados da distribuição Red Hat, incluindo Red Hat Enterprise Linux, CentOS, Oracle Linux e Amazon Linux. Os recursos de complemento de formulário AEM só são compatíveis com CentOS 7 e Red Hat Enterprise Linux 6.5 e 7.
1. AEM Assets: Consulte a seção [Suporte para gravação de metadados XMP](#requirements-for-aem-assets-xmp-metadata-write-back)
...
A Adobe recomenda usar o Adobe Managed Services para implantar o AEM no Azure ou AWS. Os serviços gerenciados da Adobe fornecem aos especialistas experiência e habilidades de implantação e operação do AEM nesses ambientes de computação em nuvem. Verifique nossa documentação [adicional sobre os Serviços](https://www.adobe.com/marketing-cloud/enterprise-content-management/managed-services-cloud-platform.html?aemClk=t)gerenciados da Adobe.

Em todos os outros casos de implantação do AEM no Azure ou AWS, ou em qualquer outro ambiente de computação em nuvem, o suporte da Adobe será contido no ambiente de computação virtual, de acordo com as especificações técnicas listadas nesta página. Qualquer problema reportado relativo ao AEM em execução em qualquer um desses ambientes de nuvem precisará ser reprodutível independentemente de qualquer serviço de nuvem específico ao ambiente de computação em nuvem, a menos que o serviço de nuvem seja especificamente suportado como parte dos requisitos técnicos listados nesta página, por exemplo, o armazenamento do Azure Blob ou o AWS S3.

Para obter recomendações sobre como implantar o AEM no Azure ou AWS, fora dos Serviços gerenciados da Adobe, recomendamos trabalhar diretamente com o provedor de nuvem ou parceiros da Adobe que suportam a implantação do AEM no ambiente de nuvem de sua escolha. O provedor ou parceiro de nuvem selecionado será responsável pelas especificações de dimensionamento, design e implementação da arquitetura. Para atender aos seus requisitos específicos de desempenho, carga, escalabilidade e segurança.

### Plataformas do Dispatcher (Servidores Web) {#dispatcher-platforms-web-servers}

O Dispatcher é o componente de cache e balanceamento de carga. [Baixe a versão](https://helpx.adobe.com/experience-manager/dispatcher/release-notes.html)mais recente do Dispatcher. O Experience Manager 6.4 requer a versão 4.3.1 ou superior do Dispatcher.
...
A interface do usuário do Adobe Experience Manager funciona com as seguintes plataformas clientes. Todos os navegadores são testados com o conjunto padrão de plug-ins e suplementos.

A interface do usuário do AEM é otimizada para telas maiores (geralmente notebooks e desktops) e fator de forma de tablet (como o Apple iPad ou o Microsoft Surface). O fator de forma de telefone não é suportado.

>[!NOTE]
>
>**Suporte para navegadores com ciclos de lançamento rápido:**
>
>Mozilla Firefox, Google Chrome e Microsoft Edge lançam atualizações a cada poucos meses. A Adobe tem o compromisso de fornecer atualizações para que o Adobe Experience Manager mantenha o nível de suporte, conforme declarado abaixo, com as versões futuras desses navegadores.
...
Geralmente, o suporte de navegador para sites renderizados pelo AEM Sites depende da implementação de modelos de página do AEM, design e saída de componente, e portanto está sob o controle da parte que implementa essas partes.

### Clientes WebDAV {#webdav-clients}

**Microsoft Windows 7+**

Para se conectar com êxito ao Microsoft Windows 7+ em uma instância do AEM que não esteja protegida com SSL, a autenticação básica por rede não segura deve estar habilitada no Windows. Isso requer uma alteração no Registro do Windows do WebClient:
...
O AEM Dynamic Media está desativado por padrão. See [Enabling Dynamic Media](/help/assets/config-dynamic.md#enabling-dynamic-media).
...
>
>Para os clientes do Dynamic Media que executam o modo Dynamic Media - Scene7 (ou seja, **dynamicmedia_sceno7** runmode), não há requisitos de sistema adicionais; somente os mesmos requisitos de sistema do AEM. Mídia dinâmica - A arquitetura do modo Scene7 usa o serviço de imagem baseado em nuvem, não o serviço incorporado no AEM.
...
>
>Portanto, a Adobe recomenda que, se você estiver executando um sistema desse tipo, desative a NUMA usando a opção de **inicialização numa=off** para evitar que o kernel mate esses processos.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p><a href="https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html">Faixa clássica do Acrobat 2017</a></p> </td> 
...
>Além disso:
>
>* O Gerador de PDF requer o rastreamento clássico do [Acrobat 2017 versão 17.011.30078 ou posterior](https://helpx.adobe.com/acrobat/release-note/release-notes-acrobat-reader.html) para executar a conversão.
>* O AEM Forms suporta apenas edições de 32 bits de software suportado.
>* Os recursos OCR PDF (PDF pesquisável), Otimizar PDF e Exportar PDF são suportados apenas no Microsoft Windows.
...
### Requisitos para o write-back de metadados XMP do AEM Assets {#requirements-for-aem-assets-xmp-metadata-write-back}

A gravação XMP é suportada e ativada para as seguintes plataformas e formatos de arquivo:

**Sistemas operacionais**

* Linux (32 bits, requer suporte para aplicativos de 32 bits em sistemas de 64 bits). Para obter etapas para instalar bibliotecas de clientes de 32 bits, consulte [Como ativar a extração e gravação de XMP em RedHat Linux](https://helpx.adobe.com/experience-manager/kb/enable-xmp-write-back-64-bit-redhat.html)de 64 bits.
