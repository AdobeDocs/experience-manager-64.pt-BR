...
seo-description: Saiba mais sobre o procedimento que precisa seguir para atualizar AEM.
...
## Camada do autor do TarMK {#tarmk-author-tier}
...
### Atualizar Execução {#upgrade-execution-1}

![execute_upgrade](assets/execute_upgrade.jpg)

1. Execute o [upgrade no local](/help/sites-deploying/in-place-upgrade.md)
...
1. Atualize o arquivo `DocumentNodeStoreService.cfg` no Autor principal para refletir seu conjunto de réplicas de membro único
...
1. Execute [tarefas de manutenção de pré-atualização](/help/sites-deploying/pre-upgrade-maintenance-tasks.md) na instância principal do autor
1. Se necessário, atualize o MongoDB na instância principal do Mongo para a versão 3.2 com WiredTiger

### Atualizar Execução {#execution}

![execução mongo](assets/mongo-execution.jpg)

1. Execute um [upgrade no local](/help/sites-deploying/in-place-upgrade.md) no Autor principal
1. Atualize o Dispatcher ou o Módulo Web *se necessário*
1. O controle de qualidade valida a atualização

### Se tiver êxito {#successful-1}
...
1. Atualize os arquivos `DocumentNodeStoreService.cfg` para refletir o conjunto completo de réplicas
...
1. Configure os arquivos `DocumentNodeStoreService.cfg` nas instâncias secundárias do autor para apontar para o conjunto de réplicas de instâncias Mongo ainda não atualizadas
...
### Atualizar Execução {#execution-upgrade}

![upgrade-publish2](assets/upgrade-publish2.png)

1. Pare o tráfego para a instância Publicar 2 no balanceador de carga
1. Execute [manutenção de pré-atualização](/help/sites-deploying/pre-upgrade-maintenance-tasks.md) no Publish 2
1. Execute um [upgrade no local](/help/sites-deploying/in-place-upgrade.md) no Publish 2
1. Atualize o Dispatcher ou o Módulo Web *se necessário*
...
1. Execute [verificações pós-atualização](/help/sites-deploying/post-upgrade-checks-and-troubleshooting.md).
