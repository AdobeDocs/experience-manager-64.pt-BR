...
A reindexação offline do TarMK é a abordagem de reindexação mais simples `oak-run.jar` baseada no TarMK, pois requer um único `oak-run.jar` comentário. No entanto, requer que a instância AEM seja desligada.
...
Se o impacto no tempo ou no desempenho da reindexação usando não `oak-run.jar` métodos for muito alto, a seguinte abordagem `oak-run.jar` baseada pode ser usada para importar e reindexar as definições do Índice Lucene em uma instalação AEM com base no TarMK.
