...
Após a atualização [no local](/help/sites-deploying/in-place-upgrade.md) , as seguintes atividades devem ser executadas para finalizar a atualização. Pressupõe-se que o AEM tenha sido iniciado com o jar 6.4 e que a base de código atualizada tenha sido implantada.
...
Após a atualização, você verá que a versão Oak foi atualizada para **1.8.2**. Para verificar a versão Oak, navegue até o console OSGi e verifique a versão associada aos pacotes Oak: Oak Core, Oak Commons, Oak Segment Tar.
...
Aplique todos os Service Packs relevantes do AEM 6.4 se eles tiverem sido lançados.
...
Esta seção contém alguns cenários de problema que podem ser enfrentados durante o procedimento de atualização para o AEM 6.4.
...
Depois de atualizar para o AEM 6.4 de uma versão anterior, a Configuração da Dynamic Media Cloud de configurações anteriores pode ficar inacessível na interface do usuário do AEM 6.4 TouchUI. Para resolver esse problema, use o CRXDE Lite para remover as configurações anteriores e crie uma nova Configuração da Dynamic Media Cloud. Consulte também a reestruturação do repositório do [Dynamic Media no AEM 6.4](/help/sites-deploying/dynamicmedia-repository-restructuring-in-aem-6-4.md).
